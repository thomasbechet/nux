...
{
    // Create module
    ctx->debug = nux_arena_alloc(&ctx->core_arena, sizeof(*ctx->debug));
    NUX_CHECK(ctx->debug, return NUX_FAILURE);
    nux_debug_module_t *module = ctx->debug;

    // Create debug console
    if (ctx->config.debug.console)
    {
        // Create canvas
    ...
    // Create log buffer
    module->lines
        = nux_arena_alloc(&ctx->core_arena, LOG_LINE_LEN * LOG_LINE_COUNT);
...
void
nux_debug_free (nux_ctx_t *ctx)
{
}
void
nux_debug_update (nux_ctx_t *ctx)
{
    nux_debug_module_t *module = ctx->debug;
    ...
    {
        nux_canvas_text(ctx,
                        module->console_canvas,
                        10,
                        y,
                        module->lines + LOG_LINE_LEN * i);
        y += 10;
    }
}
void
nux_debug_log_callback (nux_ctx_t      *ctx,
