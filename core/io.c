...
nux_io_load (nux_ctx_t *ctx, const nux_c8_t *path, nux_u32_t *size)
{
    ...
    {
        NUX_ERROR("Failed to open file %s", path);
        return NUX_NULL;
    }

    // Get the file size
    nux_file_stat_t stat;
    if (!nux_io_stat(ctx, &file, &stat))
    {
        NUX_ERROR("Failed to stat file");
        goto cleanup;
    }

    // Seek to beginning
    if (!nux_io_seek(ctx, &file, 0))
    {
        NUX_ERROR("Failed to seek file");
        goto cleanup;
    }
    ...
    if (read != stat.size)
    {
        NUX_ERROR("Failed to read file content");
        data = NUX_NULL;
        goto cleanup;
    }
...
{
    if (nux_os_file_read(ctx, slot, p, n) != n)
    {
        return NUX_FAILURE;
    }
    return NUX_SUCCESS;
...
{
    if (nux_os_file_write(ctx, slot, p, n) != n)
    {
        return NUX_FAILURE;
    }
    return NUX_SUCCESS;
                       ...
                       nux_u32_t          size)
{
    ...
    // Write path + data
    if (!nux_os_file_seek(ctx, writer->slot, writer->cursor))
    {
    ...
    // Write entry
    if (!nux_os_file_seek(ctx, writer->slot, entry_cursor))
    {
