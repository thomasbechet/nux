...
nux_mesh_new (nux_ctx_t *ctx, nux_res_t arena, nux_u32_t capa)
{
    nux_res_t   res;
    nux_mesh_t *mesh
        = nux_arena_alloc_res(ctx, arena, NUX_RES_MESH, sizeof(*mesh), &res);
    NUX_CHECK(mesh, return NUX_NULL);
    mesh->count = capa;
    mesh->data
        = nux_arena_alloc(ctx, arena, sizeof(nux_f32_t) * 5 * mesh->count);
...
void
nux_mesh_gen_bounds (nux_ctx_t *ctx, nux_res_t mesh)
{
    nux_mesh_t *m = nux_res_check(ctx, NUX_RES_MESH, mesh);
    NUX_CHECK(m, return);
    if (m->bounds_first || m->count == 0)
    {
        return;
    }
    ...
    nux_graphics_push_vertices(
        ctx, NUX_ARRAY_SIZE(positions), data, &m->bounds_first);
}
