...
nux_gpu_buffer_init (nux_ctx_t *ctx, nux_gpu_buffer_t *buffer)
{
    ...
    NUX_CHECKM(slot, return NUX_FAILURE, "out of gpu buffer");
    buffer->slot = *slot;
...
nux_gpu_texture_init (nux_ctx_t *ctx, nux_gpu_texture_t *texture)
{
    ...
    NUX_CHECKM(slot, return NUX_FAILURE, "out of gpu textures");
    texture->slot = *slot;
    ...
    if (texture->type == NUX_TEXTURE_RENDER_TARGET)
    {
        ...
        NUX_CHECKM(slot, return NUX_FAILURE, "out of gpu framebuffer slots");
...
nux_gpu_pipeline_init (nux_ctx_t *ctx, nux_gpu_pipeline_t *pipeline)
{
    ...
    NUX_CHECKM(slot, return NUX_FAILURE, "out of gpu pipelines");
    pipeline->slot = *slot;
    NUX_CHECKM(
