...
nux_scene_draw (nux_ctx_t *ctx, nux_u32_t scene, nux_u32_t camera)
{
    ...
    // Push constants
    nux_entity_t *ce = nux_object_get(ctx, NUX_TYPE_ENTITY, camera);
    NUX_CHECK(ce, return);
    nux_transform_t *ct
...
nux_entity_new (nux_ctx_t *ctx, nux_u32_t scene)
{
    ...
    nux_u32_t id = nux_object_create(ctx, s->arena, NUX_TYPE_ENTITY, e);
    NUX_CHECK(id, return NUX_NULL);
...
void *
nux_scene_add_component (nux_ctx_t *ctx, nux_u32_t entity, nux_u32_t comp_type)
{
    nux_entity_t *e = nux_object_get(ctx, NUX_TYPE_ENTITY, entity);
    NUX_CHECK(e, return NUX_NULL);
    NUX_ASSERT(comp_type < ctx->component_types_count);
    nux_u32_t *comp_index = e->components + comp_type;
    NUX_ASSERT(comp_index);
    if (*comp_index)
    {
        nux_scene_remove_component(ctx, entity, comp_type);
    }
    nux_scene_t     *s = nux_object_get(ctx, NUX_TYPE_SCENE, e->scene);
...
void *
nux_scene_get_component (nux_ctx_t *ctx, nux_u32_t entity, nux_u32_t comp_type)
{
    nux_entity_t *e = nux_object_get(ctx, NUX_TYPE_ENTITY, entity);
    NUX_CHECK(e, return NUX_NULL);
    NUX_ASSERT(comp_type < ctx->component_types_count);
...
// }
// static nux_status_t
// nux_camera_parse (nux_ctx_t *ctx, nux_u32_t e, lua_State *L, nux_u32_t
// t_index)
// {
...
//         // Check is table
//         NUX_CHECKM(
//             lua_istable(L, -2), "Entity value is not a table", return
//             NUX_NULL);
//
//         // Create entity
...
//             {
//                 const nux_c8_t *key
//                     = lua_tostring(L, -1); // No side effect as key is a
//                     string
//
//                 // Check is table
