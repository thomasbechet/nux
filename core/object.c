...
void
nux_object_init_table (nux_instance_t inst,
                       nux_u32_t      capa,
                       nux_u32_t      static_capa)
{
    inst->objects_capa         = capa;
    inst->objects_count        = 0;
    inst->objects_static_head  = static_capa + 1; // reserve index 0 for NULL
...
void
nux_object_add_static (nux_env_t         env,
                       nux_u32_t         index,
                       nux_object_type_t type,
                       nux_u32_t         block)
{
    if (index == 0 || index > env->inst->objects_static_head)
    {
