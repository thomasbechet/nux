...
nux_create_texture (nux_env_t env, nux_oid_t oid, nux_u32_t size)
{
    ...
    {
        nux_set_error(env->inst, NUX_ERROR_INVALID_TEXTURE_SIZE);
        return NUX_FAILURE;
    }
    nux_object_t *object = nux_object_set(env->inst, oid, NUX_OBJECT_TEXTURE);
    NU_CHECK(object, return NUX_FAILURE);
    object->texture.size = size;
    NU_CHECK(
        nux_malloc(env->inst, nux_texture_memsize(size), &object->texture.data),
        return NUX_FAILURE);
    return NUX_SUCCESS;
}
void
...
{
    nux_object_t *object
        = nux_instance_get_object(env->inst, NUX_OBJECT_TEXTURE, oid);
