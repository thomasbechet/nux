...
void
nux_file_close (nux_ctx_t *ctx, nux_id_t id)
{
    nux_file_t *file = nux_id_check(ctx, NUX_TYPE_FILE, id);
    NUX_CHECK(file, return);
    nux_io_close(ctx, file);
}
nux_u32_t
nux_file_read (nux_ctx_t *ctx, nux_id_t id, void *data, nux_u32_t n)
{
    nux_file_t *file = nux_id_check(ctx, NUX_TYPE_FILE, id);
...
nux_u32_t
nux_file_write (nux_ctx_t *ctx, nux_id_t id, const void *data, nux_u32_t n)
{
    nux_file_t *file = nux_id_check(ctx, NUX_TYPE_FILE, id);
    NUX_CHECK(file, return 0);
...
nux_status_t
nux_file_seek (nux_ctx_t *ctx, nux_id_t id, nux_u32_t cursor)
{
    nux_file_t *file = nux_id_check(ctx, NUX_TYPE_FILE, id);
    NUX_CHECK(file, return 0);
