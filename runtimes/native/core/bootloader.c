...
load_texture (vm_t *vm, const cart_chunk_entry_t *entry)
{
    ...
    api_alloctex(vm, entry->extra.texture.index, size);
...
load_mesh (vm_t *vm, const cart_chunk_entry_t *entry)
{
    ...
    vm_log(vm,
           ...
           primitive,
           attributes);
    api_allocmesh(vm, entry->extra.mesh.index, count, primitive, attributes);
...
load_model (vm_t *vm, const cart_chunk_entry_t *entry)
{
    ...
    NU_CHECK(api_allocmodel(vm, entry->extra.model.index, node_count),
             return NU_FAILURE);
    vm_log(vm,
           NU_LOG_INFO,
