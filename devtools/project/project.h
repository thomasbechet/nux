...
typedef struct
{
    ...
    nu_char_t          prebuild[NUX_NAME_MAX];
} nux_project_t;

NU_API nu_bool_t nux_project_init_empty(nux_project_t *project, nu_sv_t path);
...
NU_API void nux_command_init(nu_sv_t path, nu_sv_t lang, nu_bool_t verbose);
NU_API void nux_command_build(nu_sv_t path, nu_bool_t verbose);
