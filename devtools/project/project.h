...
typedef struct
{
    ...
    project_chunk_entry_t *chunks;
    nu_size_t              chunks_count;
    nu_char_t              prebuild[PROJECT_NAME_MAX];
} project_t;

NU_API nu_status_t project_generate_template(nu_sv_t          path,
                                             nu_sv_t          lang,
                                             project_error_t *error);
NU_API nu_status_t project_build(const project_t *project,
                                 project_error_t *error);
NU_API nu_status_t project_load(project_t       *project,
                                nu_sv_t          path,
                                project_error_t *error);
NU_API nu_status_t project_save(const project_t *project,
                                nu_sv_t          path,
                                project_error_t *error);
