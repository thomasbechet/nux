...
/*
   ...
   included #define RGFW_PRINT_ERRORS - (optional) makes it so RGFW prints
   errors when they're found #define RGFW_OSMESA - (optional) use OSmesa as
   backend (instead of system's opengl api + regular opengl) #define RGFW_BUFFER
   - (optional) just draw directly to (RGFW) window pixel buffer that is drawn
   to screen (the buffer is in the RGBA format) #define RGFW_EGL - (optional)
   use EGL for loading an OpenGL context (instead of the system's opengl api)
        #define RGFW_OPENGL_ES1 - (optional) use EGL to load and use Opengl ES
   (version 1) for backend rendering (instead of the system's opengl api) This
   version doesn't work for desktops (I'm pretty sure) #define RGFW_OPENGL_ES2 -
   (optional) use OpenGL ES (version 2) #define RGFW_OPENGL_ES3 - (optional) use
   OpenGL ES (version 3) #define RGFW_DIRECTX - (optional) use directX for the
   rendering backend (rather than opengl) (windows only, defaults to opengl for
   unix) #define RGFW_WEBGPU - (optional) use webGPU for rendering (Web ONLY)
        #define RGFW_NO_API - (optional) don't use any rendering API (no opengl,
   no vulkan, no directX)
   ...
   RGFW_LINK_OSMESA (optional) (windows only) if EGL is being used, if OS Mesa
   functions should be defined dymanically  (using GetProcAddress)

        #define RGFW_X11 (optional) (unix only) if X11 should be used. This
   option is turned on by default by unix systems except for MacOS #define
   RGFW_WGL_LOAD (optional) (windows only) if WGL should be loaded dynamically
   during runtime #define RGFW_NO_X11_CURSOR (optional) (unix only) don't use
   XCursor #define RGFW_NO_X11_CURSOR_PRELOAD (optional) (unix only) Use
   XCursor, but don't link it in code, (you'll have to link it with -lXcursor)

        #define RGFW_NO_DPI - Do not include calculate DPI (no XRM nor libShcore
   included)

        #define RGFW_ALLOC_DROPFILES (optional) if room should be allocating for
   drop files (by default it's global data) #define RGFW_MALLOC x - choose what
   function to use to allocate, by default the standard malloc is used #define
   RGFW_CALLOC x - choose what function to use to allocate (calloc), by default
   the standard calloc is used #define RGFW_FREE x - choose what function to use
   to allocated memory, by default the standard free is used

        #define RGFW_EXPORT - Use when building RGFW
    #define RGFW_IMPORT - Use when linking with RGFW (not as a single-header)

        #define RGFW_STD_INT - force the use stdint.h (for systems that might
   not have stdint.h (msvc))
*/
...
/*
   ...
   documented, GLFW's source code was referenced and used throughout the project
   (used code is marked in some way), this mainly includes, code for drag and
   drops, code for setting the icon to a bitmap and the code for managing the
   clipboard for X11 (as these parts are not documented very well)
...
#define RGFW_HEADER
     ...
     !defined(RGFW_STD_INT)) /* MSVC might not have stdint.h */
...
#if defined(_WIN32) && !defined(RGFW_X11) &&                                   \
    !defined(RGFW_WEBASM) /* (if you're using X11 on windows some how) */
#define RGFW_WINDOWS
...
#elif (defined(__unix__) || defined(RGFW_MACOS_X11) || defined(RGFW_X11)) &&   \
    !defined(RGFW_WEBASM)
#define RGFW_MACOS_X11
#define RGFW_X11
#include <X11/Xlib.h>
...
#ifndef RGFW_ALPHA
#define RGFW_ALPHA                                                             \
  128 /* alpha value for RGFW_TRANSPARENT_WINDOW (WINAPI ONLY, macOS + linux   \
         don't need this) */
#endif

/*! Optional arguments for making a windows */
#define RGFW_TRANSPARENT_WINDOW                                                \
  (1L << 9) /*!< the window is transparent (only properly works on X11 and     \
               MacOS, although it's although for windows) */
#define RGFW_NO_BORDER (1L << 3) /*!< the window doesn't have border */
#define RGFW_NO_RESIZE                                                         \
  (1L << 4) /*!< the window cannot be resized  by the user */
#define RGFW_ALLOW_DND (1L << 5) /*!< the window supports drag and drop*/
#define RGFW_HIDE_MOUSE                                                        \
  (1L << 6) /*! the window should hide the mouse or not (can be toggled later  \
               on) using `RGFW_window_mouseShow*/
#define RGFW_FULLSCREEN                                                        \
  (1L << 8)                    /* the window is fullscreen by default or not */
#define RGFW_CENTER (1L << 10) /*! center the window on the screen */
#define RGFW_OPENGL_SOFTWARE (1L << 11) /*! use OpenGL software rendering */
#define RGFW_COCOA_MOVE_TO_RESOURCE_DIR                                        \
  (1L << 12) /* (cocoa only), move to resource folder */
#define RGFW_SCALE_TO_MONITOR (1L << 13) /* scale the window to the screen */
#define RGFW_NO_INIT_API                                                       \
  (1L << 2) /* DO not init an API (mostly for bindings, you should use         \
               `#define RGFW_NO_API` in C */

#define RGFW_NO_GPU_RENDER                                                     \
  (1L << 14) /* don't render (using the GPU based API)*/
#define RGFW_NO_CPU_RENDER                                                     \
  (1L << 15) /* don't render (using the CPU based buffer rendering)*/
...
typedef RGFW_ENUM(u8, RGFW_event_types){
    ...
    */
    ...
    RGFW_windowResized, /*!< the window was resized (by the user), [on webASM
                           this means the browser was resized] */
    ...
    */
    ...
    RGFW_dnd_init /*!< the start of a dnd event, when the place where the file
                     drop is known */
                  /* dnd data note
...
/*! joystick button codes (based on xbox/playstation), you may need to change
...
typedef struct RGFW_Event {
  ...
  u8 keyCode; /*!< keycode of event 	!!Keycodes defined at the bottom of the
                 RGFW_HEADER part of this file!! */

  b8 repeat;  /*!< key press event repeated (the key is being held) */
  b8 inFocus; /*!< if the window is in focus or not (this is always true for
                 MacOS windows due to the api being weird) */
...
/*! this has to be set before createWindow is called, else the fulscreen size is
 * used */
RGFWDEF void RGFW_setBufferSize(
    RGFW_area size); /*!< the buffer cannot be resized (by RGFW) */
...
/*!
   ...
   negative integer, the loop will not return until it gets another event
*/
typedef RGFW_ENUM(i32, RGFW_eventWait){RGFW_NEXT = -1, RGFW_NO_WAIT = 0};
...
RGFWDEF void RGFW_window_setIcon(
...
/*!< sets mouse to bitmap (very simular to RGFW_window_setIcon), image NOT
 * resized by default*/
RGFWDEF void RGFW_window_setMouse(RGFW_window *win, u8 *image, RGFW_area a,
                                  i32 channels);
...
RGFWDEF void RGFW_window_setMouseDefault(
...
this is run by default if the user uses the arg `RGFW_SCALE_TO_MONITOR` during
window creation
*/
...
/*error handling*/
RGFWDEF b8 RGFW_Error(void); /*!< returns true if an error has occurred (doesn't
                                print errors itself) */

/*! returns true if the key should be shifted */
RGFWDEF b8 RGFW_shouldShift(u32 keycode, u8 lockState);

/*! get char from RGFW keycode (using a LUT), uses shift'd version if shift =
 * true */
...
RGFWDEF b8 RGFW_isPressed(RGFW_window *win,
                          u8 key); /*!< if key is pressed (key code)*/

RGFWDEF b8 RGFW_wasPressed(
    RGFW_window *win,
    u8 key); /*!< if key was pressed (checks previous state only) (key code)*/

RGFWDEF b8 RGFW_isHeld(RGFW_window *win,
                       u8 key); /*!< if key is held (key code)*/
RGFWDEF b8 RGFW_isReleased(RGFW_window *win,
                           u8 key); /*!< if key is released (key code)*/
...
RGFWDEF b8 RGFW_isMousePressed(RGFW_window *win,
                               u8 button /*!< mouse button code */);
/*! if a mouse button is held */
RGFWDEF b8 RGFW_isMouseHeld(RGFW_window *win,
                            u8 button /*!< mouse button code */);
/*! if a mouse button was released */
RGFWDEF b8 RGFW_isMouseReleased(RGFW_window *win,
                                u8 button /*!< mouse button code */);
/*! if a mouse button was pressed (checks previous state only) */
RGFWDEF b8 RGFW_wasMousePressed(RGFW_window *win,
                                u8 button /*!< mouse button code */);
/** @} */
...
RGFWDEF void RGFW_clipboardFree(
    char
        *str); /*!< the string returned from RGFW_readClipboard must be freed */
 ...
 * keycode, the string version, the state of mod keys, if it was a press (else
 * it's a release) */
typedef void (*RGFW_keyfunc)(RGFW_window *win, u32 keycode, char keyName[16],
                             u8 lockState, b8 pressed);
 ...
 * it's a release)  */
typedef void (*RGFW_mousebuttonfunc)(RGFW_window *win, u8 button, double scroll,
                                     b8 pressed);
/*! RGFW_jsButtonPressed / RGFW_jsButtonReleased, the window that got the event,
 * the button that was pressed, the scroll value, if it was a press (else it's a
 * release) */
typedef void (*RGFW_jsButtonfunc)(RGFW_window *win, u16 joystick, u8 button,
                                  b8 pressed);
/*! RGFW_jsAxisMove, the window that got the event, the joystick in question,
 * the axis values and the amount of axises */
typedef void (*RGFW_jsAxisfunc)(RGFW_window *win, u16 joystick,
...
#ifdef RGFW_ALLOC_DROPFILES
typedef void (*RGFW_dndfunc)(RGFW_window *win, char **droppedFiles,
...
/*! set callback for a joystick axis mov event returns previous callback
 * function (if it was set)  */
...
/*!< register joystick to window based on a number (the number is based on when
 * it was connected eg. /dev/js0)*/
...
typedef RGFW_ENUM(u8, RGFW_mouseIcons){
    RGFW_MOUSE_NORMAL = 0, RGFW_MOUSE_ARROW,         RGFW_MOUSE_IBEAM,
    RGFW_MOUSE_CROSSHAIR,  RGFW_MOUSE_POINTING_HAND, RGFW_MOUSE_RESIZE_EW,
    RGFW_MOUSE_RESIZE_NS,  RGFW_MOUSE_RESIZE_NWSE,   RGFW_MOUSE_RESIZE_NESW,
    RGFW_MOUSE_RESIZE_ALL, RGFW_MOUSE_NOT_ALLOWED,
};
...
macos : gcc main.c -framework Foundation -framework AppKit -framework OpenGL
-framework CoreVideo
...
int main() {
        RGFW_window* win = RGFW_createWindow("name", RGFW_RECT(500, 500, 500,
500), (u64)0);
...
a while loop may cause input lag if (win->event.type == RGFW_quit)) break;
...
system uses) static : ar rcs RGFW.a RGFW.o shared : windows: gcc -shared RGFW.o
-lwinmm -lopengl32 -lshell32 -lgdi32 -o RGFW.dll linux: gcc -shared RGFW.o -lX11
-lXcursor -lGL -lXrandr -o RGFW.so macos: gcc -shared RGFW.o -framework
Foundation -framework AppKit -framework OpenGL -framework CoreVideo
*/
...
void RGFW_init_keys(void) {
#endif
  RGFW_MAP[RGFW_OS_BASED_VALUE(49, 192, 50, DOM_VK_BACK_QUOTE,
                               KEY_GRAVE)] = RGFW_Backtick RGFW_NEXT

      RGFW_MAP[RGFW_OS_BASED_VALUE(19, 0x30, 29, DOM_VK_0, KEY_0)] = RGFW_0
          RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(10, 0x31, 18, DOM_VK_1,
                                                 KEY_1)] = RGFW_1 RGFW_NEXT
              RGFW_MAP[RGFW_OS_BASED_VALUE(11, 0x32, 19, DOM_VK_2, KEY_2)] =
                  RGFW_2 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                      12, 0x33, 20, DOM_VK_3, KEY_3)] = RGFW_3 RGFW_NEXT
                      RGFW_MAP[RGFW_OS_BASED_VALUE(13, 0x34, 21, DOM_VK_4,
                                                   KEY_4)] = RGFW_4 RGFW_NEXT
                          RGFW_MAP[RGFW_OS_BASED_VALUE(
                              14, 0x35, 23, DOM_VK_5, KEY_5)] = RGFW_5 RGFW_NEXT
                              RGFW_MAP[RGFW_OS_BASED_VALUE(15, 0x36, 22,
                                                           DOM_VK_6, KEY_6)] =
                                  RGFW_6 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                      16, 0x37, 26, DOM_VK_7, KEY_7)] = RGFW_7
                                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                          17, 0x38, 28, DOM_VK_8,
                                          KEY_8)] = RGFW_8 RGFW_NEXT
                                          RGFW_MAP[RGFW_OS_BASED_VALUE(
                                              18, 0x39, 25, DOM_VK_9, KEY_9)] =
                                              RGFW_9,

  RGFW_MAP[RGFW_OS_BASED_VALUE(65, 0x20, 49, DOM_VK_SPACE, KEY_SPACE)] =
      RGFW_Space,

  RGFW_MAP[RGFW_OS_BASED_VALUE(
      38, 0x41, 0, DOM_VK_A,
      KEY_A)] = RGFW_a RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(56, 0x42,
                                                              11, DOM_VK_B, KEY_B)] = RGFW_b RGFW_NEXT RGFW_MAP
      [RGFW_OS_BASED_VALUE(54, 0x43, 8, DOM_VK_C, KEY_C)] = RGFW_c RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
          40,
          0x44, 2, DOM_VK_D, KEY_D)] = RGFW_d RGFW_NEXT RGFW_MAP
          [RGFW_OS_BASED_VALUE(26, 0x45, 14, DOM_VK_E, KEY_E)] = RGFW_e RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
              41,
              0x46, 3, DOM_VK_F, KEY_F)] = RGFW_f RGFW_NEXT RGFW_MAP
              [RGFW_OS_BASED_VALUE(42, 0x47, 5, DOM_VK_G, KEY_G)] = RGFW_g RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                  43,
                  0x48, 4, DOM_VK_H, KEY_H)] = RGFW_h RGFW_NEXT RGFW_MAP
                  [RGFW_OS_BASED_VALUE(31, 0x49, 34, DOM_VK_I, KEY_I)] = RGFW_i RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                      44,
                      0x4A, 38, DOM_VK_J,
                      KEY_J)] = RGFW_j RGFW_NEXT RGFW_MAP
                      [RGFW_OS_BASED_VALUE(45, 0x4B, 40, DOM_VK_K, KEY_K)] = RGFW_k RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                          46,
                          0x4C, 37, DOM_VK_L,
                          KEY_L)] = RGFW_l RGFW_NEXT RGFW_MAP
                          [RGFW_OS_BASED_VALUE(58, 0x4D, 46, DOM_VK_M, KEY_M)] = RGFW_m RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                              57,
                              0x4E, 45, DOM_VK_N,
                              KEY_N)] =
                              RGFW_n RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                  32,
                                  0x4F, 31, DOM_VK_O,
                                  KEY_O)] = RGFW_o RGFW_NEXT RGFW_MAP
                                  [RGFW_OS_BASED_VALUE(33, 0x50, 35, DOM_VK_P,
                                                       KEY_P)] = RGFW_p
                                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                          24,
                                          0x51, 12, DOM_VK_Q,
                                          KEY_Q)] = RGFW_q RGFW_NEXT RGFW_MAP
                                          [RGFW_OS_BASED_VALUE(
                                              27,
                                              0x52, 15, DOM_VK_R,
                                              KEY_R)] = RGFW_r RGFW_NEXT RGFW_MAP
                                              [RGFW_OS_BASED_VALUE(
                                                  39,
                                                  0x53, 1, DOM_VK_S,
                                                  KEY_S)] = RGFW_s RGFW_NEXT RGFW_MAP
                                                  [RGFW_OS_BASED_VALUE(
                                                      28,
                                                      0x54, 17, DOM_VK_T,
                                                      KEY_T)] = RGFW_t RGFW_NEXT RGFW_MAP
                                                      [RGFW_OS_BASED_VALUE(
                                                          30,
                                                          0x55, 32, DOM_VK_U,
                                                          KEY_U)] = RGFW_u RGFW_NEXT RGFW_MAP
                                                          [RGFW_OS_BASED_VALUE(
                                                              55,
                                                              0x56, 9, DOM_VK_V,
                                                              KEY_V)] = RGFW_v RGFW_NEXT RGFW_MAP
                                                              [RGFW_OS_BASED_VALUE(
                                                                  25,
                                                                  0x57, 13,
                                                                  DOM_VK_W,
                                                                  KEY_W)] = RGFW_w RGFW_NEXT RGFW_MAP
                                                                  [RGFW_OS_BASED_VALUE(
                                                                      53,
                                                                      0x58, 7,
                                                                      DOM_VK_X,
                                                                      KEY_X)] = RGFW_x RGFW_NEXT RGFW_MAP
                                                                      [RGFW_OS_BASED_VALUE(
                                                                          29,
                                                                          0x59,
                                                                          16,
                                                                          DOM_VK_Y,
                                                                          KEY_Y)] = RGFW_y RGFW_NEXT RGFW_MAP
                                                                          [RGFW_OS_BASED_VALUE(
                                                                              52,
                                                                              0x5A,
                                                                              6,
  ...
  RGFW_MAP[RGFW_OS_BASED_VALUE(60, 190, 47, DOM_VK_PERIOD, KEY_DOT)] =
      RGFW_Period RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
          59, 188, 43, DOM_VK_COMMA, KEY_COMMA)] = RGFW_Comma RGFW_NEXT
          RGFW_MAP[RGFW_OS_BASED_VALUE(61, 191, 44, DOM_VK_SLASH, KEY_SLASH)] =
              RGFW_Slash RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                  34, 219, 33, DOM_VK_OPEN_BRACKET, KEY_LEFTBRACE)] =
                  RGFW_Bracket RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                      35, 221, 30, DOM_VK_CLOSE_BRACKET, KEY_RIGHTBRACE)] =
                      RGFW_CloseBracket RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                          47, 186, 41, DOM_VK_SEMICOLON, KEY_SEMICOLON)] =
                          RGFW_Semicolon RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                              48, 222, 39, DOM_VK_QUOTE, KEY_APOSTROPHE)] =
                              RGFW_Quote RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                  51, 322, 42, DOM_VK_BACK_SLASH,
                                  KEY_BACKSLASH)] = RGFW_BackSlash,

  RGFW_MAP[RGFW_OS_BASED_VALUE(
      36, 0x0D, 36, DOM_VK_RETURN,
      KEY_ENTER)] = RGFW_Return RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(119,
                                                                       0x2E,
                                                                       118,
                                                                       DOM_VK_DELETE, KEY_DELETE)] = RGFW_Delete
      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
          77,
          0x90, 72, DOM_VK_NUM_LOCK, KEY_NUMLOCK)] =
          RGFW_Numlock RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
              106, 0x6F, 82, DOM_VK_DIVIDE, KEY_KPSLASH)] = RGFW_KP_Slash
              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                  63,
                  0x6A, 76, DOM_VK_MULTIPLY, KEY_KPASTERISK)] = RGFW_Multiply
                  RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                      82,
                      0x6D, 67, DOM_VK_SUBTRACT, KEY_KPMINUS)] = RGFW_KP_Minus
                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                          87,
                          0x61, 84, DOM_VK_NUMPAD1, KEY_KP1)] = RGFW_KP_1
                          RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                              88,
                              0x62, 85, DOM_VK_NUMPAD2,
                              KEY_KP2)] = RGFW_KP_2 RGFW_NEXT RGFW_MAP
                              [RGFW_OS_BASED_VALUE(89, 0x63, 86, DOM_VK_NUMPAD3,
                                                   KEY_KP3)] = RGFW_KP_3
                                  RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                      83,
                                      0x64, 87, DOM_VK_NUMPAD4,
                                      KEY_KP4)] = RGFW_KP_4 RGFW_NEXT RGFW_MAP
                                      [RGFW_OS_BASED_VALUE(84, 0x65, 88,
                                                           DOM_VK_NUMPAD5,
                                                           KEY_KP5)] = RGFW_KP_5
                                          RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                              85,
                                              0x66, 89, DOM_VK_NUMPAD6,
                                              KEY_KP6)] = RGFW_KP_6 RGFW_NEXT RGFW_MAP
                                              [RGFW_OS_BASED_VALUE(
                                                  79,
                                                  0x67, 90, DOM_VK_NUMPAD7,
                                                  KEY_KP7)] = RGFW_KP_7 RGFW_NEXT RGFW_MAP
                                                  [RGFW_OS_BASED_VALUE(
                                                      80,
                                                      0x68, 92, DOM_VK_NUMPAD8,
                                                      KEY_KP8)] = RGFW_KP_8 RGFW_NEXT RGFW_MAP
                                                      [RGFW_OS_BASED_VALUE(
                                                          81,
                                                          0x69, 93,
                                                          DOM_VK_NUMPAD9,
                                                          KEY_KP9)] = RGFW_KP_9 RGFW_NEXT RGFW_MAP
                                                          [RGFW_OS_BASED_VALUE(
                                                              90,
                                                              0x60, 83,
                                                              DOM_VK_NUMPAD0,
                                                              KEY_KP0)] = RGFW_KP_0 RGFW_NEXT RGFW_MAP
                                                              [RGFW_OS_BASED_VALUE(
                                                                  91,
                                                                  0x6E, 65,
                                                                  DOM_VK_DECIMAL,
                                                                  KEY_KPDOT)] = RGFW_KP_Period
                                                                  RGFW_NEXT RGFW_MAP
                                                                      [RGFW_OS_BASED_VALUE(
                                                                          104,
                                                                          0x92,
  ...
  RGFW_MAP[RGFW_OS_BASED_VALUE(20, 189, 27, DOM_VK_HYPHEN_MINUS, KEY_MINUS)] =
      RGFW_Minus RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
          21, 187, 24, DOM_VK_EQUALS, KEY_EQUAL)] = RGFW_Equals RGFW_NEXT
          RGFW_MAP[RGFW_OS_BASED_VALUE(22, 8, 51, DOM_VK_BACK_SPACE,
                                       KEY_BACKSPACE)] =
              RGFW_BackSpace RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                  23, 0x09, 48, DOM_VK_TAB, KEY_TAB)] = RGFW_Tab RGFW_NEXT
                  RGFW_MAP[RGFW_OS_BASED_VALUE(66, 20, 57, DOM_VK_CAPS_LOCK,
                                               KEY_CAPSLOCK)] =
                      RGFW_CapsLock RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                          50, 0x10, 56, DOM_VK_SHIFT, KEY_LEFTSHIFT)] =
                          RGFW_ShiftL RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                              37, 0x11, 59, DOM_VK_CONTROL, KEY_LEFTCTRL)] =
                              RGFW_ControlL RGFW_NEXT
                                  RGFW_MAP[RGFW_OS_BASED_VALUE(
                                      64, 0x12, 58, DOM_VK_ALT, KEY_LEFTALT)] =
                                      RGFW_AltL RGFW_NEXT
                                          RGFW_MAP[RGFW_OS_BASED_VALUE(
                                              133, 0x5B, 55, DOM_VK_WIN,
                                              KEY_LEFTMETA)] = RGFW_SuperL,

#if !defined(RGFW_WINDOWS) && !defined(RGFW_MACOS) && !defined(RGFW_WEBASM)
  RGFW_MAP[RGFW_OS_BASED_VALUE(105, 0x11, 59, 0, KEY_RIGHTCTRL)] = RGFW_ControlR
      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(135, 0xA4, 55, 0, KEY_RIGHTMETA)] =
          RGFW_SuperR,
  RGFW_MAP[RGFW_OS_BASED_VALUE(62, 0x5C, 56, 0, KEY_RIGHTSHIFT)] = RGFW_ShiftR
      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(108, 165, 58, 0, KEY_RIGHTALT)] =
          RGFW_AltR,
#endif

  RGFW_MAP[RGFW_OS_BASED_VALUE(67, 0x70, 127, DOM_VK_F1, KEY_F1)] = RGFW_F1 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
      68,
      0x71, 121, DOM_VK_F2,
      KEY_F2)] = RGFW_F2 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(69, 0x72,
                                                                100, DOM_VK_F3, KEY_F3)] = RGFW_F3
      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
          70,
          0x73, 119, DOM_VK_F4,
          KEY_F4)] = RGFW_F4 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(71, 0x74,
                                                                    97,
                                                                    DOM_VK_F5, KEY_F5)] = RGFW_F5
          RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
              72,
              0x75, 98, DOM_VK_F6,
              KEY_F6)] = RGFW_F6 RGFW_NEXT RGFW_MAP
              [RGFW_OS_BASED_VALUE(73, 0x76, 99, DOM_VK_F7, KEY_F7)] = RGFW_F7 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                  74,
                  0x77, 101, DOM_VK_F8, KEY_F8)] = RGFW_F8 RGFW_NEXT RGFW_MAP
                  [RGFW_OS_BASED_VALUE(75, 0x78, 102, DOM_VK_F9,
                                       KEY_F9)] = RGFW_F9 RGFW_NEXT RGFW_MAP
                      [RGFW_OS_BASED_VALUE(76, 0x79, 110, DOM_VK_F10,
                                           KEY_F10)] =
                          RGFW_F10 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                              95,
                              0x7A, 104, DOM_VK_F11, KEY_F11)] =
                              RGFW_F11 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                  96,
                                  0x7B, 112, DOM_VK_F12, KEY_F12)] = RGFW_F12
                                  RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                      111, 0x26, 126,
                                      DOM_VK_UP, KEY_UP)] = RGFW_Up
                                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                          116, 0x28, 125,
                                          DOM_VK_DOWN, KEY_DOWN)] = RGFW_Down
                                          RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                              113, 0x25, 123,
                                              DOM_VK_LEFT,
                                              KEY_LEFT)] = RGFW_Left RGFW_NEXT RGFW_MAP
                                              [RGFW_OS_BASED_VALUE(
                                                  114, 0x27, 124,
                                                  DOM_VK_RIGHT,
                                                  ...
                                                  RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                      ...
                                                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                          115, 0x23, 120,
                                                          DOM_VK_END,
                                                          KEY_END)] = RGFW_End RGFW_NEXT RGFW_MAP
                                                          [RGFW_OS_BASED_VALUE(
                                                              112, 336, 117,
                                                              DOM_VK_PAGE_UP,
                                                              KEY_PAGEUP)] = RGFW_PageUp
                                                              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                                  117, 325, 122,
                                                                  DOM_VK_PAGE_DOWN,
...
#define RGFW_xor(x, y) (((x) && (!(y))) || ((y) && (!(x))))
  b8 caps4caps = (lockState & RGFW_CAPSLOCK) &&
                 ((keycode >= RGFW_a) && (keycode <= RGFW_z));
  b8 shouldShift = RGFW_xor(
      (RGFW_isPressed(NULL, RGFW_ShiftL) || RGFW_isPressed(NULL, RGFW_ShiftR)),
      caps4caps);
#undef RGFW_xor
...
char RGFW_keyCodeToCharAuto(u32 keycode, u8 lockState) {
  return RGFW_keyCodeToChar(keycode, RGFW_shouldShift(keycode, lockState));
}
...
/* joystick data */
u8 RGFW_jsPressed[4][16]; /*!< if a key is currently pressed or not (per
                             joystick) */
...
void RGFW_focusfuncEMPTY(RGFW_window *win, b8 inFocus) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(inFocus);
}
void RGFW_mouseNotifyfuncEMPTY(RGFW_window *win, RGFW_point point, b8 status) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(point);
  RGFW_UNUSED(status);
}
...
void RGFW_keyfuncEMPTY(RGFW_window *win, u32 keycode, char keyName[16],
                       u8 lockState, b8 pressed) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(keycode);
  RGFW_UNUSED(keyName);
  RGFW_UNUSED(lockState);
  RGFW_UNUSED(pressed);
}
void RGFW_mousebuttonfuncEMPTY(RGFW_window *win, u8 button, double scroll,
                               b8 pressed) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(button);
  RGFW_UNUSED(scroll);
...
void RGFW_jsButtonfuncEMPTY(RGFW_window *win, u16 joystick, u8 button,
                            b8 pressed) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(joystick);
  RGFW_UNUSED(button);
  RGFW_UNUSED(pressed);
}
void RGFW_jsAxisfuncEMPTY(RGFW_window *win, u16 joystick, RGFW_point axis[2],
                          u8 axisesCount) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(joystick);
  RGFW_UNUSED(axis);
...
#ifdef RGFW_ALLOC_DROPFILES
void RGFW_dndfuncEMPTY(RGFW_window *win, char **droppedFiles,
                       u32 droppedFilesCount) {
  RGFW_UNUSED(win);
...
RGFW_jsButtonfunc RGFW_setjsButtonCallback(RGFW_jsButtonfunc func) {
  RGFW_jsButtonfunc prev = (RGFW_jsButtonCallback == RGFW_jsButtonfuncEMPTY)
                               ? NULL
                               : RGFW_jsButtonCallback;
  RGFW_jsButtonCallback = func;
  return prev;
}
RGFW_jsAxisfunc RGFW_setjsAxisCallback(RGFW_jsAxisfunc func) {
  RGFW_jsAxisfunc prev = (RGFW_jsAxisCallback == RGFW_jsAxisfuncEMPTY)
                             ? NULL
                             : RGFW_jsAxisCallback;
...
void RGFW_setBufferSize(RGFW_area size) { RGFW_bufferSize = size; }
...
RGFW_window *RGFW_window_basic_init(RGFW_rect rect, u16 args) {
  RGFW_window *win = (RGFW_window *)RGFW_MALLOC(
      sizeof(RGFW_window)); /*!< make a new RGFW struct */

  /* clear out dnd info */
#ifdef RGFW_ALLOC_DROPFILES
  win->event.droppedFiles =
      (char **)RGFW_MALLOC(sizeof(char *) * RGFW_MAX_DROPS);
  u32 i;
  for (i = 0; i < RGFW_MAX_DROPS; i++)
    win->event.droppedFiles[i] =
        (char *)RGFW_CALLOC(RGFW_MAX_PATH, sizeof(char));
#endif
...
void RGFW_setClassName(char *name) { RGFW_className = name; }
...
b8 RGFW_isMousePressed(RGFW_window *win, u8 button) {
  assert(win != NULL);
  return RGFW_mouseButtons[button].current && (win != NULL) &&
         win->event.inFocus;
}
...
b8 RGFW_isReleased(RGFW_window *win, u8 key) {
  return (!RGFW_isPressed(win, key) && RGFW_wasPressed(win, key));
    ...
    defined(RGFW_DIRECTX) /* defines for directX context*/
...
void RGFW_window_makeCurrent(RGFW_window *win) {
  ...
  if (win == NULL)
    RGFW_dxInfo.pDeviceContext->lpVtbl->OMSetRenderTargets(
        RGFW_dxInfo.pDeviceContext, 1, NULL, NULL);
  else
    RGFW_dxInfo.pDeviceContext->lpVtbl->OMSetRenderTargets(
        RGFW_dxInfo.pDeviceContext, 1, &win->src.renderTargetView, NULL);
...
void RGFW_setDoubleBuffer(b8 useDoubleBuffer) {
...
#define RGFW_GL_RENDER_TYPE                                                    \
  RGFW_OS_BASED_VALUE(GLX_X_VISUAL_TYPE, 0x2003, 73, 0, 0)
...
#define RGFW_GL_DOUBLEBUFFER                                                   \
  RGFW_OS_BASED_VALUE(GLX_DOUBLEBUFFER, 0x2011, 5, 0, 0)
#define RGFW_GL_STENCIL_SIZE                                                   \
  RGFW_OS_BASED_VALUE(GLX_STENCIL_SIZE, 0x2023, 13, 0, 0)
...
#define RGFW_GL_AUX_BUFFERS                                                    \
  RGFW_OS_BASED_VALUE(GLX_AUX_BUFFERS, 0x2024, 7, 0, 0)
...
void RGFW_createOpenGLContext(RGFW_window *win) {
  ...
  eglChooseConfigSource =
      (PFNEGLCHOOSECONFIGPROC)eglGetProcAddress("eglChooseConfig");
  ...
  for (i = 0; i < RGFW_joystickCount; i++) {
    ...
    while ((bytes = read(RGFW_joysticks[i], &e, sizeof(e))) > 0) {
      ...
      case JS_EVENT_BUTTON:
        win->event.type =
            e.value ? RGFW_jsButtonPressed : RGFW_jsButtonReleased;
...
void RGFW_init_buffer(RGFW_window *win, XVisualInfo *vi) {
  ...
  win->src.bitmap = XCreateImage(
      win->src.display, XDefaultVisual(win->src.display, vi->screen), vi->depth,
      ZPixmap, 0, NULL, RGFW_bufferSize.w, RGFW_bufferSize.h, 32, 0);
...
RGFW_window *RGFW_createWindow(const char *name, RGFW_rect rect, u16 args) {
  ...
  if (X11Cursorhandle == NULL) {
    ...
    XcursorImageCreateSrc =
        (PFN_XcursorImageCreate)dlsym(X11Cursorhandle, "XcursorImageCreate");
    XcursorImageDestroySrc =
        (PFN_XcursorImageDestroy)dlsym(X11Cursorhandle, "XcursorImageDestroy");
    XcursorImageLoadCursorSrc = (PFN_XcursorImageLoadCursor)dlsym(
        X11Cursorhandle, "XcursorImageLoadCursor");
  }
#endif
  ...
  u64 event_mask = KeyPressMask | KeyReleaseMask | ButtonPressMask |
                   ...
                   ExposureMask; /*!< X11 events accepted*/

#ifdef RGFW_OPENGL
  u32 *visual_attribs = RGFW_initFormatAttribs(args & RGFW_OPENGL_SOFTWARE);
  i32 fbcount;
  GLXFBConfig *fbc = glXChooseFBConfig((Display *)win->src.display,
  ...
  for (i = 0; i < (u32)fbcount; i++) {
    XVisualInfo *vi =
        glXGetVisualFromFBConfig((Display *)win->src.display, fbc[i]);
    if (vi == NULL)
      continue;
    ...
    glXGetFBConfigAttrib((Display *)win->src.display, fbc[i],
                         GLX_SAMPLE_BUFFERS, &samp_buf);
    glXGetFBConfigAttrib((Display *)win->src.display, fbc[i], GLX_SAMPLES,
                         &samples);
  ...
  /* Get a visual */
  XVisualInfo *vi =
      glXGetVisualFromFBConfig((Display *)win->src.display, bestFbc);

  XFree(fbc);
#else
  XVisualInfo viNorm;

  viNorm.visual = DefaultVisual((Display *)win->src.display,
                                DefaultScreen((Display *)win->src.display));

  viNorm.depth = 0;
  XVisualInfo *vi = &viNorm;

  XMatchVisualInfo((Display *)win->src.display,
                   DefaultScreen((Display *)win->src.display), 32, TrueColor,
                   vi); /*!< for RGBA backgrounds*/
#endif
  ...
  win->src.window = XCreateWindow(
      ...
      DefaultRootWindow((Display *)win->src.display), win->r.x, win->r.y,
      win->r.w, win->r.h, 0, vi->depth, InputOutput, vi->visual,
  ...
  if ((args & RGFW_NO_INIT_API) == 0) {
#ifdef RGFW_OPENGL /* This is the second part of setting up opengl. This is    \
                      where we ask OpenGL for a specific version. */
    ...
    win->src.ctx = glXCreateContextAttribsARB(
        (Display *)win->src.display, bestFbc, ctx, True, context_attribs);
#endif
    if (RGFW_root == NULL)
      RGFW_root = win;
  ...
  if (args & RGFW_CENTER) {
    RGFW_area screenR = RGFW_getScreenSize();
    RGFW_window_move(win, RGFW_POINT((screenR.w - win->r.w) / 2,
                                     (screenR.h - win->r.h) / 2));
  }
  ...
  if (args & RGFW_NO_RESIZE) { /* make it so the user can't resize the window*/
    ...
    XSetWMSizeHints((Display *)win->src.display, (Drawable)win->src.window, sh,
                    XA_WM_NORMAL_HINTS);
    XFree(sh);
  }
  ...
  XSelectInput((Display *)win->src.display, (Drawable)win->src.window,
  ...
  if (wm_delete_window == 0)
    wm_delete_window =
        XInternAtom((Display *)win->src.display, "WM_DELETE_WINDOW", False);

  XSetWMProtocols((Display *)win->src.display, (Drawable)win->src.window,
                  &wm_delete_window, 1);

  /* connect the context to the window*/
#ifdef RGFW_OPENGL
  if ((args & RGFW_NO_INIT_API) == 0)
    glXMakeCurrent((Display *)win->src.display, (Drawable)win->src.window,
                   (GLXContext)win->src.ctx);
#endif

  /* set the background*/
  XStoreName((Display *)win->src.display, (Drawable)win->src.window,
             name); /*!< set the name*/

  XMapWindow((Display *)win->src.display,
             (Drawable)win->src.window); /* draw the window*/
  XMoveWindow((Display *)win->src.display, (Drawable)win->src.window, win->r.x,
              win->r.y); /*!< move the window to it's proper cords*/

  if (args & RGFW_ALLOW_DND) { /* init drag and drop atoms and turn on drag and
                                  drop for this window */
    win->_winArgs |= RGFW_ALLOW_DND;

    XdndTypeList =
        XInternAtom((Display *)win->src.display, "XdndTypeList", False);
    XdndSelection =
        XInternAtom((Display *)win->src.display, "XdndSelection", False);

    /* client messages */
    XdndEnter = XInternAtom((Display *)win->src.display, "XdndEnter", False);
    XdndPosition =
        XInternAtom((Display *)win->src.display, "XdndPosition", False);
    ...
    XdndFinished =
        XInternAtom((Display *)win->src.display, "XdndFinished", False);

    /* actions */
    XdndActionCopy =
        XInternAtom((Display *)win->src.display, "XdndActionCopy", False);

    XtextUriList =
        XInternAtom((Display *)win->src.display, "text/uri-list", False);
    ...
    XChangeProperty((Display *)win->src.display, (Window)win->src.window,
                    XdndAware, 4, 32, PropModeReplace, &version,
...
RGFW_point RGFW_getGlobalMousePoint(void) {
  ...
  XQueryPointer((Display *)RGFW_root->src.display,
                XDefaultRootWindow((Display *)RGFW_root->src.display), &window1,
...
RGFW_point RGFW_window_getMousePoint(RGFW_window *win) {
  ...
  u32 z;
  Window window1, window2;
  XQueryPointer((Display *)win->src.display, win->src.window, &window1,
                &window2, &x, &y, &RGFWMouse.x, &RGFWMouse.y, &z);
...
RGFW_Event *RGFW_window_checkEvent(RGFW_window *win) {
  ...
  if ((QLength(win->src.display) ||
       XEventsQueued((Display *)win->src.display, QueuedAlready) +
           XEventsQueued((Display *)win->src.display, QueuedAfterReading)) &&
      win->event.type != RGFW_quit)
  ...
  case KeyRelease: {
    ...
    if (E.type == KeyRelease &&
      ...
      if (E.xkey.time == NE.xkey.time &&
          E.xkey.keycode ==
              NE.xkey
                  .keycode) /* check if the current and next are both the same*/
        win->event.repeat = RGFW_TRUE;
    }

    /* set event key data */
    KeySym sym =
        (KeySym)XkbKeycodeToKeysym((Display *)win->src.display, E.xkey.keycode,
                                   0, E.xkey.state & ShiftMask ? 1 : 0);
    ...
    RGFW_keyboard[win->event.keyCode].prev =
        RGFW_isPressed(win, win->event.keyCode);
    ...
    RGFW_keyCallback(win, win->event.keyCode, win->event.keyName,
                     win->event.lockState, (E.type == KeyPress));
    break;
  }
  case ButtonPress:
  case ButtonRelease:
    win->event.type =
        RGFW_mouseButtonPressed + (E.type == ButtonRelease); // the events match
  ...
  case ClientMessage:
    ...
    if (E.xclient.message_type == XdndEnter) {
      ...
      if (list) {
        ...
        XGetWindowProperty((Display *)win->src.display, xdnd.source,
                           XdndTypeList, 0, LONG_MAX, False, 4, &actualType,
                           &actualFormat, &count, &bytesAfter, (u8 **)&formats);
      } else {
        count = 0;
    ...
    if (E.xclient.message_type == XdndPosition) {
      ...
      XTranslateCoordinates((Display *)win->src.display,
                            ...
                            (Window)win->src.window, xabs, yabs, &xpos, &ypos,
                            &dummy);
      ...
      XSendEvent((Display *)win->src.display, xdnd.source, False, NoEventMask,
                 &reply);
    ...
    if (xdnd.format) {
      ...
      XConvertSelection((Display *)win->src.display, XdndSelection, xdnd.format,
                        XdndSelection, (Window)win->src.window, time);
  ...
  case SelectionNotify: {
    ...
    XGetWindowProperty((Display *)win->src.display, E.xselection.requestor,
    ...
    */
    ...
    while ((line = strtok(data, "\r\n"))) {
      ...
      path[index] = '\0';
      strncpy(win->event.droppedFiles[win->event.droppedFilesCount - 1], path,
              index + 1);
    }
...
void RGFW_window_minimize(RGFW_window *win) {
  ...
  XIconifyWindow(win->src.display, (Window)win->src.window,
                 DefaultScreen(win->src.display));
...
void RGFW_window_setMousePassthrough(RGFW_window *win, b8 passthrough) {
  ...
  if (XShapeCombineMask != NULL)
    XShapeCombineMask =
        (PFN_XShapeCombineMask)dlsym(RGFW_libxshape, "XShapeCombineMask");

  if (XShapeCombineRegion != NULL)
    XShapeCombineRegion =
        (PFN_XShapeCombineRegion)dlsym(RGFW_libxshape, "XShapeCombineMask");

  if (passthrough) {
    Region region = XCreateRegion();
    XShapeCombineRegion(win->src.display, win->src.window, ShapeInput, 0, 0,
                        region, ShapeSet);
  ...
  XShapeCombineMask(win->src.display, win->src.window, ShapeInput, 0, 0, None,
                    ShapeSet);
}
#endif
...
void RGFW_window_setIcon(RGFW_window *win, u8 *icon, RGFW_area a,
                         i32 channels) {
  assert(win != NULL);
  ...
  *target++ = a.h;
  ...
  for (i = 0; i < a.w * a.h; i++) {
    if (channels == 3)
      *target++ = ((icon[i * 3 + 0]) << 16) | ((icon[i * 3 + 1]) << 8) |
                  ((icon[i * 3 + 2]) << 0) | (0xFF << 24);

    else if (channels == 4)
      *target++ = ((icon[i * 4 + 0]) << 16) | ((icon[i * 4 + 1]) << 8) |
                  ((icon[i * 4 + 2]) << 0) | ((icon[i * 4 + 3]) << 24);
  }

  static Atom NET_WM_ICON = 0;
  if (NET_WM_ICON == 0)
    NET_WM_ICON =
        XInternAtom((Display *)win->src.display, "_NET_WM_ICON", False);

  XChangeProperty((Display *)win->src.display, (Window)win->src.window,
                  NET_WM_ICON, 6, 32, PropModeReplace, (u8 *)X11Icon,
                  longCount);
...
void RGFW_window_moveMouse(RGFW_window *win, RGFW_point v) {
  ...
  XWarpPointer(win->src.display, None, win->src.window, 0, 0, 0, 0,
               (int)v.x - win->r.x, (int)v.y - win->r.y);
}

RGFWDEF void RGFW_window_disableMouse(RGFW_window *win) { RGFW_UNUSED(win); }
...
char *RGFW_readClipboard(size_t *size) {
  ...
  XConvertSelection(RGFW_root->src.display, CLIPBOARD, UTF8, XSEL_DATA,
                    RGFW_root->src.window, CurrentTime);
  XSync(RGFW_root->src.display, 0);
  XNextEvent(RGFW_root->src.display, &event);

  if (event.type != SelectionNotify ||
      event.xselection.selection != CLIPBOARD || event.xselection.property == 0)
    return NULL;

  XGetWindowProperty(event.xselection.display, event.xselection.requestor,
                     event.xselection.property, 0L, (~0L), 0, AnyPropertyType,
                     &target, &format, &sizeN, &N, (unsigned char **)&data);
...
void RGFW_writeClipboard(const char *text, u32 textLen) {
  static Atom CLIPBOARD = 0, UTF8_STRING = 0, SAVE_TARGETS = 0, TARGETS = 0,
              MULTIPLE = 0, ATOM_PAIR = 0, CLIPBOARD_MANAGER = 0;

  if (CLIPBOARD == 0) {
    CLIPBOARD =
        XInternAtom((Display *)RGFW_root->src.display, "CLIPBOARD", False);
    UTF8_STRING =
        XInternAtom((Display *)RGFW_root->src.display, "UTF8_STRING", False);
    SAVE_TARGETS =
        XInternAtom((Display *)RGFW_root->src.display, "SAVE_TARGETS", False);
    TARGETS = XInternAtom((Display *)RGFW_root->src.display, "TARGETS", False);
    MULTIPLE =
        XInternAtom((Display *)RGFW_root->src.display, "MULTIPLE", False);
    ATOM_PAIR =
        XInternAtom((Display *)RGFW_root->src.display, "ATOM_PAIR", False);
    CLIPBOARD_MANAGER = XInternAtom((Display *)RGFW_root->src.display,
                                    "CLIPBOARD_MANAGER", False);
  }

  XSetSelectionOwner((Display *)RGFW_root->src.display, CLIPBOARD,
                     (Window)RGFW_root->src.window, CurrentTime);

  XConvertSelection((Display *)RGFW_root->src.display, CLIPBOARD_MANAGER,
                    SAVE_TARGETS, None, (Window)RGFW_root->src.window,
                    CurrentTime);
  for (;;) {
    XEvent event;
    ...
    if (request->target == TARGETS) {
      ...
      XChangeProperty((Display *)RGFW_root->src.display, request->requestor,
                      request->property, 4, 32, PropModeReplace, (u8 *)targets,
    ...
    if (request->target == MULTIPLE) {
      ...
      XGetWindowProperty((Display *)RGFW_root->src.display, request->requestor,
    ...
    XSendEvent((Display *)RGFW_root->src.display, request->requestor, False, 0,
               &reply);
    XFlush(RGFW_root->src.display);
  }
}
...
u8 RGFW_window_isFullscreen(RGFW_window *win) {
  ...
  XGetWindowAttributes(win->src.display, (Window)win->src.window,
                       &windowAttributes);
  ...
  return (
      windowAttributes.x == 0 && windowAttributes.y == 0 &&
      windowAttributes.width ==
          XDisplayWidth(win->src.display, DefaultScreen(win->src.display)) &&
      windowAttributes.height ==
          XDisplayHeight(win->src.display, DefaultScreen(win->src.display)));
}
...
u8 RGFW_window_isMinimized(RGFW_window *win) {
  ...
  i16 status =
      XGetWindowProperty(win->src.display, (Window)win->src.window, prop, 0, 2,
...
u8 RGFW_window_isMaximized(RGFW_window *win) {
  ...
  if (net_wm_state == 0) {
    ...
    net_wm_state_maximized_vert =
        XInternAtom(win->src.display, "_NET_WM_STATE_MAXIMIZED_VERT", False);
    net_wm_state_maximized_horz =
        XInternAtom(win->src.display, "_NET_WM_STATE_MAXIMIZED_HORZ", False);
  }
  ...
  i16 status = XGetWindowProperty(
      win->src.display, (Window)win->src.window, net_wm_state, 0, 1024, False,
      XA_ATOM, &actual_type, &actual_format, &nitems, &bytes_after, &prop_data);
...
static void XGetSystemContentScale(Display *display, float *xscale,
                                   float *yscale) {
...
RGFW_monitor RGFW_XCreateMonitor(i32 screen) {
  ...
  if (ci == NULL) {
    float dpi_width =
        round((double)monitor.rect.w / (((double)monitor.physW) / 25.4));
    float dpi_height =
        round((double)monitor.rect.h / (((double)monitor.physH) / 25.4));
...
RGFW_monitor RGFW_getPrimaryMonitor(void) {
  ...
  for (int i = 0; i < res->noutput; i++) {
    XRROutputInfo *output_info =
        XRRGetOutputInfo(RGFW_root->src.display, res, res->outputs[i]);
    if (output_info->connection == RR_Connected && output_info->crtc) {
      XRRCrtcInfo *crtc_info =
          XRRGetCrtcInfo(RGFW_root->src.display, res, output_info->crtc);
...
#endif
    XPutImage(win->src.display, (Window)win->src.window, win->src.gc,
              win->src.bitmap, 0, 0, 0, 0, RGFW_bufferSize.w,
              RGFW_bufferSize.h);
#endif
  }
...
void RGFW_window_swapInterval(RGFW_window *win, i32 swapInterval) {
  ...
  ((PFNGLXSWAPINTERVALEXTPROC)glXGetProcAddress(
      (GLubyte *)"glXSwapIntervalEXT"))((Display *)win->src.display,
                                        (Window)win->src.window, swapInterval);
#else
...
void RGFW_window_close(RGFW_window *win) {
  ...
  if ((Display *)win->src.display) {
    ...
    if ((Drawable)win->src.window)
      XDestroyWindow((Display *)win->src.display,
...
window RGFW_dnd_init

- window args:
        #define RGFW_NO_RESIZE	 			the window cannot be
resized  by the user #define RGFW_ALLOW_DND     			the
window supports drag and drop #define RGFW_SCALE_TO_MONITOR
...
static void keyboard_key(void *data, struct wl_keyboard *keyboard,
  ...
  RGFW_updateLockState(RGFW_key_win, xkb_keymap_mod_get_index(keymap, "Lock"),
                       xkb_keymap_mod_get_index(keymap, "Mod2"));

  RGFW_keyCallback(RGFW_key_win, RGFW_key, name, RGFW_key_win->event.lockState,
                   state);
}
...
void RGFW_init_buffer(RGFW_window *win) {
  ...
  win->src.wl_buffer = wl_shm_pool_create_buffer(
      pool, 0, win->r.w, win->r.h, win->r.w * 4, WL_SHM_FORMAT_ARGB8888);
...
RGFW_window *RGFW_createWindow(const char *name, RGFW_rect rect, u16 args) {
  ...
  win->src.xdg_surface =
      xdg_wm_base_get_xdg_surface(xdg_wm_base, win->src.surface);
...
void RGFW_window_setMouse(RGFW_window *win, u8 *image, RGFW_area a,
  ...
  RGFW_UNUSED(a);
  RGFW_UNUSED(channels)
  // struct wl_cursor* cursor = wl_cursor_theme_get_cursor(RGFW_wl_cursor_theme,
  // iconStrings[mouse]); RGFW_cursor_image = image;
  struct wl_buffer *cursor_buffer =
      wl_cursor_image_get_buffer(RGFW_cursor_image);
...
u32 RGFW_mouseIconSrc[] = {OCR_NORMAL,   OCR_NORMAL,  OCR_IBEAM,  OCR_CROSS,
                           OCR_HAND,     OCR_SIZEWE,  OCR_SIZENS, OCR_SIZENWSE,
                           OCR_SIZENESW, OCR_SIZEALL, OCR_NO};
...
typedef i32(GLAPIENTRY *PFN_OSMesaMakeCurrent)(OSMesaContext, void *, int, int,
                                               int);
typedef OSMesaContext(GLAPIENTRY *PFN_OSMesaCreateContext)(GLenum,
                                                           OSMesaContext);
...
typedef HGLRC(WINAPI *PFNWGLCREATECONTEXTATTRIBSARBPROC)(HDC hdc, HGLRC hglrc,
...
void RGFW_loadXInput(void) {
  ...
  for (i = 0; i < sizeof(names) / sizeof(const char *); i++) {
    ...
    if (RGFW_XInput_dll) {
      XInputGetStateSRC = (PFN_XInputGetState)(void *)GetProcAddress(
          RGFW_XInput_dll, "XInputGetState");
...
RGFW_window *RGFW_createWindow(const char *name, RGFW_rect rect, u16 args) {
  ...
  if (RGFW_Shcore_dll == NULL) {
    ...
    GetDpiForMonitorSRC = (PFN_GetDpiForMonitor)(void *)GetProcAddress(
        RGFW_Shcore_dll, "GetDpiForMonitor");
...
#ifdef RGFW_WGL_LOAD
    wglCreateContextSRC =
        (PFN_wglCreateContext)GetProcAddress(wglinstance, "wglCreateContext");
    wglDeleteContextSRC =
        (PFN_wglDeleteContext)GetProcAddress(wglinstance, "wglDeleteContext");
    wglGetProcAddressSRC =
        (PFN_wglGetProcAddress)GetProcAddress(wglinstance, "wglGetProcAddress");
    wglMakeCurrentSRC =
        (PFN_wglMakeCurrent)GetProcAddress(wglinstance, "wglMakeCurrent");
    wglGetCurrentDCSRC =
        (PFN_wglGetCurrentDC)GetProcAddress(wglinstance, "wglGetCurrentDC");
    wglGetCurrentContextSRC = (PFN_wglGetCurrentContext)GetProcAddress(
        wglinstance, "wglGetCurrentContext");
#endif
  }
  ...
  HWND dummyWin =
      CreateWindowA(Class.lpszClassName, name, window_style, win->r.x, win->r.y,
                    win->r.w, win->r.h, 0, 0, inh, 0);
  ...
  win->src.window =
      CreateWindowA(Class.lpszClassName, name, window_style, win->r.x, win->r.y,
                    win->r.w, win->r.h + win->src.hOffset, 0, 0, inh, 0);
  ...
  if ((args & RGFW_NO_INIT_API) == 0) {
#ifdef RGFW_DIRECTX
    assert(FAILED(CreateDXGIFactory(&__uuidof(IDXGIFactory),
                                    (void **)&RGFW_dxInfo.pFactory)) == 0);

    if (FAILED(RGFW_dxInfo.pFactory->lpVtbl->EnumAdapters(
            RGFW_dxInfo.pFactory, 0, &RGFW_dxInfo.pAdapter))) {
    ...
    if (FAILED(D3D11CreateDevice(RGFW_dxInfo.pAdapter, D3D_DRIVER_TYPE_UNKNOWN,
                                 NULL, 0, featureLevels, 1, D3D11_SDK_VERSION,
                                 &RGFW_dxInfo.pDevice, NULL,
                                 &RGFW_dxInfo.pDeviceContext))) {
    ...
    RGFW_dxInfo.pFactory->lpVtbl->CreateSwapChain(
        RGFW_dxInfo.pFactory, (IUnknown *)RGFW_dxInfo.pDevice, &swapChainDesc,
        &win->src.swapchain);

    ID3D11Texture2D *pBackBuffer;
    win->src.swapchain->lpVtbl->GetBuffer(win->src.swapchain, 0,
                                          &__uuidof(ID3D11Texture2D),
                                          (LPVOID *)&pBackBuffer);
    RGFW_dxInfo.pDevice->lpVtbl->CreateRenderTargetView(
        RGFW_dxInfo.pDevice, (ID3D11Resource *)pBackBuffer, NULL,
        &win->src.renderTargetView);
    ...
    RGFW_dxInfo.pDevice->lpVtbl->CreateTexture2D(
        RGFW_dxInfo.pDevice, &depthStencilDesc, NULL, &pDepthStencilTexture);
    ...
    RGFW_dxInfo.pDevice->lpVtbl->CreateDepthStencilView(
        RGFW_dxInfo.pDevice, (ID3D11Resource *)pDepthStencilTexture,
        &depthStencilViewDesc, &win->src.pDepthStencilView);

    pDepthStencilTexture->lpVtbl->Release(pDepthStencilTexture);

    RGFW_dxInfo.pDeviceContext->lpVtbl->OMSetRenderTargets(
        RGFW_dxInfo.pDeviceContext, 1, &win->src.renderTargetView,
        win->src.pDepthStencilView);
#endif
    ...
    PIXELFORMATDESCRIPTOR pfd = {sizeof(pfd), 1,             /* version */
                                 pfd_flags,   PFD_TYPE_RGBA, /* ipixel type */
                                 24,                         /* color bits */
                                 0,           0,
                                 0,           0,
                                 0,           0,
    ...
    /* try to create the pixel format we want for opengl and then try to create
     * an opengl context for the specified version */
    if (wglCreateContextAttribsARB != NULL) {
      ...
      if (wglChoosePixelFormatARB != NULL) {
        i32 *pixel_format_attribs =
            (i32 *)RGFW_initFormatAttribs(args & RGFW_OPENGL_SOFTWARE);
  ...
  OSMesaMakeCurrentSource =
      (PFN_OSMesaMakeCurrent)GetProcAddress(win->src.hdc, "OSMesaMakeCurrent");
  OSMesaCreateContextSource = (PFN_OSMesaCreateContext)GetProcAddress(
      win->src.hdc, "OSMesaCreateContext");
  OSMesaDestroyContextSource = (PFN_OSMesaDestroyContext)GetProcAddress(
      win->src.hdc, "OSMesaDestroyContext");
#endif
#endif
  ...
  if (args & RGFW_TRANSPARENT_WINDOW) {
    SetWindowLong(win->src.window, GWL_EXSTYLE,
                  GetWindowLong(win->src.window, GWL_EXSTYLE) | WS_EX_LAYERED);
    SetLayeredWindowAttributes(win->src.window, RGB(255, 255, 255), RGFW_ALPHA,
                               LWA_ALPHA);
  }
...
RGFW_area RGFW_getScreenSize(void) {
  return RGFW_AREA(GetDeviceCaps(GetDC(NULL), HORZRES),
...
static i32 RGFW_checkXInput(RGFW_window *win, RGFW_Event *e) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if ((keystroke.Flags & XINPUT_KEYSTROKE_REPEAT) == 0 &&
      ...
      e->type =
          RGFW_jsButtonPressed + !(keystroke.Flags & XINPUT_KEYSTROKE_KEYDOWN);
      e->button = RGFW_xinput2RGFW[keystroke.VirtualKey - 0x5800];
      RGFW_jsPressed[i][e->button] =
...
#define INPUT_DEADZONE                                                         \
  (0.24f *                                                                     \
   (float)(0x7FFF)) // Default to 24% of the +/- 32767 range.   This is a
    ...
    e->axisesCount = 2;
    RGFW_point axis1 =
        RGFW_POINT(state.Gamepad.sThumbLX, state.Gamepad.sThumbLY);
    RGFW_point axis2 =
        RGFW_POINT(state.Gamepad.sThumbRX, state.Gamepad.sThumbRY);

    if (axis1.x != e->axis[0].x || axis1.y != e->axis[0].y ||
        axis2.x != e->axis[1].x || axis2.y != e->axis[1].y) {
...
RGFW_Event *RGFW_window_checkEvent(RGFW_window *win) {
  ...
  if (win->event.type == RGFW_dnd_init) {
    ...
    // win->event.droppedFiles =
    // (char**)RGFW_CALLOC(win->event.droppedFilesCount, sizeof(char*));
    ...
    for (i = 0; i < win->event.droppedFilesCount; i++) {
      ...
      strncpy(win->event.droppedFiles[i], createUTF8FromWideStringWin32(buffer),
              RGFW_MAX_PATH);
  ...
  if (PeekMessageA(&msg, win->src.window, 0u, 0u, PM_REMOVE)) {
    ...
    case WM_KEYUP: {
      ...
      RGFW_updateLockState(win, (GetKeyState(VK_CAPITAL) & 0x0001),
                           (GetKeyState(VK_NUMLOCK) & 0x0001));

      strncpy(win->event.keyName, keyName, 16);

      if (RGFW_isPressed(win, RGFW_ShiftL)) {
        ToAscii((UINT)msg.wParam,
                MapVirtualKey((UINT)msg.wParam, MAPVK_VK_TO_CHAR),
...
RGFW_monitor win32CreateMonitor(HMONITOR src) {
  ...
  if (EnumDisplayMonitors(NULL, NULL, GetMonitorByHandle, (LPARAM)&info)) {
     ...
     */
    size_t deviceIndex;
    for (deviceIndex = 0; EnumDisplayDevicesA(0, (DWORD)deviceIndex, &dd, 0);
         deviceIndex++) {
      char *deviceName = dd.DeviceName;
      if (EnumDisplayDevicesA(deviceName, info.iIndex, &dd, 0)) {
        strncpy(monitor.name, dd.DeviceString,
                128); /*!< copy the monitor's name */
        break;
      }
    }
  }
...
void si_impl_func_to_SEL_with_name(const char *class_name,
                                   const char *register_name, void *function) {
...
#define si_func_to_SEL(class_name, function)                                   \
  si_impl_func_to_SEL_with_name(class_name, #function ":", function)
...
#define si_func_to_SEL_with_name(class_name, register_name, function)          \
  si_impl_func_to_SEL_with_name(class_name, register_name ":", function)
...
typedef NS_ENUM(NSUInteger, NSBitmapFormat) {
  NSBitmapFormatAlphaFirst = 1
                             << 0, // 0 means is alpha last (RGBA, CMYKA, etc.)
...
NSBitmapImageRep *NSBitmapImageRep_initWithBitmapData(
  ...
  return (NSBitmapImageRep *)((id(*)(id, SEL, unsigned char **, NSInteger,
                                     NSInteger, NSInteger, NSInteger, bool,
                                     bool, const char *, NSBitmapFormat,
...
NSColor *NSColor_colorWithSRGB(CGFloat red, CGFloat green, CGFloat blue,
                               CGFloat alpha) {
...
typedef NS_ENUM(NSInteger, NSOpenGLContextParameter) {
  NSOpenGLContextParameterSwapInterval NS_OPENGL_ENUM_DEPRECATED(10.0, 10.14) =
      222, /* 1 param.  0 -> Don't sync, 1 -> Sync to vertical retrace     */
  NSOpenGLContextParametectxaceOrder NS_OPENGL_ENUM_DEPRECATED(10.0, 10.14) =
      235, /* 1 param.  1 -> Above Window (default), -1 -> Below Window    */
  NSOpenGLContextParametectxaceOpacity NS_OPENGL_ENUM_DEPRECATED(10.0, 10.14) =
      236, /* 1 param.  1-> Surface is opaque (default), 0 -> non-opaque   */
  NSOpenGLContextParametectxaceBackingSize NS_OPENGL_ENUM_DEPRECATED(
      10.0, 10.14) = 304, /* 2 params.  Width/height of surface backing size */
  ...
  NSOpenGLContextParameterGPUVertexProcessing NS_OPENGL_ENUM_DEPRECATED(10.0,
                                                                        10.14) =
  ...
  NSOpenGLContextParameterHasDrawable NS_OPENGL_ENUM_DEPRECATED(10.0, 10.14) =
  ...
  NSOpenGLContextParameterSwapRectangle API_DEPRECATED("", macos(10.0, 10.14)) =
      200, /* 4 params.  Set or get the swap rectangle {x, y, w, h} */
  NSOpenGLContextParameterSwapRectangleEnable API_DEPRECATED(
      "", macos(10.0, 10.14)) = 201, /* Enable or disable the swap rectangle */
  NSOpenGLContextParameterRasterizationEnable API_DEPRECATED(
      "", macos(10.0, 10.14)) = 221, /* Enable or disable all rasterization */
...
void NSOpenGLContext_setValues(NSOpenGLContext *context, const int *vals,
...
NSPasteboard *NSPasteboard_generalPasteboard(void) {
  return (NSPasteboard *)objc_msgSend_id((id)objc_getClass("NSPasteboard"),
...
const char *NSPasteboard_stringForType(NSPasteboard *pasteboard,
                                       NSPasteboardType dataType) {
  void *func = sel_registerName("stringForType:");
  return (const char *)NSString_to_char(
...
void NSregisterForDraggedTypes(void *view, NSPasteboardType *newTypes,
                               size_t len) {
...
NSInteger NSPasteBoard_declareTypes(NSPasteboard *pasteboard,
                                    NSPasteboardType *newTypes, size_t len,
                                    void *owner) {
  NSString **ntypes = cstrToNSStringArray((char **)newTypes, len);
...
bool NSPasteBoard_setString(NSPasteboard *pasteboard, const char *stringToWrite,
                            NSPasteboardType dataType) {
...
NSPasteboardType const NSPasteboardTypeString =
    "public.utf8-plain-text"; // Replaces NSStringPboardType
...
const char **NSPasteboard_readObjectsForClasses(NSPasteboard *pasteboard,
                                                Class *classArray, size_t len,
                                                void *options) {
  ...
  NSRelease(array);
  NSUInteger count =
      ((NSUInteger(*)(id, SEL))objc_msgSend)(output, sel_registerName("count"));
...
char *RGFW_mouseIconSrc[] = {"arrowCursor",
                             "arrowCursor",
                             "IBeamCursor",
                             "crosshairCursor",
                             "pointingHandCursor",
...
void *RGFW_getProcAddress(const char *procname) {
  ...
  void *symbol =
      CFBundleGetFunctionPointerForName(RGFWnsglFramework, symbolName);
...
bool performDragOperation(id self, SEL sel, id sender) {
  ...
  // NSPasteboard* pasteBoard = objc_msgSend_id(sender,
...
static void NSMoveToResourceDir(void) {
  ...
  if (CFStringCompare(CFSTR("Resources"), last, 0) != kCFCompareEqualTo ||
      CFURLGetFileSystemRepresentation(resourcesURL, true, (u8 *)resourcesPath,
                                       255) == 0) {
    CFRelease(last);
...
void RGFW__osxWindowMove(void *self, SEL sel) {
  ...
  NSRect frame = ((NSRect(*)(id, SEL))abi_objc_msgSend_stret)(
      win->src.window, sel_registerName("frame"));
...
void *RGFW_cocoaGetLayer(void) {
  return objc_msgSend_class(objc_getClass("CAMetalLayer"),
  ...
  */
  ...
  if ((args & RGFW_NO_INIT_API) == 0) {
    ...
    win->src.view = NSOpenGLView_initWithFrame(
        (NSRect){{0, 0}, {win->r.w, win->r.h}}, format);
  ...
  objc_msgSend_void_id(win->src.window, sel_registerName("setContentView:"),
                       win->src.view);
...
#define NSOpenGLCPSurfaceOpacity 236
      NSOpenGLContext_setValues(win->src.ctx, &opacity,
    ...
    objc_msgSend_void_id(win->src.window,
                         sel_registerName("setBackgroundColor:"),
                         NSColor_colorWithSRGB(0, 0, 0, 0));
  }

  win->src.display = CGMainDisplayID();
  CVDisplayLinkCreateWithCGDisplay(win->src.display,
                                   (CVDisplayLinkRef *)&win->src.displayLink);
  ...
  objc_msgSend_void_id(win->src.window, sel_registerName("setDelegate:"),
                       delegate);
  ...
  // Show the window
  objc_msgSend_void_bool(NSApp, sel_registerName("activateIgnoringOtherApps:"),
                         true);
  ((id(*)(id, SEL, SEL))objc_msgSend)(
      win->src.window, sel_registerName("makeKeyAndOrderFront:"), NULL);
...
void RGFW_window_setBorder(RGFW_window *win, u8 border) {
  ...
  ((void (*)(id, SEL, NSBackingStoreType))objc_msgSend)(
      win->src.window, sel_registerName("setStyleMask:"), storeType);

  objc_msgSend_void_bool(win->src.window, sel_registerName("setHasShadow:"),
                         border);
}
...
RGFW_point RGFW_window_getMousePoint(RGFW_window *win) {
  NSPoint p = ((NSPoint(*)(id, SEL))objc_msgSend)(
      win->src.window, sel_registerName("mouseLocationOutsideOfEventStream"));
...
typedef NS_ENUM(
    u32, NSEventType) { /* various types of events */
                        ...
                        /* The following event types are available on some
                           hardware on 10.5.2 and later */
...
typedef NS_ENUM(
    unsigned long long,
    NSEventMask) { /* masks for the types of events */
                   ...
                   NSEventMaskLeftMouseDragged = 1ULL
                                                 << NSEventTypeLeftMouseDragged,
                   NSEventMaskRightMouseDragged =
                       1ULL << NSEventTypeRightMouseDragged,
                   ...
                   NSEventMaskTabletProximity = 1ULL
                                                << NSEventTypeTabletProximity,
                   NSEventMaskOtherMouseDown = 1ULL
                                               << NSEventTypeOtherMouseDown,
                   NSEventMaskOtherMouseUp = 1ULL << NSEventTypeOtherMouseUp,
                   NSEventMaskOtherMouseDragged =
                       1ULL << NSEventTypeOtherMouseDragged,
                   /* The following event masks are available on some hardware
                      on 10.5.2 and later */
                   NSEventMaskGesture API_AVAILABLE(macos(10.5)) =
                       1ULL << NSEventTypeGesture,
                   NSEventMaskMagnify API_AVAILABLE(macos(10.5)) =
                       1ULL << NSEventTypeMagnify,
                   NSEventMaskSwipe API_AVAILABLE(macos(10.5)) =
                       1ULL << NSEventTypeSwipe,
                   NSEventMaskRotate API_AVAILABLE(macos(10.5)) =
                       1ULL << NSEventTypeRotate,
                   ...
                   /* Note: You can only use these event masks on 64 bit. In
                    * other words, you cannot setup a local, nor global, event
                    * monitor for these event types on 32 bit. Also, you cannot
                    * search the event queue for them
...
void RGFW_stopCheckEvents(void) {
  ...
  NSEvent *e = (NSEvent *)((
      id(*)(id, SEL, NSEventType, NSPoint, NSEventModifierFlags, void *,
            NSInteger, void **, short, NSInteger, NSInteger))objc_msgSend)(
      NSApp,
...
void RGFW_window_eventWait(RGFW_window *win, i32 waitMS) {
  ...
  NSEvent *e = (NSEvent *)((
      id(*)(id, SEL, NSEventMask, void *, NSString *, bool))objc_msgSend)(
      NSApp,
...
RGFW_Event *RGFW_window_checkEvent(RGFW_window *win) {
  ...
  if ((win->event.type == RGFW_dnd || win->event.type == RGFW_dnd_init) &&
      win->src.dndPassed == 0) {
    win->src.dndPassed = 1;
    return &win->event;
  }
  ...
  case NSEventTypeKeyDown: {
    ...
    RGFW_keyboard[win->event.keyCode].prev =
        RGFW_keyboard[win->event.keyCode].current;

    win->event.type = RGFW_keyPressed;
    char *str = (char *)(const char *)NSString_to_char(
        objc_msgSend_id(e, sel_registerName("characters")));
    strncpy(win->event.keyName, str, 16);
    win->event.repeat = RGFW_isPressed(win, win->event.keyCode);
    RGFW_keyboard[win->event.keyCode].current = 1;
  ...
  case NSEventTypeFlagsChanged: {
    ...
    RGFW_keyCallback(win, win->event.keyCode, win->event.keyName,
                     win->event.lockState, win->event.type == RGFW_keyPressed);
...
void RGFW_window_move(RGFW_window *win, RGFW_point v) {
  ...
  ((void (*)(id, SEL, NSRect, bool, bool))objc_msgSend)(
      win->src.window, sel_registerName("setFrame:display:animate:"),
...
void RGFW_window_setMousePassthrough(RGFW_window *win, b8 passthrough) {
  objc_msgSend_void_bool(
      win->src.window, sel_registerName("setIgnoresMouseEvents:"), passthrough);
}
#endif
...
void RGFW_window_setIcon(RGFW_window *win, u8 *data, RGFW_area area,
                         i32 channels) {
  assert(win != NULL);

  /* code by EimaMei  */
  // Make a bitmap representation, then copy the loaded image into it.
  void *representation = NSBitmapImageRep_initWithBitmapData(
      NULL, area.w, area.h, 8, channels, (channels == 4), false,
      "NSCalibratedRGBColorSpace", 1 << 1, area.w * channels, 8 * channels);
  memcpy(NSBitmapImageRep_bitmapData(representation), data,
...
void RGFW_window_setMouse(RGFW_window *win, u8 *image, RGFW_area a,
  ...
  memcpy(NSBitmapImageRep_bitmapData(representation), image,
         a.w * a.h * channels);
...
u8 RGFW_window_isFullscreen(RGFW_window *win) {
  ...
  NSWindowStyleMask mask = (NSWindowStyleMask)objc_msgSend_uint(
      win->src.window, sel_registerName("styleMask"));
...
static RGFW_monitor RGFW_NSCreateMonitor(CGDirectDisplayID display) {
  ...
  monitor.scaleY =
      ((monitor.rect.h / (screenSizeMM.height / 25.4)) / 96) + 0.25;
...
char *RGFW_readClipboard(size_t *size) {
  char *clip = (char *)NSPasteboard_stringForType(
      NSPasteboard_generalPasteboard(), NSPasteboardTypeString);
...
CGImageRef createImageFromBytes(unsigned char *buffer, int width, int height) {
  ...
  CGContextRef context =
      CGBitmapContextCreate(buffer, width, height, 8, RGFW_bufferSize.w * 4,
...
void RGFW_window_swapBuffers(RGFW_window *win) {
  ...
  if (!(win->_winArgs & RGFW_NO_CPU_RENDER)) {
    ...
    id graphicsContext = objc_msgSend_class(objc_getClass("NSGraphicsContext"),
...
EM_BOOL Emscripten_on_keydown(int eventType, const EmscriptenKeyboardEvent *e,
                              void *userData) {
  ...
  RGFW_keyboard[RGFW_apiKeyCodeToRGFW(e->keyCode)].prev =
      RGFW_keyboard[RGFW_apiKeyCodeToRGFW(e->keyCode)].current;
  RGFW_keyboard[RGFW_apiKeyCodeToRGFW(e->keyCode)].current = 1;
  RGFW_keyCallback(RGFW_root, RGFW_apiKeyCodeToRGFW(e->keyCode),
                   RGFW_events[RGFW_eventLen].keyName, 0, 1);

  return EM_TRUE;
}

EM_BOOL Emscripten_on_keyup(int eventType, const EmscriptenKeyboardEvent *e,
                            void *userData) {
  RGFW_UNUSED(eventType);
  RGFW_UNUSED(userData);
  ...
  RGFW_keyCallback(RGFW_root, RGFW_apiKeyCodeToRGFW(e->keyCode),
                   RGFW_events[RGFW_eventLen].keyName, 0, 0);
...
EM_BOOL Emscripten_on_fullscreenchange(int eventType,
...
EM_BOOL Emscripten_on_touchcancel(int eventType, const EmscriptenTouchEvent *e,
                                  void *userData) {
  RGFW_UNUSED(eventType);
  RGFW_UNUSED(userData);
  RGFW_UNUSED(e);
...
RGFW_window *RGFW_createWindow(const char *name, RGFW_rect rect, u16 args) {
  ...
  emscripten_set_keydown_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL,
                                  EM_FALSE, Emscripten_on_keydown);
...
void RGFW_window_setMouse(RGFW_window *win, u8 *image, RGFW_area a,
                          i32 channels) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(channels) RGFW_UNUSED(a) RGFW_UNUSED(image)
}

const char RGFW_CURSORS[11][12] = {"default",   "default",     "text",
                                   "crosshair", "pointer",     "ew-resize",
                                   "ns-resize", "nwse-resize", "nesw-resize",
                                   "move",      "not-allowed"};
...
void RGFW_window_swapBuffers(RGFW_window *win) {
  ...
  if (!(win->_winArgs & RGFW_NO_CPU_RENDER)) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, RGFW_bufferSize.w,
                 RGFW_bufferSize.h, 0, GL_RGBA, GL_UNSIGNED_BYTE, win->buffer);
...
void RGFW_window_setIcon(RGFW_window *win, u8 *icon, RGFW_area a,
                         i32 channels) {
  RGFW_UNUSED(win) RGFW_UNUSED(icon) RGFW_UNUSED(a) RGFW_UNUSED(channels)
}
void RGFW_window_hide(RGFW_window *win) { RGFW_UNUSED(win) }
void RGFW_window_show(RGFW_window *win){RGFW_UNUSED(win)} b8
    RGFW_window_isHidden(RGFW_window *win) {
  RGFW_UNUSED(win) return 0;
}
...
RGFW_monitor RGFW_window_getMonitor(RGFW_window *win) {
  RGFW_UNUSED(win) return (RGFW_monitor){};
}
