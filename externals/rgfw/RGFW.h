...
/*
        (MAKE SURE RGFW_IMPLEMENTATION is in exactly one header or you use -D
   RGFW_IMPLEMENTATION) #define RGFW_IMPLEMENTATION - makes it so source code is
   included with header
*/

/*
        #define RGFW_IMPLEMENTATION - (required) makes it so the source code is
   included #define RGFW_DEBUG - (optional) makes it so RGFW prints debug
   messages anderrors when they're found #define RGFW_OSMESA - (optional) use
   OSmesa as backend (instead of system's opengl api + regular opengl) #define
   RGFW_BUFFER - (optional) just draw directly to (RGFW) window pixel buffer
   that is drawn to screen (the buffer is in the RGBA format) #define RGFW_EGL -
   (optional) use EGL for loading an OpenGL context (instead of the system's
   opengl api) #define RGFW_OPENGL_ES1 - (optional) use EGL to load and use
   Opengl ES (version 1) for backend rendering (instead of the system's opengl
   api) This version doesn't work for desktops (I'm pretty sure) #define
   RGFW_OPENGL_ES2 - (optional) use OpenGL ES (version 2) #define
   RGFW_OPENGL_ES3 - (optional) use OpenGL ES (version 3) #define RGFW_DIRECTX -
   (optional) use directX for the rendering backend (rather than opengl)
   (windows only, defaults to opengl for unix) #define RGFW_WEBGPU - (optional)
   use webGPU for rendering (Web ONLY) #define RGFW_NO_API - (optional) don't
   use any rendering API (no opengl, no vulkan, no directX)

        #define RGFW_LINK_EGL (optional) (windows only) if EGL is being used, if
   EGL functions should be defined dymanically (using GetProcAddress) #define
   RGFW_X11 (optional) (unix only) if X11 should be used. This option is turned
   on by default by unix systems except for MacOS #define RGFW_WAYLAND
   (optional) (unix only) if Wayland should be used. (This can be used with X11)
        #define RGFW_NO_X11 (optional) (unix only) if X11 should be used (with
   Wayland). #define RGFW_NO_LOAD_WGL (optional) (windows only) if WGL should be
   loaded dynamically during runtime #define RGFW_NO_X11_CURSOR (optional) (unix
   only) don't use XCursor #define RGFW_NO_X11_CURSOR_PRELOAD (optional) (unix
   only) Use XCursor, but don't link it in code, (you'll have to link it with
   -lXcursor) #define RGFW_NO_LOAD_WINMM (optional) (windows only) Use winmm
   (timeBeginPeriod), but don't link it in code, (you'll have to link it with
   -lwinmm) #define RGFW_NO_WINMM (optional) (windows only) don't use winmm
        #define RGFW_NO_IOKIT (optional) (macOS) don't use IOKit
        #define RGFW_NO_UNIX_CLOCK (optional) (unux) don't link unix clock
   functions #define RGFW_NO_DWM (windows only) - Do not use or linj dwmapi
        #define RGFW_USE_XDL (optional) (X11) use X11 in RGFW (must include
   XDL.h along with RGFW) (XLib Dynamic Loader)

        #define RGFW_NO_DPI - Do not include calculate DPI (no XRM nor libShcore
   included)

        #define RGFW_ALLOC x  - choose what default function to use to allocate,
   by default the standard malloc is used #define RGFW_FREE x - choose what
   default function to use to allocated memory, by default the standard free is
   used #define RGFW_USERPTR x - choose the default userptr sent to the malloc
   call, (NULL by default)

        #define RGFW_EXPORT - Use when building RGFW
    #define RGFW_IMPORT - Use when linking with RGFW (not as a single-header)

        #define RGFW_USE_INT - force the use c-types rather than stdint.h (for
   systems that might not have stdint.h (msvc)) #define RGFW_bool x - choose
   what type to use for bool, by default u32 is used
*/
...
0x00, 0x00, 0xFF,   0xFF, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0xFF, 0xFF, 0xFF,
0x00, 0xFF,     0xFF, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0xFF, 0xFF, 0x00,
0x00, 0xFF};

int main() {
        RGFW_window* win = RGFW_createWindow("name", RGFW_RECT(100, 100, 500,
500), (u64)0);

        RGFW_window_setIcon(win, icon, RGFW_AREA(3, 3), 4);

        for (;;) {
                RGFW_window_checkEvent(win); // NOTE: checking events outside of
a while loop may cause input lag if (win->event.type == RGFW_quit ||
RGFW_isPressed(win, RGFW_escape)) break;
        ...
        if you wish to compile the library all you have to do is create a new
file with this in it
        ...
        then you can use gcc (or whatever compile you wish to use) to compile
the library into object file

        ex. gcc -c RGFW.c -fPIC

        after you compile the library into an object file, you can also turn the
object file into an static or shared library

        (commands ar and gcc can be replaced with whatever equivalent your
system uses)
        ...
        shared :
                ...
                macos:
                        gcc -shared RGFW.o -framework Cocoa -framework OpenGL
-framework IOKit
*/

/*
        Credits :
                EimaMei/Sacode : Much of the code for creating windows using
   winapi, Wrote the Silicon library, helped with MacOS Support, siliapp.h ->
   referencing

                stb - This project is heavily inspired by the stb single header
   files

                GLFW:
                        certain parts of winapi and X11 are very poorly
   documented, GLFW's source code was referenced and used throughout the
   project.

                contributors : (feel free to put yourself here if you
   contribute) krisvers -> code review EimaMei (SaCode) -> code review
                Code-Nycticebus -> bug fixes
                Rob Rohan -> X11 bugs and missing features, MacOS/Cocoa fixing
   memory issues/bugs AICDG (@THISISAGOODNAME) -> vulkan support (example)
                ...
                Nick Porcino (meshula) - testing, organization, review (MacOS,
   examples)
...
#define RGFW_BOOL(x)                                                           \
  ((x) ? RGFW_TRUE : RGFW_FALSE) /* force an value to be 0 or 1 */
#define RGFW_TRUE 1
#define RGFW_FALSE 0
...
#if defined(_WIN32) && !defined(RGFW_UNIX) && !defined(RGFW_WASM) &&           \
    !defined(                                                                  \
        RGFW_CUSTOM_BACKEND) /* (if you're using X11 on windows some how) */
#define RGFW_WINDOWS
...
#if !defined(RGFW_NO_API) &&                                                   \
    (!defined(RGFW_BUFFER) || defined(RGFW_OPENGL)) && !defined(RGFW_OSMESA)
#define RGFW_EGL
#define RGFW_OPENGL
#define RGFW_UNIX
...
#endif
#if !defined(RGFW_NO_X11) && !defined(RGFW_NO_X11) &&                          \
    (defined(__unix__) || defined(RGFW_MACOS_X11) || defined(RGFW_X11)) &&     \
    !defined(RGFW_WASM) && !defined(RGFW_CUSTOM_BACKEND)
#define RGFW_MACOS_X11
#define RGFW_X11
#define RGFW_UNIX
#include <X11/Xlib.h>
#elif defined(__APPLE__) && !defined(RGFW_MACOS_X11) && !defined(RGFW_X11) &&  \
    !defined(RGFW_WASM) && !defined(RGFW_CUSTOM_BACKEND)
#define RGFW_MACOS
#endif

#if (defined(RGFW_OPENGL_ES1) || defined(RGFW_OPENGL_ES2) ||                   \
     defined(RGFW_OPENGL_ES3)) &&                                              \
    !defined(RGFW_EGL)
#define RGFW_EGL
#endif

#if !defined(RGFW_OSMESA) && !defined(RGFW_EGL) && !defined(RGFW_OPENGL) &&    \
    !defined(RGFW_DIRECTX) && !defined(RGFW_BUFFER) && !defined(RGFW_NO_API)
#define RGFW_OPENGL
#endif
...
/*! (unix) Toggle use of wayland, this will be on by default if you use
   `RGFW_WAYLAND` (if you don't use RGFW_WAYLAND, you dont' expose WAYLAND
   functions) this is mostly used to allow you to force the use of XWayland
*/
...
typedef RGFW_ENUM(u8, RGFW_eventType){
    ...
    /*! key event note
            ...
            RGFW_event.key
            !!Keycodes defined at the bottom of the RGFW_HEADER part of this
       file!!
    ...
    */
    RGFW_mouseButtonPressed,  /*!< a mouse button has been pressed
                                 (left,middle,right)*/
    RGFW_mouseButtonReleased, /*!< a mouse button has been released
                                 (left,middle,right)*/
    RGFW_mousePosChanged,     /*!< the position of the mouse has been changed*/
    /*! mouse event note
            the x and y of the mouse can be found in the vector,
       RGFW_event.point
    ...
    */
    ...
    RGFW_windowResized, /*!< the window was resized (by the user), [on WASM this
                           means the browser was resized] */
    ...
    */
    ...
    RGFW_DNDInit /*!< the start of a dnd event, when the place where the file
                    drop is known */
                 /* dnd data note
                         The x and y coords of the drop are stored in the vector
                    RGFW_event.point
             
                         RGFW_event.droppedFilesCount holds how many files were dropped
             
                         This is also the size of the array which stores all the dropped file
                    string,              RGFW_event.droppedFiles
                 */
};
...
typedef RGFW_ENUM(u8, RGFW_mouseButton){
    ...
    RGFW_mouseMisc1,      RGFW_mouseMisc2, RGFW_mouseMisc3,
    RGFW_mouseMisc4,      RGFW_mouseMisc5, RGFW_mouseFinal};
...
typedef RGFW_ENUM(u8, RGFW_keymod){
    RGFW_modCapsLock = RGFW_BIT(0),  RGFW_modNumLock = RGFW_BIT(1),
    RGFW_modControl = RGFW_BIT(2),   RGFW_modAlt = RGFW_BIT(3),
    RGFW_modShift = RGFW_BIT(4),     RGFW_modSuper = RGFW_BIT(5),
    RGFW_modScrollLock = RGFW_BIT(6)};

/*! gamepad button codes (based on xbox/playstation), you may need to change
 * these values per controller */
...
/*
        NOTE : Monitor functions should be ran only as many times as needed (not
   in a loop)
*/
...
/*!< loads mouse from bitmap (similar to RGFW_window_setIcon), icon NOT resized
 * by default*/
...
/* NOTE: some parts of the data can represent different things based on the
 * event (read comments in RGFW_event struct) */
...
typedef struct RGFW_event {
  ...
  RGFW_key key; /*!< the physical key of the event, refers to where key is
                   physically !!Keycodes defined at the bottom of the
                   RGFW_HEADER part of this file!! */
  u8 keyChar;   /*!< mapped key char of the event*/

  RGFW_bool repeat;  /*!< key press event repeated (the key is being held) */
  RGFW_bool inFocus; /*!< if the window is in focus or not (this is always true
                        for MacOS windows due to the api being weird) */
...
typedef struct RGFW_window_src {
  ...
  RGFW_area maxSize, minSize,
      aspectRatio; /*!< for setting max/min resize (RGFW_WINDOWS) */
} RGFW_window_src;
...
typedef RGFW_ENUM(u32, RGFW_windowFlags){
    RGFW_windowNoInitAPI =
        RGFW_BIT(0), /* DO not init an API (mostly for bindings, you should use
                        `#define RGFW_NO_API` in C */
    RGFW_windowNoBorder = RGFW_BIT(1), /*!< the window doesn't have border */
    RGFW_windowNoResize =
        RGFW_BIT(2), /*!< the window cannot be resized  by the user */
    RGFW_windowAllowDND = RGFW_BIT(3), /*!< the window supports drag and drop*/
    RGFW_windowHideMouse =
        RGFW_BIT(4), /*! the window should hide the mouse or not (can be toggled
                        later on) using `RGFW_window_mouseShow*/
    RGFW_windowFullscreen =
        RGFW_BIT(5), /* the window is fullscreen by default or not */
    RGFW_windowTransparent =
        RGFW_BIT(6), /*!< the window is transparent (only properly works on X11
                        and MacOS, although it's although for windows) */
    RGFW_windowCenter = RGFW_BIT(7), /*! center the window on the screen */
    RGFW_windowOpenglSoftware =
        RGFW_BIT(8), /*! use OpenGL software rendering */
    RGFW_windowCocoaCHDirToRes =
        RGFW_BIT(9), /* (cocoa only), change directory to resource folder */
    RGFW_windowScaleToMonitor =
        RGFW_BIT(10),               /* scale the window to the screen */
...
/*!
 * the class name for X11 and WinAPI. apps with the same class will be grouped
 * by the WM by default the class name will == the root window's name
 */
...
/* NOTE: (windows)If the executable has an icon resource named RGFW_ICON, it
 * will be set as the initial icon for the window.*/
...
RGFWDEF RGFW_window *RGFW_createWindowPtr(
    ...
    RGFW_rect rect,   /* rect of window */
    RGFW_windowFlags
        flags,       /* extra arguments (NULL / (u32)0 means no flags used)*/
...
RGFWDEF void RGFW_window_initBufferPtr(RGFW_window *win, u8 *buffer,
                                       RGFW_area area);
...
/*!
        this function checks an *individual* event (and updates window structure
   attributes) this means, using this function without a while loop may cause
   event lag

        ex.

        while (RGFW_window_checkEvent(win) != NULL) [this keeps checking events
   until it reaches the last one]

        this function is optional if you choose to use event callbacks,
        although you still need some way to tell RGFW to process events eg.
   `RGFW_window_checkEvents`
*/

RGFWDEF RGFW_event *RGFW_window_checkEvent(
    RGFW_window *win); /*!< check current event (returns a pointer to win->event
                          or NULL if there is no event)*/

/*!
        for RGFW_window_eventWait and RGFW_window_checkEvents
        waitMS -> Allows th	e function to keep checking for events even
   after `RGFW_window_checkEvent == NULL` if waitMS == 0, the loop will not wait
   for events if waitMS == a positive integer, the loop will wait that many
   miliseconds after there are no more events until it returns if waitMS == a
   the max size of a 32-bit int (or -1), the loop will not return until it gets
   another event
*/
typedef RGFW_ENUM(u32, RGFW_eventWait){RGFW_eventNoWait = 0,
...
/*!
        Tell RGFW_window_eventWait to stop waiting, to be ran from another
   thread
*/
RGFWDEF void RGFW_stopCheckEvents(void);

/*! window managment functions*/
RGFWDEF void RGFW_window_close(
    RGFW_window *win); /*!< close the window and free leftover data */

/*! moves window to a given point */
RGFWDEF void RGFW_window_move(RGFW_window *win, RGFW_point v /*!< new pos*/
);

#ifndef RGFW_NO_MONITOR
/*! move to a specific monitor */
RGFWDEF void RGFW_window_moveToMonitor(RGFW_window *win,
                                       RGFW_monitor m /* monitor */);
#endif
...
RGFWDEF void
RGFW_window_maximize(RGFW_window *win); /*!< maximize the window size */
RGFWDEF void RGFW_window_setFullscreen(
    RGFW_window *win, RGFW_bool fullscreen); /*!< fullscreen the window size */
RGFWDEF void RGFW_window_center(RGFW_window *win); /*!< center the window */
RGFWDEF void RGFW_window_minimize(
    RGFW_window *win); /*!< minimize the window (in taskbar (per OS))*/
RGFWDEF void RGFW_window_restore(
    RGFW_window *win); /*!< restore the window from minimized (per OS)*/
...
RGFWDEF RGFW_bool RGFW_window_setIcon(
    RGFW_window *win, /*!< source window */
    u8 *icon /*!< icon bitmap */,
...
/*!< sets the mouse to a standard API cursor (based on RGFW_MOUSE, as seen at
 * the end of the RGFW_HEADER part of this file) */
RGFWDEF RGFW_bool RGFW_window_setMouseStandard(RGFW_window *win, u8 mouse);

RGFWDEF RGFW_bool RGFW_window_setMouseDefault(
    RGFW_window *win); /*!< sets the mouse to the default mouse icon */
/*
...
this is run by default if the user uses the arg `RGFW_scaleToMonitor` during
window creation
*/
...
/*! if window == NULL, it checks if the key is pressed globally. Otherwise, it
 * checks only if the key is pressed while the window in focus.*/
RGFWDEF RGFW_bool RGFW_isPressed(
    RGFW_window *win, RGFW_key key); /*!< if key is pressed (key code)*/

RGFWDEF RGFW_bool RGFW_wasPressed(
    RGFW_window *win,
    RGFW_key
        key); /*!< if key was pressed (checks previous state only) (key code)*/

RGFWDEF RGFW_bool RGFW_isHeld(RGFW_window *win,
                              RGFW_key key); /*!< if key is held (key code)*/
RGFWDEF RGFW_bool RGFW_isReleased(
    RGFW_window *win, RGFW_key key); /*!< if key is released (key code)*/

/* if a key is pressed and then released, pretty much the same as
 * RGFW_isReleased */
RGFWDEF RGFW_bool RGFW_isClicked(RGFW_window *win,
                                 RGFW_key key /*!< key code*/);

/*! if a mouse button is pressed */
RGFWDEF RGFW_bool RGFW_isMousePressed(
    RGFW_window *win, RGFW_mouseButton button /*!< mouse button code */);
/*! if a mouse button is held */
RGFWDEF RGFW_bool RGFW_isMouseHeld(
    RGFW_window *win, RGFW_mouseButton button /*!< mouse button code */);
/*! if a mouse button was released */
RGFWDEF RGFW_bool RGFW_isMouseReleased(
    RGFW_window *win, RGFW_mouseButton button /*!< mouse button code */);
/*! if a mouse button was pressed (checks previous state only) */
RGFWDEF RGFW_bool RGFW_wasMousePressed(
    RGFW_window *win, RGFW_mouseButton button /*!< mouse button code */);
/** @} */
...
RGFWDEF const char *
RGFW_readClipboard(size_t *size); /*!< read clipboard data */
/*! read clipboard data or send a NULL str to just get the length of the
 * clipboard data */
RGFWDEF RGFW_ssize_t RGFW_readClipboardPtr(char *str, size_t strCapacity);
RGFWDEF void
RGFW_writeClipboard(const char *text,
                    u32 textLen); /*!< write text to the clipboard */
/** @} */
...
/*! RGFW_focusIn / RGFW_focusOut, the window who's focus has changed and if its
 * inFocus */
typedef void (*RGFW_focusfunc)(RGFW_window *win, RGFW_bool inFocus);
/*! RGFW_mouseEnter / RGFW_mouseLeave, the window that changed, the point of the
 * mouse (enter only) and if the mouse has entered */
typedef void (*RGFW_mouseNotifyfunc)(RGFW_window *win, RGFW_point point,
                                     RGFW_bool status);
/*! RGFW_mousePosChanged, the window that the move happened on and the new point
 * of the mouse  */
...
/*! RGFW_keyPressed / RGFW_keyReleased, the window that got the event, the
 * mapped key, the physical key, the string version, the state of mod keys, if
 * it was a press (else it's a release) */
typedef void (*RGFW_keyfunc)(RGFW_window *win, u8 key, char keyChar,
                             RGFW_keymod keyMod, RGFW_bool pressed);
/*! RGFW_mouseButtonPressed / RGFW_mouseButtonReleased, the window that got the
 * event, the button that was pressed, the scroll value, if it was a press (else
 * it's a release)  */
typedef void (*RGFW_mousebuttonfunc)(RGFW_window *win, RGFW_mouseButton button,
                                     double scroll, RGFW_bool pressed);
/*!gamepad /gamepad, the window that got the event, the button that was pressed,
 * the scroll value, if it was a press (else it's a release) */
typedef void (*RGFW_gamepadButtonfunc)(RGFW_window *win, u16 gamepad, u8 button,
                                       RGFW_bool pressed);
/*! RGFW_gamepadAxisMove, the window that got the event, the gamepad in
 * question, the axis values and the amount of axises */
typedef void (*RGFW_gamepadAxisfunc)(RGFW_window *win, u16 gamepad,
                                     RGFW_point axis[2], u8 axisesCount,
                                     u8 whichAxis);
/*! RGFW_gamepadConnected/RGFW_gamepadDisconnected, the window that got the
 * event, the gamepad in question, if the controller was connected (or
 * disconnected if false) */
typedef void (*RGFW_gamepadfunc)(RGFW_window *win, u16 gamepad,
                                 RGFW_bool connected);

/*!  RGFW_dnd, the window that had the drop, the drop data and the amount files
 * dropped returns previous callback function (if it was set) */
typedef void (*RGFW_dndfunc)(RGFW_window *win,
                             char droppedFiles[RGFW_MAX_DROPS][RGFW_MAX_PATH],
                             u32 droppedFilesCount);

/*! set callback for a window move event returns previous callback function (if
 * it was set)  */
RGFWDEF RGFW_windowmovefunc
RGFW_setWindowMoveCallback(RGFW_windowmovefunc func);
/*! set callback for a window resize event returns previous callback function
 * (if it was set)  */
RGFWDEF RGFW_windowresizefunc
RGFW_setWindowResizeCallback(RGFW_windowresizefunc func);
/*! set callback for a window quit event returns previous callback function (if
 * it was set)  */
RGFWDEF RGFW_windowquitfunc
RGFW_setWindowQuitCallback(RGFW_windowquitfunc func);
/*! set callback for a mouse move event returns previous callback function (if
 * it was set)  */
RGFWDEF RGFW_mouseposfunc RGFW_setMousePosCallback(RGFW_mouseposfunc func);
/*! set callback for a window refresh event returns previous callback function
 * (if it was set)  */
RGFWDEF RGFW_windowrefreshfunc
RGFW_setWindowRefreshCallback(RGFW_windowrefreshfunc func);
/*! set callback for a window focus change event returns previous callback
 * function (if it was set)  */
RGFWDEF RGFW_focusfunc RGFW_setFocusCallback(RGFW_focusfunc func);
/*! set callback for a mouse notify event returns previous callback function (if
 * it was set)  */
RGFWDEF RGFW_mouseNotifyfunc
RGFW_setMouseNotifyCallBack(RGFW_mouseNotifyfunc func);
/*! set callback for a drop event event returns previous callback function (if
 * it was set)  */
RGFWDEF RGFW_dndfunc RGFW_setDndCallback(RGFW_dndfunc func);
/*! set callback for a start of a drop event returns previous callback function
 * (if it was set)  */
RGFWDEF RGFW_dndInitfunc RGFW_setDndInitCallback(RGFW_dndInitfunc func);
/*! set callback for a key (press / release ) event returns previous callback
 * function (if it was set)  */
RGFWDEF RGFW_keyfunc RGFW_setKeyCallback(RGFW_keyfunc func);
/*! set callback for a mouse button (press / release ) event returns previous
 * callback function (if it was set)  */
RGFWDEF RGFW_mousebuttonfunc
RGFW_setMouseButtonCallback(RGFW_mousebuttonfunc func);
/*! set callback for a controller button (press / release ) event returns
 * previous callback function (if it was set)  */
RGFWDEF RGFW_gamepadButtonfunc
RGFW_setgamepadButtonCallback(RGFW_gamepadButtonfunc func);
/*! set callback for a gamepad axis mov event returns previous callback function
 * (if it was set)  */
RGFWDEF RGFW_gamepadAxisfunc
RGFW_setgamepadAxisCallback(RGFW_gamepadAxisfunc func);
...
/*! NOTE! (for X11/linux) : if you define a window in a thread, it must be run
 * after the original thread's window is created or else there will be a memory
 * error */
/*
...
#if defined(__unix__) || defined(__APPLE__) || defined(RGFW_WASM) ||           \
    defined(RGFW_CUSTOM_BACKEND)
typedef void *(*RGFW_threadFunc_ptr)(void *);
#else
typedef DWORD(__stdcall *RGFW_threadFunc_ptr)(LPVOID lpThreadParameter);
#endif

RGFWDEF RGFW_thread RGFW_createThread(RGFW_threadFunc_ptr ptr,
                                      void *args);  /*!< create a thread*/
RGFWDEF void RGFW_cancelThread(RGFW_thread thread); /*!< cancels a thread*/
RGFWDEF void
RGFW_joinThread(RGFW_thread thread); /*!< join thread to current thread */
RGFWDEF void
RGFW_setThreadPriority(RGFW_thread thread,
                       u8 priority); /*!< sets the priority priority  */
#endif
...
typedef RGFW_ENUM(u8, RGFW_gamepadType){
    RGFW_gamepadMicrosoft = 0, RGFW_gamepadSony, RGFW_gamepadNintendo,
    RGFW_gamepadLogitech, RGFW_gamepadUnknown};

/*! gamepad count starts at 0*/
RGFWDEF u32 RGFW_isPressedGamepad(RGFW_window *win, u8 controller,
                                  RGFW_gamepadCodes button);
RGFWDEF u32 RGFW_isReleasedGamepad(RGFW_window *win, u8 controller,
                                   RGFW_gamepadCodes button);
RGFWDEF u32 RGFW_isHeldGamepad(RGFW_window *win, u8 controller,
                               RGFW_gamepadCodes button);
RGFWDEF u32 RGFW_wasPressedGamepad(RGFW_window *win, u8 controller,
                                   RGFW_gamepadCodes button);
RGFWDEF RGFW_point RGFW_getGamepadAxis(RGFW_window *win, u16 controller,
                                       u16 whichAxis);
...
/*< updates fps / sets fps to cap (must by ran manually by the user at the end
 * of a frame), returns current fps */
RGFWDEF u32 RGFW_window_checkFPS(RGFW_window *win, u32 fpsCap);

/* supports openGL, directX, OSMesa, EGL and software rendering */
RGFWDEF void
RGFW_window_swapBuffers(RGFW_window *win); /*!< swap the rendering buffer */
...
/*! OpenGL init hints */
RGFWDEF void RGFW_setGLStencil(
    i32 stencil); /*!< set stencil buffer bit size (8 by default) */
RGFWDEF void RGFW_setGLSamples(
    i32 samples); /*!< set number of sampiling buffers (4 by default) */
RGFWDEF void
RGFW_setGLStereo(i32 stereo); /*!< use GL_STEREO (GL_FALSE by default) */
RGFWDEF void RGFW_setGLAuxBuffers(
...
RGFWDEF void *RGFW_getProcAddress(
    const char *procname); /*!< get native opengl proc address */
RGFWDEF void RGFW_window_makeCurrent_OpenGL(
    RGFW_window *win); /*!< to be called by RGFW_window_makeCurrent */
...
RGFWDEF VkResult RGFW_window_createVKSurface(RGFW_window *win,
                                             VkInstance instance,
...
*/
#undef RGFW_key
typedef RGFW_ENUM(u8, RGFW_key){RGFW_keyNULL = 0,
                                RGFW_escape = '\033',
                                RGFW_backtick = '`',
                                RGFW_0 = '0',
                                RGFW_1 = '1',
...
typedef RGFW_ENUM(u8, RGFW_mouseIcons){
    RGFW_mouseNormal = 0, RGFW_mouseArrow,        RGFW_mouseIbeam,
    RGFW_mouseCrosshair,  RGFW_mousePointingHand, RGFW_mouseResizeEW,
    RGFW_mouseResizeNS,   RGFW_mouseResizeNWSE,   RGFW_mouseResizeNESW,
    RGFW_mouseResizeAll,  RGFW_mouseNotAllowed,
};
...
This is the start of keycode data
        ...
        Linux -> Only symcodes are values, (XK_0 - XK_1, XK_a - XK_z) are larger
than 0xFF00, I can't find any way to work with them without making the array an
unreasonable size MacOS -> windows and linux already don't have keycodes as
macros, so there's no point
*/
...
u8 RGFW_keycodes[RGFW_OS_BASED_VALUE(136, 0x15C + 1, 128,
                                     DOM_VK_WIN_OEM_CLEAR + 1)] = {
#ifdef __cplusplus
    0};
void RGFW_init_keys(void) {
#endif
  RGFW_MAP[RGFW_OS_BASED_VALUE(49, 0x029, 50,
                               DOM_VK_BACK_QUOTE)] = RGFW_backtick RGFW_NEXT

      RGFW_MAP[RGFW_OS_BASED_VALUE(19, 0x00B, 29, DOM_VK_0)] = RGFW_0 RGFW_NEXT
          RGFW_MAP[RGFW_OS_BASED_VALUE(10, 0x002, 18, DOM_VK_1)] = RGFW_1
              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(11, 0x003, 19, DOM_VK_2)] =
                  RGFW_2 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                      12, 0x004, 20, DOM_VK_3)] = RGFW_3 RGFW_NEXT
                      RGFW_MAP[RGFW_OS_BASED_VALUE(13, 0x005, 21, DOM_VK_4)] =
                          RGFW_4 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                              14, 0x006, 23, DOM_VK_5)] = RGFW_5 RGFW_NEXT
                              RGFW_MAP[RGFW_OS_BASED_VALUE(15, 0x007, 22,
                                                           DOM_VK_6)] =
                                  RGFW_6 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                      16, 0x008, 26, DOM_VK_7)] = RGFW_7
                                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                          17, 0x009, 28, DOM_VK_8)] =
                                          RGFW_8 RGFW_NEXT
                                              RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                  18, 0x00A, 25, DOM_VK_9)] =
                                                  RGFW_9,
  RGFW_MAP[RGFW_OS_BASED_VALUE(65, 0x039, 49, DOM_VK_SPACE)] = RGFW_space,
  RGFW_MAP[RGFW_OS_BASED_VALUE(
      38, 0x01E, 0,
      DOM_VK_A)] = RGFW_a RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(56, 0x030, 11,
                                                                 DOM_VK_B)] =
      RGFW_b RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
          54, 0x02E, 8, DOM_VK_C)] = RGFW_c RGFW_NEXT RGFW_MAP
          [RGFW_OS_BASED_VALUE(40, 0x020, 2, DOM_VK_D)] = RGFW_d RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
              26,
              0x012, 14, DOM_VK_E)] = RGFW_e
              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(41, 0x021, 3, DOM_VK_F)] = RGFW_f RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                  42,
                  0x022, 5, DOM_VK_G)] = RGFW_g RGFW_NEXT RGFW_MAP
                  [RGFW_OS_BASED_VALUE(43, 0x023, 4, DOM_VK_H)] = RGFW_h RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                      31,
                      0x017, 34, DOM_VK_I)] =
                      RGFW_i RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                          44,
                          0x024, 38, DOM_VK_J)] = RGFW_j RGFW_NEXT RGFW_MAP
                          [RGFW_OS_BASED_VALUE(45, 0x025, 40, DOM_VK_K)] = RGFW_k RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                              46,
                              0x026, 37, DOM_VK_L)] = RGFW_l RGFW_NEXT RGFW_MAP
                              [RGFW_OS_BASED_VALUE(58, 0x032, 46, DOM_VK_M)] = RGFW_m RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                  57,
                                  0x031, 45,
                                  DOM_VK_N)] = RGFW_n RGFW_NEXT RGFW_MAP
                                  [RGFW_OS_BASED_VALUE(32, 0x018, 31, DOM_VK_O)] = RGFW_o RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                      33,
                                      0x019, 35, DOM_VK_P)] = RGFW_p
                                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                          24,
                                          0x010, 12,
                                          DOM_VK_Q)] = RGFW_q RGFW_NEXT RGFW_MAP
                                          [RGFW_OS_BASED_VALUE(
                                              27,
                                              0x013, 15,
                                              DOM_VK_R)] = RGFW_r RGFW_NEXT RGFW_MAP
                                              [RGFW_OS_BASED_VALUE(
                                                  39,
                                                  0x01F, 1,
                                                  DOM_VK_S)] = RGFW_s RGFW_NEXT RGFW_MAP
                                                  [RGFW_OS_BASED_VALUE(
                                                      28,
                                                      0x014, 17,
                                                      DOM_VK_T)] = RGFW_t RGFW_NEXT RGFW_MAP
                                                      [RGFW_OS_BASED_VALUE(
                                                          30,
                                                          0x016, 32,
                                                          DOM_VK_U)] = RGFW_u RGFW_NEXT RGFW_MAP
                                                          [RGFW_OS_BASED_VALUE(
                                                              55,
                                                              0x02F, 9,
                                                              DOM_VK_V)] = RGFW_v RGFW_NEXT RGFW_MAP
                                                              [RGFW_OS_BASED_VALUE(
                                                                  25,
                                                                  0x011, 13,
                                                                  DOM_VK_W)] = RGFW_w RGFW_NEXT RGFW_MAP
                                                                  [RGFW_OS_BASED_VALUE(
                                                                      53,
                                                                      0x02D, 7,
                                                                      DOM_VK_X)] = RGFW_x RGFW_NEXT RGFW_MAP
                                                                      [RGFW_OS_BASED_VALUE(
                                                                          29,
                                                                          0x015,
                                                                          16,
                                                                          DOM_VK_Y)] = RGFW_y RGFW_NEXT RGFW_MAP
                                                                          [RGFW_OS_BASED_VALUE(
                                                                              52,
                                                                              0x02C,
                                                                              6,
                                                                              DOM_VK_Z)] =
                                                                              RGFW_z,
  RGFW_MAP[RGFW_OS_BASED_VALUE(60, 0x034, 47, DOM_VK_PERIOD)] = RGFW_period
      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(59, 0x033, 43, DOM_VK_COMMA)] =
          RGFW_comma RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(61, 0x035, 44,
                                                            DOM_VK_SLASH)] =
              RGFW_slash RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                  34, 0x01A, 33, DOM_VK_OPEN_BRACKET)] = RGFW_bracket RGFW_NEXT
                  RGFW_MAP[RGFW_OS_BASED_VALUE(35, 0x01B, 30,
                                               DOM_VK_CLOSE_BRACKET)] =
                      RGFW_closeBracket RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                          47, 0x027, 41, DOM_VK_SEMICOLON)] =
                          RGFW_semicolon RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                              48, 0x028, 39, DOM_VK_QUOTE)] = RGFW_apostrophe
                              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                  51, 0x02B, 42, DOM_VK_BACK_SLASH)] =
                                  RGFW_backSlash,
  RGFW_MAP[RGFW_OS_BASED_VALUE(
      36, 0x01C, 36,
      DOM_VK_RETURN)] = RGFW_return RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(119,
                                                                           0x153,
                                                                           118,
                                                                           DOM_VK_DELETE)] = RGFW_delete
      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(77, 0x145, 72, DOM_VK_NUM_LOCK)] =
          RGFW_numLock RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
              106,
              0x135, 82, DOM_VK_DIVIDE)] = RGFW_KP_Slash RGFW_NEXT RGFW_MAP
              [RGFW_OS_BASED_VALUE(63, 0x037, 76,
                                   DOM_VK_MULTIPLY)] = RGFW_multiply
                  RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                      82,
                      0x04A, 67, DOM_VK_SUBTRACT)] = RGFW_KP_Minus
                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                          87,
                          0x04F, 84, DOM_VK_NUMPAD1)] = RGFW_KP_1
                          RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                              88,
                              0x050, 85, DOM_VK_NUMPAD2)] = RGFW_KP_2
                              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                  89,
                                  0x051, 86, DOM_VK_NUMPAD3)] = RGFW_KP_3
                                  RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                      83,
                                      0x04B, 87, DOM_VK_NUMPAD4)] = RGFW_KP_4
                                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                          84,
                                          0x04C, 88,
                                          DOM_VK_NUMPAD5)] = RGFW_KP_5
                                          RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                              85,
                                              0x04D, 89,
                                              DOM_VK_NUMPAD6)] = RGFW_KP_6
                                              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                  79,
                                                  0x047, 90,
                                                  DOM_VK_NUMPAD7)] = RGFW_KP_7
                                                  RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                      80,
                                                      0x048, 92,
                                                      DOM_VK_NUMPAD8)] = RGFW_KP_8
                                                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                          81,
                                                          0x049, 93,
                                                          DOM_VK_NUMPAD9)] = RGFW_KP_9
                                                          RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                              90,
                                                              0x052, 83,
                                                              DOM_VK_NUMPAD0)] = RGFW_KP_0
                                                              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                                  91,
                                                                  0x053, 65,
                                                                  DOM_VK_DECIMAL)] = RGFW_KP_Period
                                                                  RGFW_NEXT RGFW_MAP
                                                                      [RGFW_OS_BASED_VALUE(
                                                                          104,
                                                                          0x11C,
                                                                          ...
                                                                          0)] =
                                                                          RGFW_KP_Return,
  RGFW_MAP[RGFW_OS_BASED_VALUE(20, 0x00C, 27, DOM_VK_HYPHEN_MINUS)] = RGFW_minus
      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(21, 0x00D, 24, DOM_VK_EQUALS)] =
          RGFW_equals RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
              22, 0x00E, 51, DOM_VK_BACK_SPACE)] = RGFW_backSpace RGFW_NEXT
              RGFW_MAP[RGFW_OS_BASED_VALUE(23, 0x00F, 48, DOM_VK_TAB)] =
                  RGFW_tab RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                      66, 0x03A, 57, DOM_VK_CAPS_LOCK)] =
                      RGFW_capsLock RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                          50, 0x02A, 56, DOM_VK_SHIFT)] = RGFW_shiftL RGFW_NEXT
                          RGFW_MAP[RGFW_OS_BASED_VALUE(37, 0x01D, 59,
                                                       DOM_VK_CONTROL)] =
                              RGFW_controlL RGFW_NEXT
                                  RGFW_MAP[RGFW_OS_BASED_VALUE(64, 0x038, 58,
                                                               DOM_VK_ALT)] =
                                      RGFW_altL RGFW_NEXT
                                          RGFW_MAP[RGFW_OS_BASED_VALUE(
                                              133, 0x15B, 55, DOM_VK_WIN)] =
                                              RGFW_superL,
...
#endif
  RGFW_MAP[RGFW_OS_BASED_VALUE(67, 0x03B, 127, DOM_VK_F1)] = RGFW_F1 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
      68,
      0x03C, 121,
      DOM_VK_F2)] = RGFW_F2 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(69, 0x03D,
                                                                   100,
                                                                   DOM_VK_F3)] = RGFW_F3 RGFW_NEXT RGFW_MAP
      [RGFW_OS_BASED_VALUE(70, 0x03E, 119, DOM_VK_F4)] = RGFW_F4 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
          71,
          0x03F, 97, DOM_VK_F5)] = RGFW_F5 RGFW_NEXT RGFW_MAP
          [RGFW_OS_BASED_VALUE(72, 0x040, 98, DOM_VK_F6)] = RGFW_F6 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
              73,
              0x041, 99, DOM_VK_F7)] = RGFW_F7 RGFW_NEXT RGFW_MAP
              [RGFW_OS_BASED_VALUE(74, 0x042, 101, DOM_VK_F8)] = RGFW_F8 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                  75,
                  0x043, 102,
                  DOM_VK_F9)] = RGFW_F9 RGFW_NEXT RGFW_MAP
                  [RGFW_OS_BASED_VALUE(76, 0x044, 110, DOM_VK_F10)] = RGFW_F10 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                      95,
                      0x057, 104,
                      DOM_VK_F11)] = RGFW_F11
                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(96, 0x058, 112,
                                                             DOM_VK_F12)] =
                          RGFW_F12 RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                              111, 0x148,
                              126, DOM_VK_UP)] = RGFW_up
                              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                  116, 0x150,
                                  125, DOM_VK_DOWN)] = RGFW_down
                                  RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                      113, 0x14B,
                                      123, DOM_VK_LEFT)] = RGFW_left
                                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                          114, 0x14D,
                                          124, DOM_VK_RIGHT)] = RGFW_right
                                          RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                              118, 0x152,
                                              115, DOM_VK_INSERT)] = RGFW_insert
                                              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                  115, 0x14F,
                                                  120,
                                                  DOM_VK_END)] = RGFW_end RGFW_NEXT RGFW_MAP
                                                  [RGFW_OS_BASED_VALUE(
                                                      112, 0x149,
                                                      117,
                                                      DOM_VK_PAGE_UP)] = RGFW_pageUp
                                                      RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                          117, 0x151,
                                                          122,
                                                          DOM_VK_PAGE_DOWN)] = RGFW_pageDown
                                                          RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                              9, 0x001, 53,
                                                              DOM_VK_ESCAPE)] = RGFW_escape
                                                              RGFW_NEXT RGFW_MAP[RGFW_OS_BASED_VALUE(
                                                                  110, 0x147,
                                                                  116,
                                                                  DOM_VK_HOME)] = RGFW_home
                                                                  RGFW_NEXT RGFW_MAP
                                                                      [RGFW_OS_BASED_VALUE(
                                                                          78,
                                                                          0x046,
...
#ifdef __cplusplus
  if (RGFW_OS_BASED_VALUE(49, 192, 50, DOM_VK_BACK_QUOTE, KEY_GRAVE) !=
      RGFW_backtick) {
    RGFW_init_keys();
  }
#endif
...
/* gamepad data */
RGFW_keyState RGFW_gamepadPressed[4][18]; /*!< if a key is currently pressed or
                                             not (per gamepad) */
RGFW_point RGFW_gamepadAxes[4][4]; /*!< if a key is currently pressed or not
                                      (per gamepad) */

RGFW_gamepadType RGFW_gamepads_type[4]; /*!< if a key is currently pressed or
                                           not (per gamepad) */
...
*/
void RGFW_windowmovefuncEMPTY(RGFW_window *win, RGFW_rect r) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(r);
}
void RGFW_windowresizefuncEMPTY(RGFW_window *win, RGFW_rect r) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(r);
}
...
void RGFW_mouseNotifyfuncEMPTY(RGFW_window *win, RGFW_point point,
                               RGFW_bool status) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(point);
  RGFW_UNUSED(status);
}
void RGFW_mouseposfuncEMPTY(RGFW_window *win, RGFW_point point,
                            RGFW_point vector) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(point);
  RGFW_UNUSED(vector);
...
void RGFW_keyfuncEMPTY(RGFW_window *win, RGFW_key key, char keyChar,
                       RGFW_keymod keyMod, RGFW_bool pressed) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(key);
  RGFW_UNUSED(keyChar);
...
void RGFW_mousebuttonfuncEMPTY(RGFW_window *win, RGFW_mouseButton button,
                               double scroll, RGFW_bool pressed) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(button);
  RGFW_UNUSED(scroll);
...
void RGFW_gamepadButtonfuncEMPTY(RGFW_window *win, u16 gamepad, u8 button,
                                 RGFW_bool pressed) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(gamepad);
  RGFW_UNUSED(button);
...
void RGFW_gamepadAxisfuncEMPTY(RGFW_window *win, u16 gamepad,
                               RGFW_point axis[2], u8 axisesCount,
                               u8 whichAxis) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(gamepad);
  RGFW_UNUSED(axis);
  RGFW_UNUSED(axisesCount);
  RGFW_UNUSED(whichAxis);
}
void RGFW_gamepadfuncEMPTY(RGFW_window *win, u16 gamepad, RGFW_bool connected) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(gamepad);
  RGFW_UNUSED(connected);
}
void RGFW_dndfuncEMPTY(RGFW_window *win,
                       char droppedFiles[RGFW_MAX_DROPS][RGFW_MAX_PATH],
                       u32 droppedFilesCount) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(droppedFiles);
  RGFW_UNUSED(droppedFilesCount);
}
...
RGFW_windowmovefunc RGFW_setWindowMoveCallback(RGFW_windowmovefunc func) {
  RGFW_windowmovefunc prev =
      (RGFW_windowMoveCallback == RGFW_windowmovefuncEMPTY)
          ? NULL
          : RGFW_windowMoveCallback;
  RGFW_windowMoveCallback = func;
  return prev;
}
RGFW_windowresizefunc RGFW_setWindowResizeCallback(RGFW_windowresizefunc func) {
  RGFW_windowresizefunc prev =
      (RGFW_windowResizeCallback == RGFW_windowresizefuncEMPTY)
          ? NULL
          : RGFW_windowResizeCallback;
  RGFW_windowResizeCallback = func;
  return prev;
}
RGFW_windowquitfunc RGFW_setWindowQuitCallback(RGFW_windowquitfunc func) {
  RGFW_windowquitfunc prev =
      (RGFW_windowQuitCallback == RGFW_windowquitfuncEMPTY)
          ? NULL
...
RGFW_mouseposfunc RGFW_setMousePosCallback(RGFW_mouseposfunc func) {
  RGFW_mouseposfunc prev = (RGFW_mousePosCallback == RGFW_mouseposfuncEMPTY)
                               ? NULL
                               : RGFW_mousePosCallback;
  RGFW_mousePosCallback = func;
  return prev;
}
RGFW_windowrefreshfunc
RGFW_setWindowRefreshCallback(RGFW_windowrefreshfunc func) {
  RGFW_windowrefreshfunc prev =
      (RGFW_windowRefreshCallback == RGFW_windowrefreshfuncEMPTY)
          ? NULL
          : RGFW_windowRefreshCallback;
  RGFW_windowRefreshCallback = func;
  return prev;
}
RGFW_focusfunc RGFW_setFocusCallback(RGFW_focusfunc func) {
  RGFW_focusfunc prev =
      (RGFW_focusCallback == RGFW_focusfuncEMPTY) ? NULL : RGFW_focusCallback;
  RGFW_focusCallback = func;
  return prev;
}

RGFW_mouseNotifyfunc RGFW_setMouseNotifyCallBack(RGFW_mouseNotifyfunc func) {
  RGFW_mouseNotifyfunc prev =
      (RGFW_mouseNotifyCallBack == RGFW_mouseNotifyfuncEMPTY)
          ? NULL
          : RGFW_mouseNotifyCallBack;
  RGFW_mouseNotifyCallBack = func;
  return prev;
}
RGFW_dndfunc RGFW_setDndCallback(RGFW_dndfunc func) {
  RGFW_dndfunc prev =
      (RGFW_dndCallback == RGFW_dndfuncEMPTY) ? NULL : RGFW_dndCallback;
  RGFW_dndCallback = func;
  return prev;
}
RGFW_dndInitfunc RGFW_setDndInitCallback(RGFW_dndInitfunc func) {
  RGFW_dndInitfunc prev = (RGFW_dndInitCallback == RGFW_dndInitfuncEMPTY)
                              ? NULL
                              : RGFW_dndInitCallback;
  RGFW_dndInitCallback = func;
  return prev;
}
RGFW_keyfunc RGFW_setKeyCallback(RGFW_keyfunc func) {
  RGFW_keyfunc prev =
      (RGFW_keyCallback == RGFW_keyfuncEMPTY) ? NULL : RGFW_keyCallback;
  RGFW_keyCallback = func;
  return prev;
}
RGFW_mousebuttonfunc RGFW_setMouseButtonCallback(RGFW_mousebuttonfunc func) {
  RGFW_mousebuttonfunc prev =
      (RGFW_mouseButtonCallback == RGFW_mousebuttonfuncEMPTY)
          ? NULL
          : RGFW_mouseButtonCallback;
  RGFW_mouseButtonCallback = func;
  return prev;
}
RGFW_gamepadButtonfunc
RGFW_setgamepadButtonCallback(RGFW_gamepadButtonfunc func) {
  RGFW_gamepadButtonfunc prev =
      (RGFW_gamepadButtonCallback == RGFW_gamepadButtonfuncEMPTY)
          ? NULL
          : RGFW_gamepadButtonCallback;
  RGFW_gamepadButtonCallback = func;
  return prev;
}
RGFW_gamepadAxisfunc RGFW_setgamepadAxisCallback(RGFW_gamepadAxisfunc func) {
  RGFW_gamepadAxisfunc prev =
      (RGFW_gamepadAxisCallback == RGFW_gamepadAxisfuncEMPTY)
          ? NULL
          : RGFW_gamepadAxisCallback;
  RGFW_gamepadAxisCallback = func;
  return prev;
}
RGFW_gamepadfunc RGFW_setGamepadCallback(RGFW_gamepadfunc func) {
  RGFW_gamepadfunc prev = (RGFW_gamepadCallback == RGFW_gamepadfuncEMPTY)
                              ? NULL
...
#define RGFW_NO_GPU_RENDER                                                     \
  RGFW_BIT(25) /* don't render (using the GPU based API)*/
#define RGFW_NO_CPU_RENDER                                                     \
  RGFW_BIT(26) /* don't render (using the CPU based buffer rendering)*/
...
#define RGFW_BUFFER_ALLOC                                                      \
  RGFW_BIT(30) /* if window.buffer was allocated by RGFW */

RGFW_window *RGFW_createWindow(const char *name, RGFW_rect rect,
...
RGFWDEF void RGFW_window_basic_init(RGFW_window *win, RGFW_rect rect,
                                    RGFW_windowFlags flags);
 ...
 */
void RGFW_window_basic_init(RGFW_window *win, RGFW_rect rect,
                            RGFW_windowFlags flags) {
  RGFW_UNUSED(flags);
...
void RGFW_window_basic_flags(RGFW_window *win, RGFW_windowFlags flags) {
  ...
  if (flags & RGFW_windowCenterCursor)
    RGFW_window_moveMouse(
        win, RGFW_POINT(win->r.x + (win->r.w / 2), win->r.y + (win->r.h / 2)));
...
RGFW_bool RGFW_isMousePressed(RGFW_window *win, RGFW_mouseButton button) {
  return RGFW_mouseButtons[button].current &&
         (win == NULL || win->event.inFocus);
}
...
RGFW_bool RGFW_isMouseHeld(RGFW_window *win, RGFW_mouseButton button) {
  return (RGFW_isMousePressed(win, button) &&
          RGFW_wasMousePressed(win, button));
}
...
void RGFW_window_center(RGFW_window *win) {
  ...
  RGFW_window_move(
      win, RGFW_POINT((screenR.w - win->r.w) / 2, (screenR.h - win->r.h) / 2));
}
...
void RGFW_window_scaleToMonitor(RGFW_window *win) {
  ...
  RGFW_window_resize(win, RGFW_AREA((u32)(monitor.scaleX * (float)win->r.w),
                                    (u32)(monitor.scaleY * (float)win->r.h)));
}
...
u32 RGFW_isReleasedGamepad(RGFW_window *win, u8 controller,
                           RGFW_gamepadCodes button) {
  RGFW_UNUSED(win);
  return !RGFW_isPressedGamepad(win, controller, button) &&
         RGFW_wasPressedGamepad(win, controller, button);
}
u32 RGFW_isHeldGamepad(RGFW_window *win, u8 controller,
                       RGFW_gamepadCodes button) {
  RGFW_UNUSED(win);
  return RGFW_isPressedGamepad(win, controller, button) &&
         RGFW_wasPressedGamepad(win, controller, button);
...
RGFW_point RGFW_getGamepadAxis(RGFW_window *win, u16 controller,
                               u16 whichAxis) {
  RGFW_UNUSED(win);
...
RGFWDEF void RGFW_updateKeyMod(RGFW_window *win, RGFW_keymod mod,
                               RGFW_bool value);
void RGFW_updateKeyMod(RGFW_window *win, RGFW_keymod mod, RGFW_bool value) {
  RGFW_setBit((u32 *)&win->event.keyMod, mod, value);
}

RGFWDEF void RGFW_updateKeyModsPro(RGFW_window *win, RGFW_bool capital,
                                   RGFW_bool numlock, RGFW_bool control,
                                   RGFW_bool alt, RGFW_bool shift,
                                   RGFW_bool super, RGFW_bool scroll);
void RGFW_updateKeyModsPro(RGFW_window *win, RGFW_bool capital,
                           RGFW_bool numlock, RGFW_bool control, RGFW_bool alt,
                           RGFW_bool shift, RGFW_bool super, RGFW_bool scroll) {
...
RGFWDEF void RGFW_updateKeyMods(RGFW_window *win, RGFW_bool capital,
                                RGFW_bool numlock, RGFW_bool scroll);
void RGFW_updateKeyMods(RGFW_window *win, RGFW_bool capital, RGFW_bool numlock,
                        RGFW_bool scroll) {
  RGFW_updateKeyModsPro(
      win, capital, numlock,
...
RGFW_bool RGFW_window_isFullscreen(RGFW_window *win) {
  return RGFW_BOOL(win->_flags & RGFW_windowFullscreen);
}
RGFW_bool RGFW_window_allowsDND(RGFW_window *win) {
  return RGFW_BOOL(win->_flags & RGFW_windowAllowDND);
}
#ifndef RGFW_WINDOWS
...
#ifndef RGFW_EGL
i32 RGFW_STENCIL = 8, RGFW_SAMPLES = 4, RGFW_STEREO = 0, RGFW_AUX_BUFFERS = 0,
    RGFW_DOUBLE_BUFFER = 1;
#else
i32 RGFW_STENCIL = 0, RGFW_SAMPLES = 0, RGFW_STEREO = 0, RGFW_AUX_BUFFERS = 0,
    RGFW_DOUBLE_BUFFER = 1;
#endif
...
void RGFW_setDoubleBuffer(RGFW_bool useDoubleBuffer) {
  RGFW_DOUBLE_BUFFER = useDoubleBuffer;
}
...
#define RGFW_GL_RENDER_TYPE                                                    \
  RGFW_OS_BASED_VALUE(GLX_X_VISUAL_TYPE, 0x2003, 73, 0)
...
#define RGFW_GL_STENCIL_SIZE                                                   \
  RGFW_OS_BASED_VALUE(GLX_STENCIL_SIZE, 0x2023, 13, 0)
...
/*  The window'ing api needs to know how to render the data we (or opengl) give
   it MacOS and Windows do this using a structure called a "pixel format" X11
   calls it a "Visual" This function returns the attributes for the format we
   want */
...
#ifdef RGFW_MACOS
  /* macOS has the surface attribs and the opengl attribs connected for some
     reason maybe this is to give macOS more control to limit openGL/the opengl
     version? */
...
void RGFW_createOpenGLContext(RGFW_window *win) {
#if defined(RGFW_LINK_EGL)
  eglInitializeSource =
      (PFNEGLINITIALIZEPROC)eglGetProcAddress("eglInitialize");
  eglGetConfigsSource =
      (PFNEGLGETCONFIGSPROC)eglGetProcAddress("eglGetConfigs");
  eglChooseConfigSource =
      (PFNEGLCHOOSECONFIgamepadROC)eglGetProcAddress("eglChooseConfig");
  eglCreateWindowSurfaceSource =
      (PFNEGLCREATEWINDOWSURFACEPROC)eglGetProcAddress(
          "eglCreateWindowSurface");
  eglCreateContextSource =
      (PFNEGLCREATECONTEXTPROC)eglGetProcAddress("eglCreateContext");
  eglMakeCurrentSource =
      (PFNEGLMAKECURRENTPROC)eglGetProcAddress("eglMakeCurrent");
  eglGetDisplaySource =
      (PFNEGLGETDISPLAYPROC)eglGetProcAddress("eglGetDisplay");
  eglSwapBuffersSource =
      (PFNEGLSWAPBUFFERSPROC)eglGetProcAddress("eglSwapBuffers");
  eglSwapIntervalSource =
      (PFNEGLSWAPINTERVALPROC)eglGetProcAddress("eglSwapInterval");
  eglBindAPISource = (PFNEGLBINDAPIPROC)eglGetProcAddress("eglBindAPI");
  eglDestroyContextSource =
      (PFNEGLDESTROYCONTEXTPROC)eglGetProcAddress("eglDestroyContext");
  eglTerminateSource = (PFNEGLTERMINATEPROC)eglGetProcAddress("eglTerminate");
  eglDestroySurfaceSource =
      (PFNEGLDESTROYSURFACEPROC)eglGetProcAddress("eglDestroySurface");
#endif /* RGFW_LINK_EGL */
  ...
  if (RGFW_useWaylandBool)
    win->src.EGL_display =
        eglGetDisplay((EGLNativeDisplayType)win->src.wl_display);
  else
    win->src.EGL_display =
        eglGetDisplay((EGLNativeDisplayType)win->src.display);
#else
...
#define EGL_OPENGL_ES1_BIT 0x1
  ...
  win->src.EGL_surface = eglCreateWindowSurface(
      win->src.EGL_display, config, (EGLNativeWindowType)layer, NULL);
#elif defined(RGFW_WINDOWS)
  win->src.EGL_surface = eglCreateWindowSurface(
      win->src.EGL_display, config, (EGLNativeWindowType)win->src.window, NULL);
#elif defined(RGFW_WAYLAND)
  if (RGFW_useWaylandBool)
    win->src.EGL_surface =
        eglCreateWindowSurface(win->src.EGL_display, config,
                               (EGLNativeWindowType)win->src.eglWindow, NULL);
  else
    win->src.EGL_surface =
        eglCreateWindowSurface(win->src.EGL_display, config,
                               (EGLNativeWindowType)win->src.window, NULL);
#else
  win->src.EGL_surface = eglCreateWindowSurface(
      win->src.EGL_display, config, (EGLNativeWindowType)win->src.window, NULL);
#endif
  ...
  if (RGFW_majorVersion) {
    ...
    if (RGFW_profile == RGFW_glCore) {
      RGFW_GL_ADD_ATTRIB(EGL_CONTEXT_OPENGL_PROFILE_MASK,
  ...
  win->src.EGL_context =
      eglCreateContext(win->src.EGL_display, config, EGL_NO_CONTEXT, attribs);
  ...
  eglMakeCurrent(win->src.EGL_display, win->src.EGL_surface,
                 win->src.EGL_surface, win->src.EGL_context);
...
VkResult RGFW_window_createVKSurface(RGFW_window *win, VkInstance instance,
                                     VkSurfaceKHR *surface) {
  assert(win != NULL);
  assert(instance);
  assert(surface != NULL);
  ...
  *surface = VK_NULL_HANDLE;
  ...
  VkXlibSurfaceCreateInfoKHR x11 = {
      VK_STRUCTURE_TYPE_XLIB_SURFACE_CREATE_INFO_KHR, 0, 0,
      (Display *)win->src.display, (Window)win->src.window};
  ...
  VkWin32SurfaceCreateInfoKHR win32 = {
      ...
      GetModuleHandle(NULL), (HWND)win->src.window};

  return vkCreateWin32SurfaceKHR(instance, &win32, NULL, surface);
#elif defined(RGFW_MACOS) && !defined(RGFW_MACOS_X11)
  VkMacOSSurfaceCreateFlagsMVK macos = {
      VK_STRUCTURE_TYPE_MACOS_SURFACE_CREATE_INFO_MVK, 0, 0, vulkWin->display,
      (void *)win->src.window};
...
u32 RGFW_linux_updateGamepad(RGFW_window *win) {
  ...
  static const char *str[] = {"/dev/input/js0", "/dev/input/js1",
                              "/dev/input/js2", "/dev/input/js3",
                              "/dev/input/js4", "/dev/input/js5"};
  ...
  for (size_t i = 0; i < 6; i++) {
    ...
    int axes, buttons;
    if (ioctl(js, JSIOCGAXES, &axes) < 0 ||
        ioctl(js, JSIOCGBUTTONS, &buttons) < 0) {
      close(js);
      continue;
    }
    ...
    ioctl(js, JSIOCGNAME(sizeof(RGFW_gamepads_name[index])),
          RGFW_gamepads_name[index]);
    ...
    if (strstr(RGFW_gamepads_name[index], "Microsoft") ||
        strstr(RGFW_gamepads_name[index], "X-Box"))
      RGFW_gamepads_type[index] = RGFW_gamepadMicrosoft;
    else if (strstr(RGFW_gamepads_name[index], "PlayStation") ||
             strstr(RGFW_gamepads_name[index], "PS3") ||
             strstr(RGFW_gamepads_name[index], "PS4") ||
             strstr(RGFW_gamepads_name[index], "PS5"))
    ...
    while ((bytes = read(RGFW_gamepads[i], &e, sizeof(e))) > 0) {
      ...
      case JS_EVENT_BUTTON: {
        ...
        if (RGFW_gamepads_type[i] == RGFW_gamepadMicrosoft)
          typeIndex = 1;
        else if (RGFW_gamepads_type[i] == RGFW_gamepadLogitech)
          typeIndex = 2;

        win->event.type =
            e.value ? RGFW_gamepadButtonPressed : RGFW_gamepadButtonReleased;
        u8 RGFW_linux2RGFW[3][RGFW_gamepadR3 + 8] = {
            {
                /* ps */
                RGFW_gamepadA,
                RGFW_gamepadB,
            ...
            {/* xbox */
             ...
             RGFW_gamepadL1, RGFW_gamepadR1, RGFW_gamepadSelect,
             RGFW_gamepadStart, RGFW_gamepadHome, RGFW_gamepadL3,
             RGFW_gamepadR3, 255, 255, RGFW_gamepadUp, RGFW_gamepadDown,
             RGFW_gamepadLeft, RGFW_gamepadRight},
            {/* Logitech */
             RGFW_gamepadA, RGFW_gamepadB, RGFW_gamepadX, RGFW_gamepadY,
             RGFW_gamepadL1, RGFW_gamepadR1, RGFW_gamepadL2, RGFW_gamepadR2,
             RGFW_gamepadSelect, RGFW_gamepadStart, RGFW_gamepadHome,
             RGFW_gamepadL3, RGFW_gamepadR3, RGFW_gamepadUp, RGFW_gamepadDown,
             RGFW_gamepadLeft, RGFW_gamepadRight}};

        win->event.button = RGFW_linux2RGFW[typeIndex][e.number];
        win->event.gamepad = i;
        if (win->event.button == 255)
          break;

        RGFW_gamepadPressed[i][win->event.button].prev =
            RGFW_gamepadPressed[i][win->event.button].current;
      ...
      case JS_EVENT_AXIS: {
        ...
        RGFW_gamepadAxisCallback(win, i, win->event.axis,
                                 win->event.axisesCount, win->event.whichAxis);
        return 1;
      }
      default:
        break;
...
- fix RGFW_keyPressed lock state
        ...
        RGFW_windowResized  	the window was resized (by the user), [on WASM
this means the browser was resized] RGFW_windowRefresh	 	The window
content needs to be refreshed

        RGFW_DND 				a file has been dropped into the
window RGFW_DNDInit

- window args:
        #define RGFW_windowNoResize	 			the window
cannot be resized  by the user #define RGFW_windowAllowDND
...
void RGFW_eventPipe_push(RGFW_window *win, RGFW_event event) {
  ...
  if (win->src.eventLen >=
      (i32)(sizeof(win->src.events) / sizeof(win->src.events[0])))
    return;
...
static void pointer_enter(void *data, struct wl_pointer *pointer,
  ...
  RGFW_UNUSED(pointer);
  RGFW_UNUSED(serial);
  RGFW_UNUSED(surface_x);
  RGFW_UNUSED(surface_y);
...
static void pointer_motion(void *data, struct wl_pointer *pointer,
  ...
  RGFW_mousePosCallback(
      RGFW_mouse_win, RGFW_POINT(wl_fixed_to_double(x), wl_fixed_to_double(y)),
      ev.vector);
}
...
static void pointer_axis(void *data, struct wl_pointer *pointer, uint32_t time,
  ...
  RGFW_mouseButtonCallback(RGFW_mouse_win, RGFW_mouseScrollUp + (scroll < 0),
                           scroll, 1);
}
...
static struct wl_pointer_listener pointer_listener =
    (struct wl_pointer_listener){
        ...
        &pointer_axis,           (void *)&RGFW_doNothing,
        (void *)&RGFW_doNothing, (void *)&RGFW_doNothing,
        (void *)&RGFW_doNothing, (void *)&RGFW_doNothing,
        (void *)&RGFW_doNothing};
...
static void keyboard_key(void *data, struct wl_keyboard *keyboard,
  ...
  RGFW_updateKeyMods(RGFW_key_win, xkb_keymap_mod_get_index(keymap, "Lock"),
                     xkb_keymap_mod_get_index(keymap, "Mod2"), ,
                     xkb_keymap_mod_get_index(keymap, "ScrollLock"));

  RGFW_keyCallback(RGFW_key_win, RGFW_key, (u8)keysym,
                   RGFW_key_win->event.keyMod, state);
}
...
static struct wl_keyboard_listener keyboard_listener = {
    ...
    &keyboard_key,    &keyboard_modifiers, (void *)&RGFW_doNothing};
...
static void wl_global_registry_handler(void *data, struct wl_registry *registry,
  ...
  if (RGFW_STRNCMP(interface, "wl_compositor", 16) == 0) {
  ...
  } else if (RGFW_STRNCMP(interface, zxdg_decoration_manager_v1_interface.name,
                          255) == 0) {
    ...
    win->src.shm = wl_registry_bind(registry, id, &wl_shm_interface, 1);
...
static void wl_global_registry_remove(void *data, struct wl_registry *registry,
                                      uint32_t name) {
  RGFW_UNUSED(data);
  RGFW_UNUSED(registry);
  RGFW_UNUSED(name);
}
...
#define RGFW_GOTO_WAYLAND(fallback)                                            \
  if (RGFW_useWaylandBool && fallback == 0)                                    \
  goto wayland
#else
#define RGFW_GOTO_WAYLAND(fallback)
...
Atom XdndAware, XdndTypeList, XdndSelection, XdndEnter, XdndPosition,
    XdndStatus, XdndLeave, XdndDrop, XdndFinished, XdndActionCopy, XtextPlain,
    XtextUriList;
...
#if defined(RGFW_OPENGL) && !defined(RGFW_EGL)
void *RGFW_getProcAddress(const char *procname) {
  return (void *)glXGetProcAddress((GLubyte *)procname);
}
#endif
...
void RGFW_window_initBufferPtr(RGFW_window *win, u8 *buffer, RGFW_area area) {
  ...
  OSMesaMakeCurrent(win->src.ctx, win->buffer, GL_UNSIGNED_BYTE, area.w,
                    area.h);
#endif

  win->src.bitmap = XCreateImage(
      win->src.display,
      XDefaultVisual(win->src.display, XDefaultScreen(win->src.display)), 32,
...
wayland:
  ...
  if (fd < 0) {
    ...
    win->src.wl_buffer =
        wl_shm_pool_create_buffer(pool, 0, win->r.w, win->r.h, win->r.w * 4,
...
#define RGFW_LOAD_ATOM(name)                                                   \
  static Atom name = 0;                                                        \
  if (name == 0)                                                               \
    name = XInternAtom(RGFW_root->src.display, #name, False);
  ...
  void RGFW_window_setBorder(RGFW_window * win, RGFW_bool border) {
    ...
    XChangeProperty(win->src.display, win->src.window, _MOTIF_WM_HINTS,
                    _MOTIF_WM_HINTS, 32, PropModeReplace, (u8 *)&hints, 5);
  ...
  void RGFW_releaseCursor(RGFW_window * win) {
    ...
    XISelectEvents(win->src.display, XDefaultRootWindow(win->src.display), &em,
                   1);
#endif
#ifdef RGFW_WAYLAND
  wayland:
  ...
  void RGFW_captureCursor(RGFW_window * win, RGFW_rect r) {
    ...
    XGrabPointer(win->src.display, win->src.window, True, PointerMotionMask,
                 GrabModeAsync, GrabModeAsync, None, None, CurrentTime);
    RGFW_window_moveMouse(
        win, RGFW_POINT(win->r.x + (i32)(r.w / 2), win->r.y + (i32)(r.h / 2)));
#endif
#ifdef RGFW_WAYLAND
  wayland:
#endif
...
#define RGFW_LOAD_LIBRARY(x, lib)                                              \
  if (x == NULL)                                                               \
  x = dlopen(lib, RTLD_LAZY | RTLD_LOCAL)
#define RGFW_PROC_DEF(proc, name)                                              \
  if (name##SRC == NULL && proc != NULL)                                       \
  name##SRC = (PFN_##name)(void *)dlsym(proc, #name)
  ...
  RGFW_LOAD_LIBRARY(X11Xihandle, "libXi.so.6");
    ...
    RGFW_GOTO_WAYLAND(0);
#ifdef RGFW_X11
    u64 event_mask = KeyPressMask | KeyReleaseMask | ButtonPressMask |
                     ButtonReleaseMask | PointerMotionMask |
                     StructureNotifyMask | FocusChangeMask | LeaveWindowMask |
                     EnterWindowMask | ExposureMask; /*!< X11 events accepted*/

#if defined(RGFW_OPENGL) && !defined(RGFW_EGL)
    u32 *visual_attribs =
        (u32 *)RGFW_initFormatAttribs(flags & RGFW_windowOpenglSoftware);
    i32 fbcount;
    GLXFBConfig *fbc =
        glXChooseFBConfig(win->src.display, DefaultScreen(win->src.display),
                          (i32 *)visual_attribs, &fbcount);
  ...
  viNorm.visual =
      DefaultVisual(win->src.display, DefaultScreen(win->src.display));

  viNorm.depth = 0;
  XVisualInfo *vi = &viNorm;

  XMatchVisualInfo(win->src.display, DefaultScreen(win->src.display), 32,
                   TrueColor, vi); /*!< for RGBA backgrounds*/
#endif
    ...
    swa.colormap = cmap =
        XCreateColormap(win->src.display, DefaultRootWindow(win->src.display),
                        vi->visual, AllocNone);
    ...
    /* create the window*/
    win->src.window = XCreateWindow(
        win->src.display, DefaultRootWindow(win->src.display), win->r.x,
        win->r.y, win->r.w, win->r.h, 0, vi->depth, InputOutput, vi->visual,
...
#if defined(RGFW_OPENGL) &&                                                    \
    !defined(RGFW_EGL) /* This is the second part of setting up opengl. This   \
                          is where we ask OpenGL for a specific version. */
      ...
      win->src.ctx = glXCreateContextAttribsARB(win->src.display, bestFbc, ctx,
    ...
    if (flags &
        RGFW_windowNoResize) { /* make it so the user can't resize the window*/
      XSizeHints sh;
      sh.flags = (1L << 4) | (1L << 5);
      sh.min_width = sh.max_width = win->r.w;
      sh.min_height = sh.max_height = win->r.h;

      XSetWMSizeHints(win->src.display, (Drawable)win->src.window, &sh,
                      XA_WM_NORMAL_HINTS);

      win->_flags |= RGFW_windowNoResize;
    }

    XSelectInput(win->src.display, (Drawable)win->src.window,
                 event_mask); /*!< tell X11 what events we want*/

    /* make it so the user can't close the window until the program does*/
    if (wm_delete_window == 0) {
      wm_delete_window =
          XInternAtom(win->src.display, "WM_DELETE_WINDOW", False);
    ...
    XSetWMProtocols(win->src.display, (Drawable)win->src.window,
                    &wm_delete_window, 1);

/* connect the context to the window*/
#if defined(RGFW_OPENGL) && !defined(RGFW_EGL)
    if ((flags & RGFW_windowNoInitAPI) == 0)
      glXMakeCurrent(win->src.display, (Drawable)win->src.window,
                     (GLXContext)win->src.ctx);
#endif

    /* set the background*/
    RGFW_window_setName(win, name);

    XMapWindow(win->src.display,
               (Drawable)win->src.window); /* draw the window*/
    XMoveWindow(win->src.display, (Drawable)win->src.window, win->r.x,
                win->r.y); /*!< move the window to it's proper cords*/

    if (flags & RGFW_windowAllowDND) { /* init drag and drop atoms and turn on
                                          drag and drop for this window */
      ...
      XChangeProperty(win->src.display, win->src.window, XdndAware, 4, 32,
                      PropModeReplace, &version,
                      1); /*!< turns on drag and drop */
    }
  ...
  wayland:
    ...
    win->src.window =
        XCreateWindow(win->src.display, DefaultRootWindow(win->src.display), 0,
                      0, 1, 1, 0, CopyFromParent, InputOutput, CopyFromParent,
    ...
    if (RGFW_wl_cursor_theme == NULL) {
      ...
      struct wl_cursor *cursor =
          wl_cursor_theme_get_cursor(RGFW_wl_cursor_theme, "left_ptr");
      RGFW_cursor_image = cursor->images[0];
      struct wl_buffer *cursor_buffer =
          wl_cursor_image_get_buffer(RGFW_cursor_image);
    ...
    win->src.xdg_surface =
        xdg_wm_base_get_xdg_surface(win->src.xdg_wm_base, win->src.surface);
    ...
    xdg_toplevel_add_listener(win->src.xdg_toplevel, &xdg_toplevel_listener,
                              NULL);

    xdg_surface_set_window_geometry(win->src.xdg_surface, 0, 0, win->r.w,
                                    win->r.h);
    ...
    while (wl_display_dispatch(win->src.wl_display) != -1 &&
           !RGFW_wl_configured) {
    }
    ...
    if ((flags & RGFW_windowNoInitAPI) == 0) {
      win->src.eglWindow =
          wl_egl_window_create(win->src.surface, win->r.w, win->r.h);
      RGFW_createOpenGLContext(win);
    }
#endif
...
#ifdef RGFW_WAYLAND
  wayland:
    return RGFW_AREA(RGFW_root->r.w, RGFW_root->r.h); // TODO
#endif
  }
  ...
  RGFW_point RGFW_getGlobalMousePoint(void) {
    ...
    XQueryPointer(RGFW_root->src.display,
                  XDefaultRootWindow(RGFW_root->src.display), &window1,
                  &window2, &RGFWMouse.x, &RGFWMouse.y, &x, &y, &z);

    return RGFWMouse;
  }

  RGFWDEF void RGFW_XHandleClipboardSelection(RGFW_window * win,
                                              XEvent * event);
  ...
  void RGFW_XHandleClipboardSelection(RGFW_window * win, XEvent * event) {
    ...
    if (request->target == TARGETS) {
      ...
      XChangeProperty(win->src.display, request->requestor, request->property,
                      XA_ATOM, 32, PropModeReplace, (u8 *)targets,
      ...
      XGetWindowProperty(RGFW_root->src.display, request->requestor,
                         request->property, 0, LONG_MAX, False, ATOM_PAIR,
      ...
      XChangeProperty(RGFW_root->src.display, request->requestor,
                      request->property, ATOM_PAIR, 32, PropModeReplace,
                      (u8 *)targets, count);
      ...
      for (int i = 0; i < formatCount; i++) {
        ...
        XChangeProperty(win->src.display, request->requestor, request->property,
                        ...
                        (u8 *)win->src.clipboard, win->src.clipboard_len);
      }
    }
  ...
  RGFW_event *RGFW_window_checkEvent(RGFW_window * win) {
...
#if defined(__linux__) && !defined(RGFW_NO_LINUX)
    if (RGFW_linux_updateGamepad(win))
      return &win->event;
#endif
    RGFW_GOTO_WAYLAND(0);
#ifdef RGFW_X11
    ...
    if ((QLength(win->src.display) ||
         XEventsQueued(win->src.display, QueuedAlready) +
    ...
    case KeyRelease: {
      ...
      if (E.type == KeyRelease &&
          XEventsQueued(
              win->src.display,
              QueuedAfterReading)) { /* get next event if there is one*/
        XEvent NE;
        XPeekEvent(win->src.display, &NE);

        if (E.xkey.time == NE.xkey.time &&
            E.xkey.keycode == NE.xkey.keycode) /* check if the current and next
                                                  are both the same*/
      ...
      KeySym sym =
          (KeySym)XkbKeycodeToKeysym(win->src.display, E.xkey.keycode, 0,
      ...
      /* get keystate data */
      win->event.type =
          (E.type == KeyPress) ? RGFW_keyPressed : RGFW_keyReleased;
      ...
      RGFW_updateKeyMods(win, (state.locked_mods & LockMask),
                         (state.locked_mods & Mod2Mask),
                         (state.locked_mods & Mod3Mask));

      RGFW_keyCallback(win, win->event.key, win->event.keyChar,
                       win->event.keyMod, (E.type == KeyPress));
      break;
    }
    case ButtonPress:
    case ButtonRelease:
      ...
      win->event.type = RGFW_mouseButtonPressed +
                        (E.type == ButtonRelease); // the events match
      ...
      RGFW_mouseButtons[win->event.button].prev =
          RGFW_mouseButtons[win->event.button].current;
      ...
      RGFW_mouseButtonCallback(win, win->event.button, win->event.scroll,
                               (E.type == ButtonPress));
      break;
    ...
    case GenericEvent: {
      ...
      if (E.xcookie.evtype == XI_RawMotion) {
        ...
        RGFW_window_moveMouse(win, RGFW_POINT(win->r.x + (win->r.w / 2),
                                              win->r.y + (win->r.h / 2)));
    ...
    case ClientMessage: {
      ...
      if (E.xclient.message_type == XdndEnter) {
        ...
        if (list) {
          ...
          XGetWindowProperty(win->src.display, source, XdndTypeList, 0,
                             LONG_MAX, False, 4, &actualType, &actualFormat,
                             &count, &bytesAfter, (u8 **)&formats);
        } else {
          count = 0;
      ...
      if (E.xclient.message_type == XdndPosition) {
        ...
        XTranslateCoordinates(
            win->src.display, XDefaultRootWindow(win->src.display),
            win->src.window, xabs, yabs, &xpos, &ypos, &dummy);
      ...
      if (format) {
        Time time = (version >= 1) ? (Time)E.xclient.data.l[2] : CurrentTime;

        XConvertSelection(win->src.display, XdndSelection, (Atom)format,
    ...
    case SelectionNotify: {
      ...
      XGetWindowProperty(win->src.display, E.xselection.requestor,
                         E.xselection.property, 0, LONG_MAX, False,
                         E.xselection.target, &actualType, &actualFormat,
                         &result, &bytesAfter, (u8 **)&data);
      ...
      while ((line = (char *)RGFW_strtok(data, "\r\n"))) {
        ...
        path[index] = '\0';
        RGFW_MEMCPY(win->event.droppedFiles[win->event.droppedFilesCount - 1],
                    path, index + 1);
      }
      ...
      RGFW_dndCallback(win, win->event.droppedFiles,
                       win->event.droppedFilesCount);
      break;
    }
    case FocusIn:
    ...
    case ConfigureNotify: {
      ...
      if (E.xconfigure.width != win->r.w || E.xconfigure.height != win->r.h) {
        ...
        win->r = RGFW_RECT(win->r.x, win->r.y, E.xconfigure.width,
                           E.xconfigure.height);
  ...
  void RGFW_window_minimize(RGFW_window * win) {
    ...
    win->_oldRect = win->r;
    XIconifyWindow(win->src.display, win->src.window,
  ...
  void RGFW_window_setMousePassthrough(RGFW_window * win,
                                       RGFW_bool passthrough) {
    RGFW_ASSERT(win != NULL);
  ...
  RGFW_LOAD_LIBRARY(RGFW_libxshape, "libXext.so.6");
    ...
    typedef void (*PFN_XShapeCombineMask)(Display *, Window, int, int, int,
                                          Pixmap, int);
    static PFN_XShapeCombineMask XShapeCombineMaskSRC;

    typedef void (*PFN_XShapeCombineRegion)(Display *, Window, int, int, int,
                                            Region, int);
    ...
    if (passthrough) {
      ...
      XShapeCombineRegionSRC(win->src.display, win->src.window, ShapeInput, 0,
                             0, region, ShapeSet);
    ...
    XShapeCombineMaskSRC(win->src.display, win->src.window, ShapeInput, 0, 0,
                         None, ShapeSet);
  }

#endif /* RGFW_NO_PASSTHROUGH */

  RGFW_bool RGFW_window_setIcon(RGFW_window * win, u8 * icon, RGFW_area a,
                                i32 channels) {
    RGFW_ASSERT(win != NULL);
    RGFW_ASSERT(icon != NULL);
    ...
    unsigned long *data =
        (unsigned long *)RGFW_ALLOC(count * sizeof(unsigned long));
  ...
  void RGFW_window_moveMouse(RGFW_window * win, RGFW_point p) {
    ...
    XWarpPointer(win->src.display, None, win->src.window, 0, 0, 0, 0,
                 (int)p.x - win->r.x, (int)p.y - win->r.y);
#endif
#ifdef RGFW_WAYLAND
  wayland:
#endif
...
#ifdef RGFW_WAYLAND
  wayland:
    static const char *iconStrings[] = {"left_ptr",   "left_ptr",   "text",
                                        "cross",      "pointer",    "e-resize",
                                        "n-resize",   "nw-resize",  "ne-resize",
                                        "all-resize", "not-allowed"};

    struct wl_cursor *wlcursor =
        wl_cursor_theme_get_cursor(RGFW_wl_cursor_theme, iconStrings[mouse]);
  ...
  RGFW_ssize_t RGFW_readClipboardPtr(char *str, size_t strCapacity) {
    ...
    if (XGetSelectionOwner(RGFW_root->src.display, RGFW_XCLIPBOARD) ==
        RGFW_root->src.window) {
      if (str != NULL)
    ...
    XConvertSelection(RGFW_root->src.display, RGFW_XCLIPBOARD,
                      RGFW_XUTF8_STRING, XSEL_DATA, RGFW_root->src.window,
                      CurrentTime);
    XSync(RGFW_root->src.display, 0);
    XNextEvent(RGFW_root->src.display, &event);

    if (event.type != SelectionNotify ||
        event.xselection.selection != RGFW_XCLIPBOARD ||
        event.xselection.property == 0)
      return -1;

    XGetWindowProperty(event.xselection.display, event.xselection.requestor,
                       event.xselection.property, 0L, (~0L), 0, AnyPropertyType,
                       &target, &format, &sizeN, &N, (u8 **)&data);
  ...
  void RGFW_writeClipboard(const char *text, u32 textLen) {
    ...
    /* request ownership of the clipboard section and request to convert it,
     * this means its our job to convert it */
    XSetSelectionOwner(RGFW_root->src.display, RGFW_XCLIPBOARD,
                       RGFW_root->src.window, CurrentTime);
    if (XGetSelectionOwner(RGFW_root->src.display, RGFW_XCLIPBOARD) !=
        RGFW_root->src.window) {
#ifdef RGFW_DEBUG
      fprintf(stderr,
              "RGFW: X11 failed to become owner of clipboard selection\n");
#endif
      return;
    }
  ...
  RGFW_bool RGFW_window_isMinimized(RGFW_window * win) {
    ...
    i16 status =
        XGetWindowProperty(win->src.display, win->src.window, WM_STATE, 0, 2,
                           False, AnyPropertyType, &actual_type, &actual_format,
                           &nitems, &bytes_after, &prop_data);

    if (status == Success && nitems >= 1 &&
        *((int *)prop_data) == IconicState) {
      XFree(prop_data);
      return RGFW_TRUE;
    }
  ...
  RGFW_bool RGFW_window_isMaximized(RGFW_window * win) {
    ...
    i16 status =
        XGetWindowProperty(win->src.display, win->src.window, _NET_WM_STATE, 0,
                           1024, False, XA_ATOM, &actual_type, &actual_format,
                           &nitems, &bytes_after, &prop_data);
  ...
  static u32 RGFW_XCalculateRefreshRate(XRRModeInfo mi) {
    ...
    return (u32)RGFW_ROUND((double)mi.dotClock /
  ...
  static float XGetSystemContentDPI(Display * display, i32 screen) {
    ...
    if (rms && db) {
      ...
      if (XrmGetResource(db, "Xft.dpi", "Xft.Dpi", &type, &value) && type &&
          RGFW_STRNCMP(type, "String", 7) == 0) {
        dpi = (float)atof(value.addr);
      }
      XrmDestroyDatabase(db);
    }
#else
  dpi = RGFW_ROUND(DisplayWidth(display, screen) /
                   (DisplayWidthMM(display, screen) / 25.4));
#endif
  ...
  RGFW_monitor RGFW_XCreateMonitor(i32 screen) {
    ...
    XRRScreenResources *sr =
        XRRGetScreenResourcesCurrent(display, RootWindow(display, screen));
    ...
    if (info == NULL || ci == NULL) {
      ...
      printf("RGFW INFO: monitor found: scale (%s):\n   rect: {%i, %i, %i, "
             "%i}\n   physical size:%f %f\n   scale: %f %f\n   pixelRatio: "
             "%f\n    refreshRate: %i\n",
             monitor.name, monitor.rect.x, monitor.rect.y, monitor.rect.w,
             monitor.rect.h, monitor.physW, monitor.physH, monitor.scaleX,
             monitor.scaleY, monitor.pixelRatio, monitor.refreshRate);
#endif
      return monitor;
    }
    ...
    if (monitor.physW == 0 || monitor.physH == 0) {
      ...
      float ppi_width =
          RGFW_ROUND((float)monitor.rect.w / (float)monitor.physW);
      float ppi_height =
          RGFW_ROUND((float)monitor.rect.h / (float)monitor.physH);
...
#ifdef RGFW_WAYLAND
  wayland:
    return RGFW_monitors; // TODO WAYLAND
#endif
  }
...
#ifdef RGFW_WAYLAND
  wayland:
    return (RGFW_monitor){}; // TODO WAYLAND
#endif
  }
...
#ifdef RGFW_X11
#ifndef RGFW_NO_DPI
    XRRScreenResources *screenRes = XRRGetScreenResources(
        RGFW_root->src.display, DefaultRootWindow(RGFW_root->src.display));
    if (screenRes == NULL)
      return RGFW_FALSE;
    for (int i = 0; i < screenRes->ncrtc; i++) {
      XRRCrtcInfo *crtcInfo = XRRGetCrtcInfo(RGFW_root->src.display, screenRes,
                                             screenRes->crtcs[i]);
      if (!crtcInfo)
        continue;

      if (mon.rect.x == crtcInfo->x && mon.rect.y == crtcInfo->y &&
          (u32)mon.rect.w == crtcInfo->width &&
          (u32)mon.rect.h == crtcInfo->height) {
        RRMode mode = None;
        for (int index = 0; index < screenRes->nmode; index++) {
          if (screenRes->modes[index].width == area.w &&
              ...
              (refreshRate == 0 ||
               refreshRate ==
                   RGFW_XCalculateRefreshRate(screenRes->modes[index]))) {
        ...
        if (mode != None) {
          ...
          XRROutputInfo *info =
              XRRGetOutputInfo(RGFW_root->src.display, screenRes, output);
          if (info) {
            XRRSetCrtcConfig(RGFW_root->src.display, screenRes,
                             screenRes->crtcs[i], CurrentTime, 0, 0, mode,
  ...
  RGFW_monitor RGFW_window_getMonitor(RGFW_window * win) {
...
#ifndef RGFW_NO_DPI
    XRRScreenResources *screenRes = XRRGetScreenResources(
        win->src.display, DefaultRootWindow(win->src.display));
    if (screenRes == NULL) {
      return (RGFW_monitor){};
    }
    ...
    for (int i = 0; i < screenRes->ncrtc; i++) {
      XRRCrtcInfo *crtcInfo =
          XRRGetCrtcInfo(win->src.display, screenRes, screenRes->crtcs[i]);
      if (!crtcInfo)
        continue;
      ...
      if (attrs.x >= monitorX && attrs.x < monitorX + monitorWidth &&
          attrs.y >= monitorY && attrs.y < monitorY + monitorHeight) {
        XRRFreeCrtcInfo(crtcInfo);
        XRRFreeScreenResources(screenRes);
  ...
  for (i = 0; i < (size_t)ScreenCount(RGFW_root->src.display) && i < 6; i++) {
    ...
    if (attrs.x >= 0 && attrs.x < 0 + XWidthOfScreen(screen) && attrs.y >= 0 &&
        attrs.y < 0 + XHeightOfScreen(screen))
      return RGFW_XCreateMonitor(i);
  }
#endif
#endif
...
#endif
      XPutImage(win->src.display, win->src.window, win->src.gc, win->src.bitmap,
                0, 0, 0, 0, win->bufferSize.w, win->bufferSize.h);
  ...
  void RGFW_window_swapInterval(RGFW_window * win, i32 swapInterval) {
    ...
    ((PFNGLXSWAPINTERVALEXTPROC)glXGetProcAddress(
        (GLubyte *)"glXSwapIntervalEXT"))(win->src.display, win->src.window,
                                          swapInterval);
#else
  ...
  void RGFW_window_close(RGFW_window * win) {
    ...
    RGFW_LOAD_ATOM(SAVE_TARGETS);
    if (XGetSelectionOwner(win->src.display, RGFW_XCLIPBOARD) ==
        win->src.window) {
      XConvertSelection(win->src.display, CLIPBOARD_MANAGER, SAVE_TARGETS, None,
                        win->src.window, CurrentTime);
    ...
    if (win->src.display) {
      ...
      if ((Drawable)win->src.window)
        XDestroyWindow(win->src.display,
                       (Drawable)win->src.window); /*!< close the window*/
...
#define XInputGetState XInputGetStateSRC
  ...
  typedef DWORD(WINAPI * PFN_XInputGetKeystroke)(DWORD, DWORD,
                                                 PXINPUT_KEYSTROKE);
...
#define XInputGetKeystroke XInputGetKeystrokeSRC
  ...
  u32 RGFW_mouseIconSrc[] = {
      OCR_NORMAL, OCR_NORMAL,   OCR_IBEAM,    OCR_CROSS,   OCR_HAND, OCR_SIZEWE,
      OCR_SIZENS, OCR_SIZENWSE, OCR_SIZENESW, OCR_SIZEALL, OCR_NO};
...
#define wglGetSwapIntervalEXT wglGetSwapIntervalEXTSrc
  ...
  typedef HGLRC(WINAPI * PFNWGLCREATECONTEXTATTRIBSARBPROC)(
      HDC hdc, HGLRC hglrc, const int *attribList);
...
#define wglShareLists wglShareListsSRC
  ...
  typedef HRESULT(APIENTRY * PFNWGLCHOOSEPIXELFORMATARBPROC)(
  ...
  LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
    ...
    case WM_SIZE: {
      ...
      i32 offset = (windowRect.bottom - windowRect.top) -
                   (clientRect.bottom - clientRect.top);

      if (win->src.aspectRatio.w != 0 && win->src.aspectRatio.h != 0) {
        double aspectRatio =
            (double)win->src.aspectRatio.w / win->src.aspectRatio.h;
        ...
        RGFW_window_resize(
            win, RGFW_AREA((windowRect.right - windowRect.left),
      ...
      RGFW_eventWindow.src.window = hWnd;
      return DefWindowProcA(hWnd, message, wParam,
                            lParam); // Call DefWindowProc after handling
    }
...
#define RGFW_PROC_DEF(proc, name)                                              \
  if (name##SRC == NULL && proc != NULL)                                       \
  name##SRC = (PFN_##name)(void *)GetProcAddress(proc, #name)

#ifndef RGFW_NO_XINPUT
  void RGFW_loadXInput(void) {
    u32 i;
    static const char *names[] = {"xinput1_4.dll", "xinput9_1_0.dll",
                                  "xinput1_2.dll", "xinput1_1.dll"};

    for (i = 0; i < sizeof(names) / sizeof(const char *) &&
                (XInputGetStateSRC == NULL || XInputGetStateSRC != NULL);
         i++) {
  ...
  void RGFW_window_initBufferPtr(RGFW_window * win, u8 * buffer,
                                 RGFW_area area) {
#if defined(RGFW_OSMESA) || defined(RGFW_BUFFER)
    win->buffer = buffer;
    win->bufferSize = area;
    ...
    win->src.bitmap =
        CreateDIBSection(win->src.hdc, (BITMAPINFO *)&bi, DIB_RGB_COLORS,
                         (void **)&win->src.bitmapBits, NULL, (DWORD)0);
    ...
    OSMesaMakeCurrent(win->src.ctx, win->buffer, GL_UNSIGNED_BYTE, win->r.w,
                      win->r.h);
#endif
#else
  RGFW_UNUSED(win);
  RGFW_UNUSED(buffer);
  RGFW_UNUSED(area); /*!< if buffer rendering is not being used */
#endif
  }
...
#define __uuidof(T) IID_##T
  ...
  int RGFW_window_createDXSwapChain(RGFW_window * win, IDXGIFactory * pFactory,
                                    IUnknown * pDevice,
    ...
    HRESULT hr = pFactory->lpVtbl->CreateSwapChain(
        pFactory, (IUnknown *)pDevice, &swapChainDesc, swapchain);
    if (FAILED(hr)) {
  ...
  WNDCLASSA Class = {};
    ...
    Class.hIcon =
        (HICON)LoadImageA(GetModuleHandleW(NULL), "RGFW_ICON", IMAGE_ICON, 0, 0,
                          LR_DEFAULTSIZE | LR_SHARED);
    if (Class.hIcon == NULL) {
      Class.hIcon = (HICON)LoadImageA(NULL, (LPCSTR)IDI_APPLICATION, IMAGE_ICON,
    ...
    HWND dummyWin =
        CreateWindowA(Class.lpszClassName, name, window_style, win->r.x,
                      win->r.y, win->r.w, win->r.h, 0, 0, inh, 0);

    GetWindowRect(dummyWin, &windowRect);
    GetClientRect(dummyWin, &clientRect);

    win->src.hOffset = (windowRect.bottom - windowRect.top) -
                       (clientRect.bottom - clientRect.top);
    win->src.window = CreateWindowA(Class.lpszClassName, name, window_style,
                                    win->r.x, win->r.y, win->r.w,
                                    win->r.h + win->src.hOffset, 0, 0, inh, 0);
    ...
    if ((flags & RGFW_windowNoInitAPI) == 0) {
      ...
      PIXELFORMATDESCRIPTOR pfd = {
          ...
          1,                             // Version
          pfd_flags, // Flags to specify what the pixel format supports (e.g.,
                     // PFD_SUPPORT_OPENGL)
          PFD_TYPE_RGBA, // Pixel type is RGBA
          32,            // Color bits (red, green, blue channels)
          0, 0, 0, 0, 0,
          0, // No color bits for unused channels
      ...
      /* try to create the pixel format we want for opengl and then try to
       * create an opengl context for the specified version */
      if (wglCreateContextAttribsARB != NULL) {
        ...
        if (wglChoosePixelFormatARB != NULL) {
          i32 *pixel_format_attribs =
              (i32 *)RGFW_initFormatAttribs(flags & RGFW_windowOpenglSoftware);

          int pixel_format;
          UINT num_formats;
          wglChoosePixelFormatARB(win->src.hdc, pixel_format_attribs, 0, 1,
        ...
        if (RGFW_profile == RGFW_glCore) {
          SET_ATTRIB(WGL_CONTEXT_PROFILE_MASK_ARB,
        ...
        win->src.ctx =
            (HGLRC)wglCreateContextAttribsARB(win->src.hdc, NULL, attribs);
      } else { /* fall back to a default context (probably opengl 2 or
                  something) */
#ifdef RGFW_DEBUG
  ...
  void RGFW_window_setBorder(RGFW_window * win, RGFW_bool border) {
    ...
    if (border == 0) {
      ...
      SetWindowPos(win->src.window, HWND_TOP, 0, 0, 0, 0,
  ...
  RGFW_area RGFW_getScreenSize(void) {
    HDC dc = GetDC(NULL);
    RGFW_area area =
        RGFW_AREA(GetDeviceCaps(dc, HORZRES), GetDeviceCaps(dc, VERTRES));
    ReleaseDC(NULL, dc);
    return area;
  }
  ...
  void RGFW_window_setFullscreen(RGFW_window * win, RGFW_bool fullscreen) {
    ...
    if (fullscreen == RGFW_FALSE) {
      ...
      SetWindowPos(win->src.window, HWND_NOTOPMOST, win->_oldRect.x,
                   win->_oldRect.y, win->_oldRect.w,
                   win->_oldRect.h + win->src.hOffset,
    ...
    RGFW_window_setBorder(win, 0);
    SetWindowPos(win->src.window, HWND_TOPMOST, 0, 0, mon.rect.w, mon.rect.h,
                 SWP_NOOWNERZORDER | SWP_FRAMECHANGED);
  ...
  static i32 RGFW_checkXInput(RGFW_window * win, RGFW_event * e) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if ((keystroke.Flags & XINPUT_KEYSTROKE_REPEAT) == 0 &&
        ...
        e->type = RGFW_gamepadButtonPressed +
                  !(keystroke.Flags & XINPUT_KEYSTROKE_KEYDOWN);
        e->button = RGFW_xinput2RGFW[keystroke.VirtualKey - 0x5800];
        RGFW_gamepadPressed[i][e->button].prev =
            RGFW_gamepadPressed[i][e->button].current;
        RGFW_gamepadPressed[i][e->button].current =
            (keystroke.Flags & XINPUT_KEYSTROKE_KEYDOWN);

        RGFW_gamepadButtonCallback(win, i, e->button,
                                   e->type == RGFW_gamepadButtonPressed);
        return 1;
      }
...
#define INPUT_DEADZONE                                                         \
  (0.24f *                                                                     \
   (float)(0x7FFF)) // Default to 24% of the +/- 32767 range.   This is a
                    // reasonable default value but can be altered if needed.
      ...
      e->axisesCount = 2;
      RGFW_point axis1 =
          RGFW_POINT(((float)state.Gamepad.sThumbLX / 32768.0f) * 100,
                     ((float)state.Gamepad.sThumbLY / -32768.0f) * 100);
      RGFW_point axis2 =
          RGFW_POINT(((float)state.Gamepad.sThumbRX / 32768.0f) * 100,
      ...
      if (axis1.x != e->axis[0].x || axis1.y != e->axis[0].y) {
        ...
        RGFW_gamepadAxisCallback(win, e->gamepad, e->axis, e->axisesCount,
                                 e->whichAxis);
        return 1;
      }
  ...
  RGFW_event *RGFW_window_checkEvent(RGFW_window * win) {
    ...
    case WM_KEYUP: {
      ...
      case 0x136:
        scancode = 0x36;
        break; /*  CJK IME sets the extended bit for right Shift */
      default:
        break;
      }
      ...
      if (msg.wParam == VK_CONTROL) {
        if (HIWORD(msg.lParam) & KF_EXTENDED)
          ...
          win->event.key = RGFW_controlL;
      }

      wchar_t charBuffer;
      ToUnicodeEx(msg.wParam, scancode, keyboardState, (wchar_t *)&charBuffer,
                  1, 0, NULL);
      ...
      RGFW_updateKeyMods(win, (GetKeyState(VK_CAPITAL) & 0x0001),
    ...
    case WM_INPUT: {
      ...
      GetRawInputData((HRAWINPUT)msg.lParam, RID_INPUT, &raw, &size,
    ...
    case WM_XBUTTONDOWN:
      if (msg.message == WM_XBUTTONDOWN)
        win->event.button =
            RGFW_mouseMisc1 + (GET_XBUTTON_WPARAM(msg.wParam) == XBUTTON2);
      else
        win->event.button = (msg.message == WM_LBUTTONDOWN) ? RGFW_mouseLeft
    ...
    case WM_XBUTTONUP:
      if (msg.message == WM_XBUTTONUP)
        ...
        win->event.button = (msg.message == WM_LBUTTONUP)   ? RGFW_mouseLeft
                            : (msg.message == WM_RBUTTONUP) ? RGFW_mouseRight
                                                            : RGFW_mouseMiddle;
  ...
  RGFW_bool RGFW_window_isHidden(RGFW_window * win) {
    ...
    return IsWindowVisible(win->src.window) == 0 &&
           !RGFW_window_isMinimized(win);
  }
  ...
  BOOL CALLBACK GetMonitorByHandle(HMONITOR hMonitor, HDC hdcMonitor,
  ...
  RGFW_monitor win32CreateMonitor(HMONITOR src) {
    ...
    for (DWORD deviceNum = 0; EnumDisplayDevicesA(NULL, deviceNum, &dd, 0);
         deviceNum++) {
  ...
  RGFW_monitor RGFW_monitors[6];
  BOOL CALLBACK GetMonitorHandle(HMONITOR hMonitor, HDC hdcMonitor,
                                 LPRECT lprcMonitor, LPARAM dwData) {
    RGFW_UNUSED(hdcMonitor);
    RGFW_UNUSED(lprcMonitor);
  ...
  RGFW_bool RGFW_monitor_scale(RGFW_monitor mon, RGFW_area area,
                               u32 refreshRate) {
    HMONITOR src = MonitorFromPoint((POINT){mon.rect.x, mon.rect.y},
    ...
    for (DWORD deviceNum = 0; EnumDisplayDevicesA(NULL, deviceNum, &dd, 0);
      ...
      if (EnumDisplaySettingsA(dd.DeviceName, ENUM_CURRENT_SETTINGS, &dm)) {
        ...
        if (ChangeDisplaySettingsEx(dd.DeviceName, &dm, NULL, CDS_TEST, NULL) ==
            DISP_CHANGE_SUCCESSFUL) {
          if (ChangeDisplaySettingsEx(dd.DeviceName, &dm, NULL,
                                      CDS_UPDATEREGISTRY,
  ...
  RGFW_bool RGFW_window_setMouseStandard(RGFW_window * win, u8 mouse) {
    ...
    SetClassLongPtrA(win->src.window, GCLP_HCURSOR,
                     (LPARAM)LoadCursorA(NULL, icon));
...
#define RGFW_FREE_LIBRARY(x)                                                   \
  ...
  void RGFW_window_move(RGFW_window * win, RGFW_point v) {
    ...
    win->r.x = v.x;
    win->r.y = v.y;
    SetWindowPos(win->src.window, HWND_TOP, win->r.x, win->r.y, 0, 0,
                 SWP_NOSIZE);
  }
  ...
  void RGFW_window_resize(RGFW_window * win, RGFW_area a) {
    ...
    win->r.w = a.w;
    win->r.h = a.h;
    SetWindowPos(win->src.window, HWND_TOP, 0, 0, win->r.w,
                 win->r.h + win->src.hOffset, SWP_NOMOVE);
  }
  ...
  RGFW_bool RGFW_window_setIcon(RGFW_window * win, u8 * src, RGFW_area a,
                                i32 channels) {
    RGFW_ASSERT(win != NULL);
#ifndef RGFW_WIN95
    RGFW_UNUSED(channels);
  ...
  void RGFW_window_swapBuffers(RGFW_window * win) {
    ...
    if (!(win->_flags & RGFW_NO_CPU_RENDER)) {
      ...
      if (win->buffer != win->src.bitmapBits)
        memcpy(win->src.bitmapBits, win->buffer,
               win->bufferSize.w * win->bufferSize.h * 4);

      HGDIOBJ oldbmp = SelectObject(win->src.hdcMem, win->src.bitmap);
      BitBlt(win->src.hdc, 0, 0, win->r.w, win->r.h, win->src.hdcMem, 0, 0,
             SRCCOPY);
  ...
  void RGFW_sleep(u64 ms) { Sleep(ms); }

#ifndef RGFW_NO_THREADS

  RGFW_thread RGFW_createThread(RGFW_threadFunc_ptr ptr, void *args) {
    return CreateThread(NULL, 0, ptr, args, 0, NULL);
  }
...
#define objc_msgSend_bool(x, y)                                                \
  ((BOOL(*)(id, SEL))objc_msgSend)((id)(x), (SEL)y)
#define objc_msgSend_void(x, y)                                                \
  ((void (*)(id, SEL))objc_msgSend)((id)(x), (SEL)y)
#define objc_msgSend_void_id(x, y, z)                                          \
  ((void (*)(id, SEL, id))objc_msgSend)((id)x, (SEL)y, (id)z)
#define objc_msgSend_uint(x, y)                                                \
  ((NSUInteger(*)(id, SEL))objc_msgSend)((id)(x), (SEL)y)
#define objc_msgSend_void_bool(x, y, z)                                        \
  ((void (*)(id, SEL, BOOL))objc_msgSend)((id)(x), (SEL)y, (BOOL)z)
#define objc_msgSend_bool_void(x, y)                                           \
  ((BOOL(*)(id, SEL))objc_msgSend)((id)(x), (SEL)y)
#define objc_msgSend_void_SEL(x, y, z)                                         \
  ((void (*)(id, SEL, SEL))objc_msgSend)((id)(x), (SEL)y, (SEL)z)
#define objc_msgSend_id(x, y) ((id(*)(id, SEL))objc_msgSend)((id)(x), (SEL)y)
#define objc_msgSend_id_id(x, y, z)                                            \
  ((id(*)(id, SEL, id))objc_msgSend)((id)(x), (SEL)y, (id)z)
#define objc_msgSend_id_bool(x, y, z)                                          \
  ((BOOL(*)(id, SEL, id))objc_msgSend)((id)(x), (SEL)y, (id)z)
#define objc_msgSend_int(x, y, z)                                              \
  ((id(*)(id, SEL, int))objc_msgSend)((id)(x), (SEL)y, (int)z)
#define objc_msgSend_arr(x, y, z)                                              \
  ((id(*)(id, SEL, int))objc_msgSend)((id)(x), (SEL)y, (int)z)
#define objc_msgSend_ptr(x, y, z)                                              \
  ((id(*)(id, SEL, void *))objc_msgSend)((id)(x), (SEL)y, (void *)z)
#define objc_msgSend_class(x, y)                                               \
  ((id(*)(Class, SEL))objc_msgSend)((Class)(x), (SEL)y)
#define objc_msgSend_class_char(x, y, z)                                       \
  ((id(*)(Class, SEL, char *))objc_msgSend)((Class)(x), (SEL)y, (char *)z)
...
#define NSRelease(obj) objc_msgSend_void((id)obj, sel_registerName("release"))
  ...
  id NSString_stringWithUTF8String(const char *str) {
    return ((id(*)(id, SEL, const char *))objc_msgSend)(
        ...
        sel_registerName("stringWithUTF8String:"), str);
  }

  const char *NSString_to_char(id str) {
    return ((const char *(*)(id, SEL))objc_msgSend)(
        (id)(id)str, sel_registerName("UTF8String"));
  }

  void si_impl_func_to_SEL_with_name(
      const char *class_name, const char *register_name, void *function) {
    ...
    class_addMethod(selected_class, sel_registerName(register_name),
                    (IMP)function, 0);
  }
...
#define si_func_to_SEL(class_name, function)                                   \
  si_impl_func_to_SEL_with_name(class_name, #function ":", (void *)function)
/* Creates an Objective-C method (SEL) from a regular C function with the option
 * to set the register name.*/
#define si_func_to_SEL_with_name(class_name, register_name, function)          \
  si_impl_func_to_SEL_with_name(class_name, register_name ":", (void *)function)

  unsigned char *NSBitmapImageRep_bitmapData(id imageRep) {
    return ((unsigned char *(*)(id, SEL))objc_msgSend)(
        (id)imageRep, sel_registerName("bitmapData"));
  }

  typedef RGFW_ENUM(NSUInteger, NSBitmapFormat){
      NSBitmapFormatAlphaFirst =
          1 << 0, // 0 means is alpha last (RGBA, CMYKA, etc.)
      NSBitmapFormatAlphaNonpremultiplied = 1 << 1, // 0 means is premultiplied
      NSBitmapFormatFloatingpointSamples = 1 << 2,  // 0 is integer

      NSBitmapFormatSixteenBitLittleEndian API_AVAILABLE(macos(10.10)) =
          (1 << 8),
      NSBitmapFormatThirtyTwoBitLittleEndian API_AVAILABLE(macos(10.10)) =
          (1 << 9),
      NSBitmapFormatSixteenBitBigEndian API_AVAILABLE(macos(10.10)) = (1 << 10),
      NSBitmapFormatThirtyTwoBitBigEndian API_AVAILABLE(macos(10.10)) =
          (1 << 11)};

  id NSBitmapImageRep_initWithBitmapData(
      unsigned char **planes, NSInteger width, NSInteger height, NSInteger bps,
      NSInteger spp, bool alpha, bool isPlanar, const char *colorSpaceName,
      NSBitmapFormat bitmapFormat, NSInteger rowBytes, NSInteger pixelBits) {
    SEL func = sel_registerName(
    ...
    return (id)((id(*)(id, SEL, unsigned char **, NSInteger, NSInteger,
                       NSInteger, NSInteger, bool, bool, id, NSBitmapFormat,
                       NSInteger, NSInteger))objc_msgSend)(
        NSAlloc((id)objc_getClass("NSBitmapImageRep")), func, planes, width,
        height, bps, spp, alpha, isPlanar,
        NSString_stringWithUTF8String(colorSpaceName), bitmapFormat, rowBytes,
        pixelBits);
  }

  id NSColor_colorWithSRGB(CGFloat red, CGFloat green, CGFloat blue,
                           CGFloat alpha) {
...
#define NS_OPENGL_ENUM_DEPRECATED(minVers, maxVers)                            \
  API_AVAILABLE(macos(minVers))
  typedef RGFW_ENUM(NSInteger, NSOpenGLContextParameter){
      NSOpenGLContextParameterSwapInterval NS_OPENGL_ENUM_DEPRECATED(10.0,
                                                                     10.14) =
          222, /* 1 param.  0 -> Don't sync, 1 -> Sync to vertical retrace */
      NSOpenGLContextParametectxaceOrder NS_OPENGL_ENUM_DEPRECATED(10.0,
                                                                   10.14) =
          235, /* 1 param.  1 -> Above Window (default), -1 -> Below Window */
      NSOpenGLContextParametectxaceOpacity NS_OPENGL_ENUM_DEPRECATED(10.0,
                                                                     10.14) =
          236, /* 1 param.  1-> Surface is opaque (default), 0 -> non-opaque */
      NSOpenGLContextParametectxaceBackingSize NS_OPENGL_ENUM_DEPRECATED(
          10.0, 10.14) =
          304, /* 2 params.  Width/height of surface backing size */
      NSOpenGLContextParameterReclaimResources NS_OPENGL_ENUM_DEPRECATED(
          10.0, 10.14) = 308, /* 0 params. */
      NSOpenGLContextParameterCurrentRendererID NS_OPENGL_ENUM_DEPRECATED(
          10.0, 10.14) = 309, /* 1 param.   Retrieves the current renderer ID */
      NSOpenGLContextParameterGPUVertexProcessing NS_OPENGL_ENUM_DEPRECATED(
          10.0, 10.14) =
          310, /* 1 param.   Currently processing vertices with GPU (get) */
      NSOpenGLContextParameterGPUFragmentProcessing NS_OPENGL_ENUM_DEPRECATED(
          10.0, 10.14) =
          311, /* 1 param.   Currently processing fragments with GPU (get) */
      NSOpenGLContextParameterHasDrawable NS_OPENGL_ENUM_DEPRECATED(10.0,
                                                                    10.14) =
          314, /* 1 param.   Boolean returned if drawable is attached */
      NSOpenGLContextParameterMPSwapsInFlight NS_OPENGL_ENUM_DEPRECATED(10.0,
                                                                        10.14) =
          315, /* 1 param.   Max number of swaps queued by the MP GL engine */

      NSOpenGLContextParameterSwapRectangle API_DEPRECATED("",
                                                           macos(10.0, 10.14)) =
          200, /* 4 params.  Set or get the swap rectangle {x, y, w, h} */
      NSOpenGLContextParameterSwapRectangleEnable API_DEPRECATED(
          "", macos(10.0, 10.14)) =
          201, /* Enable or disable the swap rectangle */
      NSOpenGLContextParameterRasterizationEnable API_DEPRECATED(
          "", macos(10.0, 10.14)) =
          221, /* Enable or disable all rasterization */
      NSOpenGLContextParameterStateValidation API_DEPRECATED(
          "", macos(10.0, 10.14)) =
          301, /* Validate state for multi-screen functionality */
      NSOpenGLContextParametectxaceSurfaceVolatile API_DEPRECATED(
          "", macos(10.0, 10.14)) = 306, /* 1 param.   Surface volatile state */
  };

  typedef RGFW_ENUM(NSInteger, NSWindowButton){
      NSWindowCloseButton = 0,        NSWindowMiniaturizeButton = 1,
      NSWindowZoomButton = 2,         NSWindowToolbarButton = 3,
      NSWindowDocumentIconButton = 4, NSWindowDocumentVersionsButton = 6,
      NSWindowFullScreenButton = 7,
  };

  void NSOpenGLContext_setValues(id context, const int *vals,
  ...
  id NSPasteboard_generalPasteboard(void) {
    return (id)objc_msgSend_id((id)objc_getClass("NSPasteboard"),
  ...
  const char *NSPasteboard_stringForType(
      id pasteboard, NSPasteboardType dataType, size_t * len) {
    ...
    if (len != NULL)
      *len = (size_t)((NSUInteger(*)(id, SEL, int))objc_msgSend)(
          nsString, sel_registerName("maximumLengthOfBytesUsingEncoding:"), 4);
    return str;
  }
  ...
  id c_array_to_NSArray(void *array, size_t len) {
    ...
    return ((id(*)(id, SEL, void *, NSUInteger))objc_msgSend)(NSAlloc(nsclass),
                                                              func, array, len);
  }

  void NSregisterForDraggedTypes(id view, NSPasteboardType * newTypes,
                                 size_t len) {
    id *ntypes = cstrToNSStringArray((char **)newTypes, len);

    id array = c_array_to_NSArray(ntypes, len);
    objc_msgSend_void_id(view, sel_registerName("registerForDraggedTypes:"),
                         array);
    NSRelease(array);
  }

  NSInteger NSPasteBoard_declareTypes(
      id pasteboard, NSPasteboardType * newTypes, size_t len, void *owner) {
    id *ntypes = cstrToNSStringArray((char **)newTypes, len);
  ...
  bool NSPasteBoard_setString(id pasteboard, const char *stringToWrite,
                              NSPasteboardType dataType) {
    SEL func = sel_registerName("setString:forType:");
    return ((bool (*)(id, SEL, id, id))objc_msgSend)(
        pasteboard, func, NSString_stringWithUTF8String(stringToWrite),
        NSString_stringWithUTF8String(dataType));
  }
...
#define NSRetain(obj) objc_msgSend_void((id)obj, sel_registerName("retain"))
  ...
  typedef RGFW_ENUM(u32, NSBackingStoreType){NSBackingStoreRetained = 0,
  ...
  NSPasteboardType const NSPasteboardTypeString =
      "public.utf8-plain-text"; // Replaces NSStringPasteboardType

  typedef RGFW_ENUM(i32, NSDragOperation){
      NSDragOperationNone = 0,    NSDragOperationCopy = 1,
      NSDragOperationLink = 2,    NSDragOperationGeneric = 4,
      NSDragOperationPrivate = 8, NSDragOperationMove = 16,
      NSDragOperationDelete = 32, NSDragOperationEvery = ULONG_MAX,

      // NSDragOperationAll_Obsolete	API_DEPRECATED("", macos(10.0,10.10)) =
      // 15, // Use NSDragOperationEvery NSDragOperationAll API_DEPRECATED("",
      // macos(10.0,10.10)) = NSDragOperationAll_Obsolete, // Use
  ...
  */
  ...
  const char *RGFW_mouseIconSrc[] = {"arrowCursor",
                                     "arrowCursor",
                                     "IBeamCursor",
                                     "crosshairCursor",
                                     "pointingHandCursor",
  ...
  void *RGFW_getProcAddress(const char *procname) {
    ...
    void *symbol = (void *)CFBundleGetFunctionPointerForName(RGFWnsglFramework,
                                                             symbolName);
  ...
  id NSWindow_delegate(RGFW_window * win) {
    return (id)objc_msgSend_id((id)win->src.window,
                               sel_registerName("delegate"));
  }

  u32 RGFW_OnClose(id self) {
    RGFW_window *win = NULL;
    object_getInstanceVariable(self, (const char *)"RGFW_window",
                               (void **)&win);
    if (win == NULL)
      return true;
  ...
  /* NOTE(EimaMei): Fixes the constant clicking when the app is running under a
   * terminal. */
  ...
  NSDragOperation draggingUpdated(id self, SEL sel, id sender) {
    ...
    NSPoint p = ((NSPoint(*)(id, SEL))objc_msgSend)(
        sender, sel_registerName("draggingLocation"));
  ...
  /* NOTE(EimaMei): Usually, you never need 'id self, SEL cmd' for C -> Obj-C
   * methods. This isn't the case. */
  bool performDragOperation(id self, SEL sel, id sender) {
    ...
    // id pasteBoard = objc_msgSend_id(sender,
    // sel_registerName("draggingPasteboard"));

    /////////////////////////////
    id pasteBoard =
        objc_msgSend_id(sender, sel_registerName("draggingPasteboard"));
    ...
    id fileURLs = objc_msgSend_id_id(
        pasteBoard, sel_registerName("propertyListForType:"), fileURLsType);
    int count =
        ((int (*)(id, SEL))objc_msgSend)(fileURLs, sel_registerName("count"));
    ...
    for (int i = 0; i < count; i++) {
      id fileURL =
          objc_msgSend_arr(fileURLs, sel_registerName("objectAtIndex:"), i);
      const char *filePath = ((const char *(*)(id, SEL))objc_msgSend)(
          fileURL, sel_registerName("UTF8String"));
...
#define RGFW_gamepadEventQueueMAX 11
  ...
  void RGFW__osxInputValueChangedCallback(void *context, IOReturn result,
    ...
    u8 RGFW_osx2RGFWSrc[2][RGFW_gamepadFinal] = {
        {0, RGFW_gamepadSelect, RGFW_gamepadL3, RGFW_gamepadR3,
         RGFW_gamepadStart, RGFW_gamepadUp, RGFW_gamepadRight, RGFW_gamepadDown,
         RGFW_gamepadLeft, RGFW_gamepadL2, RGFW_gamepadR2, RGFW_gamepadL1,
         RGFW_gamepadR1, RGFW_gamepadY, RGFW_gamepadB, RGFW_gamepadA,
         RGFW_gamepadX, RGFW_gamepadHome},
        {0, RGFW_gamepadA, RGFW_gamepadB, RGFW_gamepadR3, RGFW_gamepadX,
         ...
         RGFW_gamepadUp, RGFW_gamepadL3, RGFW_gamepadSelect, RGFW_gamepadStart,
         RGFW_gamepadHome}};
    ...
    case kHIDPage_Button: {
      ...
      RGFW_gamepadPressed[index][button].prev =
          RGFW_gamepadPressed[index][button].current;
      RGFW_gamepadPressed[index][button].current = intValue;
      event.type =
          intValue ? RGFW_gamepadButtonPressed : RGFW_gamepadButtonReleased;
      event.button = button;
      event.gamepad = index;
    ...
    case kHIDPage_GenericDesktop: {
      ...
      if (logicalMax <= logicalMin)
        return;
      if (intValue < logicalMin)
        intValue = logicalMin;
      if (intValue > logicalMax)
        intValue = logicalMax;

      i8 value = (i8)(-100.0 + ((intValue - logicalMin) * 200.0) /
                                   (logicalMax - logicalMin));

      switch (usage) {
      case kHIDUsage_GD_X:
        RGFW_gamepadAxes[index][0].x = value;
        event.whichAxis = 0;
        break;
      case kHIDUsage_GD_Y:
        RGFW_gamepadAxes[index][0].y = value;
        event.whichAxis = 0;
        break;
      case kHIDUsage_GD_Z:
        RGFW_gamepadAxes[index][1].x = value;
        event.whichAxis = 1;
        break;
      case kHIDUsage_GD_Rz:
        RGFW_gamepadAxes[index][1].y = value;
        event.whichAxis = 1;
        break;
      default:
        return;
      ...
      RGFW_gamepadAxisCallback(RGFW_root, index, event.axis, 2,
                               event.whichAxis);
    }
    }
  }

  void RGFW__osxDeviceAddedCallback(void *context, IOReturn result,
                                    void *sender, IOHIDDeviceRef device) {
    RGFW_UNUSED(context);
    RGFW_UNUSED(result);
    RGFW_UNUSED(sender);
    CFTypeRef usageRef =
        (CFTypeRef)IOHIDDeviceGetProperty(device, CFSTR(kIOHIDPrimaryUsageKey));
    int usage = 0;
    if (usageRef)
    ...
    for (size_t i = 0; i < 4; i++) {
      ...
      IOHIDDeviceRegisterInputValueCallback(
          device, RGFW__osxInputValueChangedCallback, NULL);

      CFStringRef deviceName =
          (CFStringRef)IOHIDDeviceGetProperty(device, CFSTR(kIOHIDProductKey));
      if (deviceName)
        CFStringGetCString(deviceName, RGFW_gamepads_name[i],
                           sizeof(RGFW_gamepads_name[i]),
                           kCFStringEncodingUTF8);

      RGFW_gamepads_type[i] = RGFW_gamepadUnknown;
      if (strstr(RGFW_gamepads_name[i], "Microsoft") ||
          strstr(RGFW_gamepads_name[i], "X-Box") ||
          strstr(RGFW_gamepads_name[i], "Xbox"))
        RGFW_gamepads_type[i] = RGFW_gamepadMicrosoft;
      else if (strstr(RGFW_gamepads_name[i], "PlayStation") ||
               strstr(RGFW_gamepads_name[i], "PS3") ||
               strstr(RGFW_gamepads_name[i], "PS4") ||
               strstr(RGFW_gamepads_name[i], "PS5"))
  ...
  void RGFW__osxDeviceRemovedCallback(void *context, IOReturn result,
                                      void *sender, IOHIDDeviceRef device) {
    RGFW_UNUSED(context);
    RGFW_UNUSED(result);
    RGFW_UNUSED(sender);
    RGFW_UNUSED(device);
    CFNumberRef usageRef = (CFNumberRef)IOHIDDeviceGetProperty(
  ...
  void RGFW_osxInitIOKit(void) {
    ...
    CFDictionarySetValue(matchingDictionary, CFSTR(kIOHIDDeviceUsagePageKey),
    ...
    IOHIDManagerRegisterDeviceMatchingCallback(
        hidManager, RGFW__osxDeviceAddedCallback, NULL);
    IOHIDManagerRegisterDeviceRemovalCallback(
        hidManager, RGFW__osxDeviceRemovedCallback, NULL);
  ...
  void RGFW__osxWindowMove(id self, SEL sel) {
    ...
    NSRect frame = ((NSRect(*)(id, SEL))abi_objc_msgSend_stret)(
        (id)win->src.window, sel_registerName("frame"));
  ...
  void RGFW_window_cocoaSetLayer(RGFW_window * win, void *layer) {
    objc_msgSend_void_id((id)win->src.view, sel_registerName("setLayer"),
                         (id)layer);
  }

  void *RGFW_cocoaGetLayer(void) {
    return objc_msgSend_class((id)objc_getClass("CAMetalLayer"),
                              (SEL)sel_registerName("layer"));
  }
  ...
  RGFW_window *RGFW_createWindowPtr(const char *name, RGFW_rect rect,
    ...
    /* NOTE(EimaMei): Why does Apple hate good code? Like wtf, who thought of
    methods being a great idea??? Imagine a universe, where MacOS had a proper
    system API (we would probably have like 20% better performance).
    */
    si_func_to_SEL_with_name("NSObject", "windowShouldClose",
                             (void *)RGFW_OnClose);

    /* NOTE(EimaMei): Fixes the 'Boop' sfx from constantly playing each time you
     * click a key. Only a problem when running in the terminal. */
    si_func_to_SEL("NSWindow", acceptsFirstResponder);
    si_func_to_SEL("NSWindow", performKeyEquivalent);

    // RR Create an autorelease pool
    id pool = objc_msgSend_class(objc_getClass("NSAutoreleasePool"),
                                 sel_registerName("alloc"));
    pool = objc_msgSend_id(pool, sel_registerName("init"));

    if (NSApp == NULL) {
      NSApp = objc_msgSend_id((id)objc_getClass("NSApplication"),
                              sel_registerName("sharedApplication"));

      ((void (*)(id, SEL, NSUInteger))objc_msgSend)(
          NSApp, sel_registerName("setActivationPolicy:"),
          NSApplicationActivationPolicyRegular);
      ...
      macArgs |= NSWindowStyleMaskTitled;
    {
      ...
      win->src.window = ((id(*)(id, SEL, NSRect, NSWindowStyleMask,
                                NSBackingStoreType, bool))objc_msgSend)(
          NSAlloc(nsclass), func, windowRect, macArgs, macArgs, false);
    }

    id str = NSString_stringWithUTF8String(name);
    objc_msgSend_void_id((id)win->src.window, sel_registerName("setTitle:"),
                         str);
    ...
    if ((flags & RGFW_windowNoInitAPI) == 0) {
      ...
      /* the pixel format can be passed directly to opengl context creation to
         create a context this is because the format also includes information
         about the opengl version (which may be a bad thing) */
      win->src.view = NSOpenGLView_initWithFrame(
          (NSRect){{0, 0}, {win->r.w, win->r.h}}, (uint32_t *)format);
      objc_msgSend_void(win->src.view, sel_registerName("prepareOpenGL"));
      win->src.ctx =
          objc_msgSend_id(win->src.view, sel_registerName("openGLContext"));
    } else
#endif
    {
      NSRect contentRect = (NSRect){{0, 0}, {win->r.w, win->r.h}};
      win->src.view = ((id(*)(id, SEL, NSRect))objc_msgSend)(
          NSAlloc((id)objc_getClass("NSView")),
          sel_registerName("initWithFrame:"), contentRect);
    }

    void *contentView = NSWindow_contentView((id)win->src.window);
    objc_msgSend_void_bool(contentView, sel_registerName("setWantsLayer:"),
                           true);

    objc_msgSend_void_id((id)win->src.window,
                         sel_registerName("setContentView:"), win->src.view);
...
#define NSOpenGLCPSurfaceOpacity 236
        NSOpenGLContext_setValues((id)win->src.ctx, &opacity,
                                  NSOpenGLCPSurfaceOpacity);
      }
#endif

      objc_msgSend_void_bool(win->src.window, sel_registerName("setOpaque:"),
                             false);
    ...
    class_addIvar(delegateClass, "RGFW_window", sizeof(RGFW_window *),
                  rint(log2(sizeof(RGFW_window *))), "L");

    class_addMethod(delegateClass, sel_registerName("windowWillResize:toSize:"),
                    (IMP)RGFW__osxWindowResize, "{NSSize=ff}@:{NSSize=ff}");
    class_addMethod(delegateClass, sel_registerName("updateLayer:"),
                    (IMP)RGFW__osxUpdateLayer, "");
    class_addMethod(delegateClass, sel_registerName("windowWillMove:"),
    ...
    class_addMethod(delegateClass, sel_registerName("draggingUpdated:"),
                    (IMP)draggingUpdated, "l@:@");
    class_addMethod(delegateClass, sel_registerName("draggingExited:"),
                    (IMP)RGFW__osxDraggingEnded, "v@:@");
    class_addMethod(delegateClass, sel_registerName("draggingEnded:"),
                    (IMP)RGFW__osxDraggingEnded, "v@:@");
    class_addMethod(delegateClass, sel_registerName("prepareForDragOperation:"),
                    (IMP)prepareForDragOperation, "B@:@");
    class_addMethod(delegateClass, sel_registerName("performDragOperation:"),
                    (IMP)performDragOperation, "B@:@");

    id delegate =
        objc_msgSend_id(NSAlloc(delegateClass), sel_registerName("init"));

    object_setInstanceVariable(delegate, "RGFW_window", win);

    objc_msgSend_void_id((id)win->src.window, sel_registerName("setDelegate:"),
                         delegate);

    if (flags & RGFW_windowAllowDND) {
      win->_flags |= RGFW_windowAllowDND;

      NSPasteboardType types[] = {NSPasteboardTypeURL, NSPasteboardTypeFileURL,
    ...
    objc_msgSend_void_bool(
        NSApp, sel_registerName("activateIgnoringOtherApps:"), true);
    ((id(*)(id, SEL, SEL))objc_msgSend)(
        (id)win->src.window, sel_registerName("makeKeyAndOrderFront:"),
        (SEL)NULL);
    objc_msgSend_void_bool(win->src.window, sel_registerName("setIsVisible:"),
                           true);
  ...
  void RGFW_window_setBorder(RGFW_window * win, RGFW_bool border) {
    NSRect frame = ((NSRect(*)(id, SEL))abi_objc_msgSend_stret)(
        (id)win->src.window, sel_registerName("frame"));
    NSRect content = ((NSRect(*)(id, SEL))abi_objc_msgSend_stret)(
        (id)win->src.view, sel_registerName("frame"));
    float offset = 0;

    RGFW_setBit(&win->_flags, RGFW_windowNoBorder, !border);
    NSBackingStoreType storeType =
        NSWindowStyleMaskBorderless | NSWindowStyleMaskFullSizeContentView;
    if (border)
      storeType = NSWindowStyleMaskTitled | NSWindowStyleMaskClosable |
                  NSWindowStyleMaskMiniaturizable;
    if (!(win->_flags & RGFW_windowNoResize)) {
      storeType |= NSWindowStyleMaskResizable;
    }

    ((void (*)(id, SEL, NSBackingStoreType))objc_msgSend)(
        (id)win->src.window, sel_registerName("setStyleMask:"), storeType);

    if (!border) {
      id miniaturizeButton = objc_msgSend_int(
          (id)win->src.window, sel_registerName("standardWindowButton:"),
          NSWindowMiniaturizeButton);
      id titleBarView =
          objc_msgSend_id(miniaturizeButton, sel_registerName("superview"));
      objc_msgSend_void_bool(titleBarView, sel_registerName("setHidden:"),
                             true);
  ...
  RGFW_area RGFW_getScreenSize(void) {
    ...
    return RGFW_AREA(CGDisplayPixelsWide(display),
  ...
  RGFW_point RGFW_getGlobalMousePoint(void) {
    ...
    return RGFW_POINT(
        (u32)point.x,
        (u32)point.y); /*!< the point is loaded during event checks */
  }

  u32 RGFW_keysPressed[10]; /*10 keys at a time*/
  typedef RGFW_ENUM(u32, NSEventType){
  ...
  typedef RGFW_ENUM(unsigned long long, NSEventMask){
      ...
      /* The following event masks are available on some hardware on 10.5.2 and
         later */
      NSEventMaskGesture API_AVAILABLE(macos(10.5)) = 1ULL
                                                      << NSEventTypeGesture,
      NSEventMaskMagnify API_AVAILABLE(macos(10.5)) = 1ULL
                                                      << NSEventTypeMagnify,
      ...
      NSEventMaskBeginGesture API_AVAILABLE(macos(10.5)) =
          1ULL << NSEventTypeBeginGesture,
      NSEventMaskEndGesture API_AVAILABLE(macos(10.5)) =
          1ULL << NSEventTypeEndGesture,

      /* Note: You can only use these event masks on 64 bit. In other words, you
       * cannot setup a local, nor global, event monitor for these event types
       * on 32 bit. Also, you cannot search the event queue for them
       * (nextEventMatchingMask:...) on 32 bit.
       */
      NSEventMaskSmartMagnify API_AVAILABLE(macos(10.8)) =
          1ULL << NSEventTypeSmartMagnify,
      NSEventMaskPressure API_AVAILABLE(macos(10.10.3)) =
          1ULL << NSEventTypePressure,
      NSEventMaskDirectTouch API_AVAILABLE(macos(10.12.2)) =
          1ULL << NSEventTypeDirectTouch,

      NSEventMaskChangeMode API_AVAILABLE(macos(10.15)) =
          1ULL << NSEventTypeChangeMode,
  ...
  void RGFW_stopCheckEvents(void) {
    ...
    id e = (id)((id(*)(id, SEL, NSEventType, NSPoint, NSEventModifierFlags,
                       void *, NSInteger, void **, short, NSInteger,
        ...
        NSEventTypeApplicationDefined, (NSPoint){0, 0}, (NSEventModifierFlags)0,
        NULL, (NSInteger)0, NULL, 0, 0, 0);
  ...
  void RGFW_window_eventWait(RGFW_window * win, u32 waitMS) {
    ...
    void *date = (void *)((id(*)(Class, SEL, double))objc_msgSend)(
        ...
        sel_registerName("dateWithTimeIntervalSinceNow:"), waitMS);

    id e = (id)((id(*)(id, SEL, NSEventMask, void *, id, bool))objc_msgSend)(
        NSApp,
        sel_registerName("nextEventMatchingMask:untilDate:inMode:dequeue:"),
        ULONG_MAX, date, NSString_stringWithUTF8String("kCFRunLoopDefaultMode"),
        true);
  ...
  RGFW_event *RGFW_window_checkEvent(RGFW_window * win) {
    ...
    if ((win->event.type == RGFW_DND || win->event.type == RGFW_DNDInit) &&
        win->src.dndPassed == 0) {
      win->src.dndPassed = 1;
      ((void (*)(id, SEL))objc_msgSend)(NSApp,
                                        sel_registerName("updateWindows"));
      return &win->event;
    }
    ...
    if (RGFW_gamepadEventQueueCount && win == RGFW_root) {
      ...
      if (RGFW_gamepadEventQueueCount)
        index++;
      else
        index = 0;
    ...
    id e = (id)((id(*)(id, SEL, NSEventMask, void *, id, bool))objc_msgSend)(
        NSApp, eventFunc, ULONG_MAX, date,
        NSString_stringWithUTF8String("kCFRunLoopDefaultMode"), true);
    ...
    case NSEventTypeMouseEntered: {
      win->event.type = RGFW_mouseEnter;
      NSPoint p = ((NSPoint(*)(id, SEL))objc_msgSend)(
          e, sel_registerName("locationInWindow"));
    ...
    case NSEventTypeKeyDown: {
      ...
      u32 mappedKey =
          *((u32 *)((char *)(const char *)NSString_to_char(objc_msgSend_id(
              e, sel_registerName("charactersIgnoringModifiers")))));
      if (((u8)mappedKey) == 239)
        mappedKey = 0;
      ...
      RGFW_keyboard[win->event.key].prev =
          RGFW_keyboard[win->event.key].current;
    ...
    case NSEventTypeFlagsChanged: {
      ...
      RGFW_keyCallback(win, win->event.key, win->event.keyChar,
                       win->event.keyMod, win->event.type == RGFW_keyPressed);
    ...
    case NSEventTypeMouseMoved: {
      ...
      p.x = ((CGFloat(*)(id, SEL))abi_objc_msgSend_fpret)(
          e, sel_registerName("deltaX"));
      p.y = ((CGFloat(*)(id, SEL))abi_objc_msgSend_fpret)(
          e, sel_registerName("deltaY"));
    ...
    case NSEventTypeScrollWheel: {
      double deltaY = ((CGFloat(*)(id, SEL))abi_objc_msgSend_fpret)(
          e, sel_registerName("deltaY"));
  ...
  void RGFW_window_move(RGFW_window * win, RGFW_point v) {
    ...
    ((void (*)(id, SEL, NSRect, bool, bool))objc_msgSend)(
        (id)win->src.window, sel_registerName("setFrame:display:animate:"),
        (NSRect){{win->r.x, win->r.y}, {win->r.w, win->r.h}}, true, true);
  }
  ...
  void RGFW_window_resize(RGFW_window * win, RGFW_area a) {
    ...
    ((void (*)(id, SEL, NSRect, bool, bool))objc_msgSend)(
        (id)win->src.window, sel_registerName("setFrame:display:animate:"),
        (NSRect){{win->r.x, win->r.y}, {win->r.w, win->r.h + offset}}, true,
        true);
  }
  ...
  void RGFW_window_setFullscreen(RGFW_window * win, RGFW_bool fullscreen) {
    ...
    objc_msgSend_void_SEL(win->src.window,
  ...
  void RGFW_window_minimize(RGFW_window * win) {
    RGFW_ASSERT(win != NULL);
    objc_msgSend_void_SEL(win->src.window,
                          sel_registerName("performMiniaturize:"), NULL);
  }
  ...
  void RGFW_window_restore(RGFW_window * win) {
    ...
    objc_msgSend_void_SEL(win->src.window, sel_registerName("deminiaturize:"),
                          NULL);
  }
  ...
  void RGFW_window_setMousePassthrough(RGFW_window * win,
                                       RGFW_bool passthrough) {
    objc_msgSend_void_bool(win->src.window,
  ...
  void RGFW_window_setAspectRatio(RGFW_window * win, RGFW_area a) {
    ...
    ((void (*)(id, SEL, NSSize))objc_msgSend)(
        (id)win->src.window, sel_registerName("setContentAspectRatio:"),
        (NSSize){a.w, a.h});
  }

  void RGFW_window_setMinSize(RGFW_window * win, RGFW_area a) {
    ((void (*)(id, SEL, NSSize))objc_msgSend)((id)win->src.window,
  ...
  void RGFW_window_setMaxSize(RGFW_window * win, RGFW_area a) {
    ...
    ((void (*)(id, SEL, NSSize))objc_msgSend)((id)win->src.window,
                                              sel_registerName("setMaxSize:"),
                                              (NSSize){a.w, a.h});
  }

  RGFW_bool RGFW_window_setIcon(RGFW_window * win, u8 * data, RGFW_area area,
                                i32 channels) {
    RGFW_ASSERT(win != NULL);

    /* code by EimaMei  */
    // Make a bitmap representation, then copy the loaded image into it.
    id representation = NSBitmapImageRep_initWithBitmapData(
        NULL, area.w, area.h, 8, channels, (channels == 4), false,
        "NSCalibratedRGBColorSpace", 1 << 1, area.w * channels, 8 * channels);
    RGFW_MEMCPY(NSBitmapImageRep_bitmapData(representation), data,
                area.w * area.h * channels);
    ...
    objc_msgSend_void_id(NSApp, sel_registerName("setApplicationIconImage:"),
                         dock_image);
    // Free the garbage.
  ...
  RGFW_mouse *RGFW_loadMouse(u8 * icon, RGFW_area a, i32 channels) {
    if (icon == NULL) {
      objc_msgSend_void(NSCursor_arrowStr("arrowCursor"),
    ...
    // Make a bitmap representation, then copy the loaded image into it.
    id representation = NSBitmapImageRep_initWithBitmapData(
        NULL, a.w, a.h, 8, channels, (channels == 4), false,
        "NSCalibratedRGBColorSpace", 1 << 1, a.w * channels, 8 * channels);
    RGFW_MEMCPY(NSBitmapImageRep_bitmapData(representation), icon,
                a.w * a.h * channels);
  ...
  void RGFW_window_show(RGFW_window * win) {
    ((id(*)(id, SEL, SEL))objc_msgSend)(
        (id)win->src.window, sel_registerName("makeKeyAndOrderFront:"), NULL);
    objc_msgSend_void_bool(win->src.window, sel_registerName("setIsVisible:"),
                           true);
  }
  ...
  RGFW_bool RGFW_window_isHidden(RGFW_window * win) {
    ...
    bool visible =
        objc_msgSend_bool(win->src.window, sel_registerName("isVisible"));
  ...
  RGFW_bool RGFW_window_isMinimized(RGFW_window * win) {
    ...
    return objc_msgSend_bool(win->src.window,
                             sel_registerName("isMiniaturized")) == YES;
  }

  RGFW_bool RGFW_window_isMaximized(RGFW_window * win) {
    RGFW_ASSERT(win != NULL);
    RGFW_bool b =
        objc_msgSend_bool(win->src.window, sel_registerName("isZoomed"));
    return b;
  }
  ...
  id RGFW_getNSScreenForDisplayID(CGDirectDisplayID display) {
    ...
    NSUInteger count =
        (NSUInteger)objc_msgSend_uint(screens, sel_registerName("count"));

    for (NSUInteger i = 0; i < count; i++) {
      id screen = ((id(*)(id, SEL, int))objc_msgSend)(
          screens, sel_registerName("objectAtIndex:"), (int)i);
      id description =
          objc_msgSend_id(screen, sel_registerName("deviceDescription"));
      id screenNumberKey = NSString_stringWithUTF8String("NSScreenNumber");
      id screenNumber = objc_msgSend_id_id(
          description, sel_registerName("objectForKey:"), screenNumberKey);

      if ((CGDirectDisplayID)objc_msgSend_uint(
              screenNumber, sel_registerName("unsignedIntValue")) == display) {
        return screen;
      }
    }
  ...
  RGFW_monitor RGFW_NSCreateMonitor(CGDirectDisplayID display, id screen) {
    ...
    monitor.rect = RGFW_RECT((int)bounds.origin.x, (int)bounds.origin.y,
    ...
    monitor.pixelRatio = ((CGFloat(*)(id, SEL))abi_objc_msgSend_fpret)(
  ...
  RGFW_monitor *RGFW_getMonitors(void) {
    ...
    for (u32 i = 0; i < count; i++)
      RGFW_monitors[i] = RGFW_NSCreateMonitor(
          displays[i], RGFW_getNSScreenForDisplayID(displays[i]));
  ...
  RGFW_bool RGFW_monitor_scale(RGFW_monitor mon, RGFW_area area,
    ...
    for (CFIndex i = 0; i < CFArrayGetCount(allModes); i++) {
      ...
      if (CGDisplayModeGetWidth(mode) == area.w &&
          CGDisplayModeGetHeight(mode) == area.h &&
          (refreshRate == 0 ||
           CGDisplayModeGetRefreshRate(mode) == refreshRate)) {
  ...
  RGFW_monitor RGFW_window_getMonitor(RGFW_window * win) {
    ...
    CGDirectDisplayID display = (CGDirectDisplayID)objc_msgSend_uint(
        screenNumber, sel_registerName("unsignedIntValue"));
  ...
  RGFW_ssize_t RGFW_readClipboardPtr(char *str, size_t strCapacity) {
    size_t clip_len;
    char *clip = (char *)NSPasteboard_stringForType(
        NSPasteboard_generalPasteboard(), NSPasteboardTypeString, &clip_len);
    if (clip == NULL)
      return -1;
  ...
  void RGFW_writeClipboard(const char *text, u32 textLen) {
    ...
    NSPasteBoard_setString(NSPasteboard_generalPasteboard(), text,
                           NSPasteboardTypeString);
  }
  ...
  CGImageRef createImageFromBytes(unsigned char *buffer, int width,
                                  int height) {
    // Define color space
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    // Create bitmap context
    CGContextRef context =
        CGBitmapContextCreate(buffer, width, height, 8, width * 4, colorSpace,
  ...
  void RGFW_window_swapBuffers(RGFW_window * win) {
    ...
    if (!(win->_flags & RGFW_NO_CPU_RENDER)) {
      ...
      id graphicsContext =
          objc_msgSend_class(objc_getClass("NSGraphicsContext"),
                             sel_registerName("currentContext"));
      // Get the CGContext from the current NSGraphicsContext
      id cgContext =
          objc_msgSend_id(graphicsContext, sel_registerName("graphicsPort"));
  ...
  u64 RGFW_getTime(void) {
    ...
    return (double)mach_absolute_time() * (double)timebase_info.numer /
  ...
  */
  ...
  EM_BOOL Emscripten_on_resize(int eventType, const EmscriptenUiEvent *e,
                               void *userData) {
  ...
  EM_BOOL Emscripten_on_fullscreenchange(
      int eventType, const EmscriptenFullscreenChangeEvent *e, void *userData) {
...
#if __EMSCRIPTEN_major__ >= 1 && __EMSCRIPTEN_minor__ >= 29 &&                 \
    __EMSCRIPTEN_tiny__ >= 0
      EmscriptenFullscreenStrategy FSStrat = {0};
      FSStrat.scaleMode =
          EMSCRIPTEN_FULLSCREEN_SCALE_STRETCH; // EMSCRIPTEN_FULLSCREEN_SCALE_ASPECT;//
                                               // :
                                               // EMSCRIPTEN_FULLSCREEN_SCALE_STRETCH;
      FSStrat.canvasResolutionScaleMode =
    ...
    emscripten_set_canvas_element_size("#canvas", RGFW_root->r.w,
                                       RGFW_root->r.h);
  ...
  EM_BOOL Emscripten_on_focusin(int eventType, const EmscriptenFocusEvent *e,
                                void *userData) {
    RGFW_UNUSED(eventType);
    RGFW_UNUSED(userData);
    RGFW_UNUSED(e);
  ...
  EM_BOOL Emscripten_on_focusout(int eventType, const EmscriptenFocusEvent *e,
                                 void *userData) {
    RGFW_UNUSED(eventType);
    RGFW_UNUSED(userData);
    RGFW_UNUSED(e);
  ...
  EM_BOOL Emscripten_on_mousemove(int eventType, const EmscriptenMouseEvent *e,
                                  void *userData) {
    RGFW_UNUSED(eventType);
    RGFW_UNUSED(userData);
    ...
    RGFW_mousePosCallback(RGFW_root, RGFW_events[RGFW_eventLen].point,
                          RGFW_events[RGFW_eventLen].vector);
    return EM_TRUE;
  }

  EM_BOOL Emscripten_on_mousedown(int eventType, const EmscriptenMouseEvent *e,
                                  void *userData) {
    RGFW_UNUSED(eventType);
    RGFW_UNUSED(userData);
    ...
    RGFW_mouseButtons[RGFW_events[RGFW_eventLen].button].prev =
        RGFW_mouseButtons[RGFW_events[RGFW_eventLen].button].current;
    RGFW_mouseButtons[RGFW_events[RGFW_eventLen].button].current = 1;

    RGFW_mouseButtonCallback(RGFW_root, RGFW_events[RGFW_eventLen].button,
                             RGFW_events[RGFW_eventLen].scroll, 1);
    RGFW_eventLen++;

    return EM_TRUE;
  }

  EM_BOOL Emscripten_on_mouseup(int eventType, const EmscriptenMouseEvent *e,
                                void *userData) {
    RGFW_UNUSED(eventType);
    RGFW_UNUSED(userData);
  ...
  EM_BOOL Emscripten_on_wheel(int eventType, const EmscriptenWheelEvent *e,
                              void *userData) {
    RGFW_UNUSED(eventType);
    RGFW_UNUSED(userData);

    RGFW_events[RGFW_eventLen].type = RGFW_mouseButtonPressed;
    RGFW_events[RGFW_eventLen].point =
        RGFW_POINT(e->mouse.targetX, e->mouse.targetY);
  ...
  EM_BOOL Emscripten_on_touchstart(int eventType, const EmscriptenTouchEvent *e,
                                   void *userData) {
    RGFW_UNUSED(eventType);
    RGFW_UNUSED(userData);
  ...
  EM_BOOL Emscripten_on_touchmove(int eventType, const EmscriptenTouchEvent *e,
                                  void *userData) {
    RGFW_UNUSED(eventType);
    RGFW_UNUSED(userData);
  ...
  EM_BOOL Emscripten_on_touchend(int eventType, const EmscriptenTouchEvent *e,
                                 void *userData) {
    RGFW_UNUSED(eventType);
    RGFW_UNUSED(userData);
  ...
  EM_BOOL Emscripten_on_touchcancel(
  ...
  EM_BOOL Emscripten_on_gamepad(int eventType,
    ...
    if (gamepadEvent->connected) {
      RGFW_MEMCPY(RGFW_gamepads_name[gamepadEvent->index], gamepadEvent->id,
                  sizeof(RGFW_gamepads_name[gamepadEvent->index]));
      RGFW_gamepads_type[i] = RGFW_gamepadUnknown;
      if (strstr(RGFW_gamepads_name[i], "Microsoft") ||
          strstr(RGFW_gamepads_name[i], "X-Box"))
        RGFW_gamepads_type[i] = RGFW_gamepadMicrosoft;
    ...
    RGFW_gamepadCallback(RGFW_root, gamepadEvent->index,
  ...
  u32 RGFW_wASMPhysicalToRGFW(u32 hash) {
    switch (hash) { /* 0x0000 */
    case 0x67243A2DU /* Escape             */:
      return RGFW_escape; /* 0x0001 */
    case 0x67251058U /* Digit0             */:
      return RGFW_0; /* 0x0002 */
    case 0x67251059U /* Digit1             */:
      return RGFW_1; /* 0x0003 */
    case 0x6725105AU /* Digit2             */:
      return RGFW_2; /* 0x0004 */
    case 0x6725105BU /* Digit3             */:
      return RGFW_3; /* 0x0005 */
    case 0x6725105CU /* Digit4             */:
      return RGFW_4; /* 0x0006 */
    case 0x6725105DU /* Digit5             */:
      return RGFW_5; /* 0x0007 */
    case 0x6725105EU /* Digit6             */:
      return RGFW_6; /* 0x0008 */
    case 0x6725105FU /* Digit7             */:
      return RGFW_7; /* 0x0009 */
    case 0x67251050U /* Digit8             */:
      return RGFW_8; /* 0x000A */
    case 0x67251051U /* Digit9             */:
      return RGFW_9; /* 0x000B */
    case 0x92E14DD3U /* Minus              */:
      return RGFW_minus; /* 0x000C */
    case 0x92E1FBACU /* Equal              */:
      return RGFW_equals; /* 0x000D */
    case 0x36BF1CB5U /* Backspace          */:
      return RGFW_backSpace; /* 0x000E */
    case 0x7B8E51E2U /* Tab                */:
      return RGFW_tab; /* 0x000F */
    case 0x2C595B51U /* KeyQ               */:
      return RGFW_q; /* 0x0010 */
    case 0x2C595B57U /* KeyW               */:
      return RGFW_w; /* 0x0011 */
    case 0x2C595B45U /* KeyE               */:
      return RGFW_e; /* 0x0012 */
    case 0x2C595B52U /* KeyR               */:
      return RGFW_r; /* 0x0013 */
    case 0x2C595B54U /* KeyT               */:
      return RGFW_t; /* 0x0014 */
    case 0x2C595B59U /* KeyY               */:
      return RGFW_y; /* 0x0015 */
    case 0x2C595B55U /* KeyU               */:
      return RGFW_u; /* 0x0016 */
    case 0x2C595B4FU /* KeyO               */:
      return RGFW_o; /* 0x0018 */
    case 0x2C595B50U /* KeyP               */:
      return RGFW_p; /* 0x0019 */
    case 0x45D8158CU /* BracketLeft        */:
      return RGFW_closeBracket; /* 0x001A */
    case 0xDEEABF7CU /* BracketRight       */:
      return RGFW_bracket; /* 0x001B */
    case 0x92E1C5D2U /* Enter              */:
      return RGFW_return; /* 0x001C */
    case 0xE058958CU /* ControlLeft        */:
      return RGFW_controlL; /* 0x001D */
    case 0x2C595B41U /* KeyA               */:
      return RGFW_a; /* 0x001E */
    case 0x2C595B53U /* KeyS               */:
      return RGFW_s; /* 0x001F */
    case 0x2C595B44U /* KeyD               */:
      return RGFW_d; /* 0x0020 */
    case 0x2C595B46U /* KeyF               */:
      return RGFW_f; /* 0x0021 */
    case 0x2C595B47U /* KeyG               */:
      return RGFW_g; /* 0x0022 */
    case 0x2C595B48U /* KeyH               */:
      return RGFW_h; /* 0x0023 */
    case 0x2C595B4AU /* KeyJ               */:
      return RGFW_j; /* 0x0024 */
    case 0x2C595B4BU /* KeyK               */:
      return RGFW_k; /* 0x0025 */
    case 0x2C595B4CU /* KeyL               */:
      return RGFW_l; /* 0x0026 */
    case 0x2707219EU /* Semicolon          */:
      return RGFW_semicolon; /* 0x0027 */
    case 0x92E0B58DU /* Quote              */:
      return RGFW_apostrophe; /* 0x0028 */
    case 0x36BF358DU /* Backquote          */:
      return RGFW_backtick; /* 0x0029 */
    case 0x26B1958CU /* ShiftLeft          */:
      return RGFW_shiftL; /* 0x002A */
    case 0x36BF2438U /* Backslash          */:
      return RGFW_backSlash; /* 0x002B */
    case 0x2C595B5AU /* KeyZ               */:
      return RGFW_z; /* 0x002C */
    case 0x2C595B58U /* KeyX               */:
      return RGFW_x; /* 0x002D */
    case 0x2C595B43U /* KeyC               */:
      return RGFW_c; /* 0x002E */
    case 0x2C595B56U /* KeyV               */:
      return RGFW_v; /* 0x002F */
    case 0x2C595B42U /* KeyB               */:
      return RGFW_b; /* 0x0030 */
    case 0x2C595B4EU /* KeyN               */:
      return RGFW_n; /* 0x0031 */
    case 0x2C595B4DU /* KeyM               */:
      return RGFW_m; /* 0x0032 */
    case 0x92E1A1C1U /* Comma              */:
      return RGFW_comma; /* 0x0033 */
    case 0x672FFAD4U /* Period             */:
      return RGFW_period; /* 0x0034 */
    case 0x92E0A438U /* Slash              */:
      return RGFW_slash; /* 0x0035 */
    case 0xC5A6BF7CU /* ShiftRight         */:
      return RGFW_shiftR;
    case 0x5D64DA91U /* NumpadMultiply     */:
      return RGFW_multiply;
    case 0xC914958CU /* AltLeft            */:
      return RGFW_altL; /* 0x0038 */
    case 0x92E09CB5U /* Space              */:
      return RGFW_space; /* 0x0039 */
    case 0xB8FAE73BU /* CapsLock           */:
      return RGFW_capsLock; /* 0x003A */
    case 0x7174B789U /* F1                 */:
      return RGFW_F1; /* 0x003B */
    case 0x7174B78AU /* F2                 */:
      return RGFW_F2; /* 0x003C */
    case 0x7174B78BU /* F3                 */:
      return RGFW_F3; /* 0x003D */
    case 0x7174B78CU /* F4                 */:
      return RGFW_F4; /* 0x003E */
    case 0x7174B78DU /* F5                 */:
      return RGFW_F5; /* 0x003F */
    case 0x7174B78EU /* F6                 */:
      return RGFW_F6; /* 0x0040 */
    case 0x7174B78FU /* F7                 */:
      return RGFW_F7; /* 0x0041 */
    case 0x7174B780U /* F8                 */:
      return RGFW_F8; /* 0x0042 */
    case 0x7174B781U /* F9                 */:
      return RGFW_F9; /* 0x0043 */
    case 0x7B8E57B0U /* F10                */:
      return RGFW_F10; /* 0x0044 */
    case 0xC925FCDFU /* Numpad7            */:
      return RGFW_multiply; /* 0x0047 */
    case 0xC925FCD0U /* Numpad8            */:
      return RGFW_KP_8; /* 0x0048 */
    case 0xC925FCD1U /* Numpad9            */:
      return RGFW_KP_9; /* 0x0049 */
    case 0x5EA3E8A4U /* NumpadSubtract     */:
      return RGFW_minus; /* 0x004A */
    case 0xC925FCDCU /* Numpad4            */:
      return RGFW_KP_4; /* 0x004B */
    case 0xC925FCDDU /* Numpad5            */:
      return RGFW_KP_5; /* 0x004C */
    case 0xC925FCDEU /* Numpad6            */:
      return RGFW_KP_6; /* 0x004D */
    case 0xC925FCD9U /* Numpad1            */:
      return RGFW_KP_1; /* 0x004F */
    case 0xC925FCDAU /* Numpad2            */:
      return RGFW_KP_2; /* 0x0050 */
    case 0xC925FCDBU /* Numpad3            */:
      return RGFW_KP_3; /* 0x0051 */
    case 0xC925FCD8U /* Numpad0            */:
      return RGFW_KP_0; /* 0x0052 */
    case 0x95852DACU /* NumpadDecimal      */:
      return RGFW_period; /* 0x0053 */
    case 0x7B8E57B1U /* F11                */:
      return RGFW_F11; /* 0x0057 */
    case 0x7B8E57B2U /* F12                */:
      return RGFW_F12; /* 0x0058 */
    case 0x7393FBACU /* NumpadEqual        */:
      return RGFW_KP_Return;
    case 0xB88EBF7CU /* AltRight           */:
      return RGFW_altR; /* 0xE038 */
    case 0xC925873BU /* NumLock            */:
      return RGFW_numLock; /* 0xE045 */
    case 0x2C595F45U /* Home               */:
      return RGFW_home; /* 0xE047 */
    case 0xC91BB690U /* ArrowUp            */:
      return RGFW_up; /* 0xE048 */
    case 0x672F9210U /* PageUp             */:
      return RGFW_pageUp; /* 0xE049 */
    case 0x3799258CU /* ArrowLeft          */:
      return RGFW_left; /* 0xE04B */
    case 0x4CE33F7CU /* ArrowRight         */:
      return RGFW_right; /* 0xE04D */
    case 0x7B8E55DCU /* End                */:
      return RGFW_end; /* 0xE04F */
    case 0x3799379EU /* ArrowDown          */:
      return RGFW_down; /* 0xE050 */
    case 0xBA90179EU /* PageDown           */:
      return RGFW_pageDown; /* 0xE051 */
    case 0x6723CB2CU /* Insert             */:
      return RGFW_insert; /* 0xE052 */
    case 0x6725C50DU /* Delete             */:
      return RGFW_delete; /* 0xE053 */
    case 0x6723658CU /* OSLeft             */:
      return RGFW_superL; /* 0xE05B */
    case 0x39643F7CU /* MetaRight          */:
      return RGFW_superR; /* 0xE05C */
    }

    return 0;
  }

  void EMSCRIPTEN_KEEPALIVE RGFW_handleKeyEvent(char *key, char *code,
                                                RGFW_bool press) {
    const char *iCode = code;

    u32 hash = 0;
    while (*iCode)
      hash = ((hash ^ 0x7E057D79U) << 3) ^ (unsigned int)*iCode++;
    ...
    RGFW_events[RGFW_eventLen].type =
        press ? RGFW_keyPressed : RGFW_keyReleased;
    ...
    RGFW_keyCallback(RGFW_root, physicalKey, mappedKey, RGFW_root->event.keyMod,
                     press);
  }

  void EMSCRIPTEN_KEEPALIVE RGFW_handleKeyMods(
      RGFW_bool capital, RGFW_bool numlock, RGFW_bool control, RGFW_bool alt,
      RGFW_bool shift, RGFW_bool super, RGFW_bool scroll) {
    RGFW_updateKeyModsPro(RGFW_root, capital, numlock, control, alt, shift,
                          super, scroll);
  }
  ...
  void RGFW_stopCheckEvents(void) { RGFW_stopCheckEvents_bool = RGFW_TRUE; }
  ...
  void RGFW_window_eventWait(RGFW_window * win, u32 waitMS) {
    ...
    while ((RGFW_eventLen == 0) && RGFW_stopCheckEvents_bool == RGFW_FALSE &&
           (waitMS != RGFW_eventWaitNext ||
            (RGFW_getTimeNS() / 1e+6) - start < waitMS)) {
      emscripten_sleep(0);
    }
  ...
  void EMSCRIPTEN_KEEPALIVE RGFW_makeSetValue(size_t index, char *file) {
    /* This seems like a terrible idea, don't replicate this unless you hate
     * yourself or the OS */
    /* TODO: find a better way to do this
     */

    RGFW_events[RGFW_eventLen].type = RGFW_DND;
    RGFW_MEMCPY((char *)RGFW_events[RGFW_eventLen].droppedFiles[index], file,
                RGFW_MAX_PATH);
  }
...
#include <sys/types.h>
  ...
  void EMSCRIPTEN_KEEPALIVE RGFW_writeFile(const char *path, const char *data,
                                           size_t len) {
  ...
  win->src.queue = wgpuDeviceGetQueue(win->src.device);
    ...
    /* load callbacks */
    emscripten_set_resize_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL,
                                   EM_FALSE, Emscripten_on_resize);
    emscripten_set_fullscreenchange_callback(EMSCRIPTEN_EVENT_TARGET_DOCUMENT,
                                             NULL, EM_FALSE,
                                             Emscripten_on_fullscreenchange);
    emscripten_set_mousemove_callback("#canvas", NULL, EM_FALSE,
                                      Emscripten_on_mousemove);
    emscripten_set_touchstart_callback("#canvas", NULL, EM_FALSE,
                                       Emscripten_on_touchstart);
    emscripten_set_touchend_callback("#canvas", NULL, EM_FALSE,
                                     Emscripten_on_touchend);
    emscripten_set_touchmove_callback("#canvas", NULL, EM_FALSE,
                                      Emscripten_on_touchmove);
    emscripten_set_touchcancel_callback("#canvas", NULL, EM_FALSE,
                                        Emscripten_on_touchcancel);
    emscripten_set_mousedown_callback("#canvas", NULL, EM_FALSE,
                                      Emscripten_on_mousedown);
    emscripten_set_mouseup_callback("#canvas", NULL, EM_FALSE,
                                    Emscripten_on_mouseup);
    emscripten_set_wheel_callback("#canvas", NULL, EM_FALSE,
                                  Emscripten_on_wheel);
    emscripten_set_focusin_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL,
                                    EM_FALSE, Emscripten_on_focusin);
    emscripten_set_focusout_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL,
                                     EM_FALSE, Emscripten_on_focusout);
    ...
    EM_ASM({
      window.addEventListener(
          ...
          (event) = >
                    {
                      var key = stringToNewUTF8(event.key);
                      var code = stringToNewUTF8(event.code);
                      Module._RGFW_handleKeyMods(
            ...
            for (var i = 0; i < count; i++) {
              ...
              reader.onloadend = (e) = > {
                if (reader.readyState != 2) {
                  out('failed to read dropped file: ' + file.name + ': ' +
                      reader.error);
                } else {
  ...
  RGFW_event *RGFW_window_checkEvent(RGFW_window * win) {
    ...
    for (int i = 0; (i < emscripten_get_num_gamepads()) && (i < 4); i++) {
      ...
      if (emscripten_get_gamepad_status(i, &gamepadState) !=
          EMSCRIPTEN_RESULT_SUCCESS)
        break;

      // Register buttons data for every connected gamepad
      for (int j = 0; (j < gamepadState.numButtons) && (j < 16); j++) {
        u32 map[] = {RGFW_gamepadA,     RGFW_gamepadB,    RGFW_gamepadX,
                     RGFW_gamepadY,     RGFW_gamepadL1,   RGFW_gamepadR1,
                     RGFW_gamepadL2,    RGFW_gamepadR2,   RGFW_gamepadSelect,
                     RGFW_gamepadStart, RGFW_gamepadL3,   RGFW_gamepadR3,
                     RGFW_gamepadUp,    RGFW_gamepadDown, RGFW_gamepadLeft,
                     RGFW_gamepadRight, RGFW_gamepadHome};

        u32 button = map[j];
        if (button == 404)
          continue;

        if (RGFW_gamepadPressed[i][button].current !=
            gamepadState.digitalButton[j]) {
          ...
          RGFW_gamepadPressed[i][button].prev =
              RGFW_gamepadPressed[i][button].current;
          RGFW_gamepadPressed[i][button].current =
              gamepadState.digitalButton[j];

          RGFW_gamepadButtonCallback(win, win->event.gamepad, win->event.button,
                                     gamepadState.digitalButton[j]);
          return &win->event;
        }
      }
      ...
      for (int j = 0; (j < gamepadState.numAxes) && (j < 4); j += 2) {
        ...
        if (RGFW_gamepadAxes[i][(size_t)(j / 2)].x !=
                (i8)(gamepadState.axis[j] * 100.0f) ||
            RGFW_gamepadAxes[i][(size_t)(j / 2)].y !=
                (i8)(gamepadState.axis[j + 1] * 100.0f)) {

          RGFW_gamepadAxes[i][(size_t)(j / 2)].x =
              (i8)(gamepadState.axis[j] * 100.0f);
          RGFW_gamepadAxes[i][(size_t)(j / 2)].y =
              (i8)(gamepadState.axis[j + 1] * 100.0f);
          win->event.axis[(size_t)(j / 2)] =
              RGFW_gamepadAxes[i][(size_t)(j / 2)];

          win->event.type = RGFW_gamepadAxisMove;
          win->event.gamepad = i;
          win->event.whichAxis = j / 2;

          RGFW_gamepadAxisCallback(win, win->event.gamepad, win->event.axis,
                                   win->event.axisesCount,
                                   win->event.whichAxis);
          return &win->event;
        }
      }
  ...
  RGFW_point RGFW_getGlobalMousePoint(void) {
    RGFW_point point;
    point.x = EM_ASM_INT({ return window.mouseX || 0; });
    point.y = EM_ASM_INT({ return window.mouseY || 0; });
    return point;
  }
  ...
  void RGFW_window_swapBuffers(RGFW_window * win) {
    ...
    if (!(win->_flags & RGFW_NO_CPU_RENDER)) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, win->bufferSize.w,
                   win->bufferSize.h, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                   win->buffer);
  ...
  void RGFW_sleep(u64 milisecond) { emscripten_sleep(milisecond); }

  u64 RGFW_getTimeNS(void) { return emscripten_get_now() * 1e+6; }

  u64 RGFW_getTime(void) { return emscripten_get_now() * 1000; }
...
#if defined(__linux__)
  void RGFW_setThreadPriority(RGFW_thread thread, u8 priority) {
    pthread_setschedprio((pthread_t)thread, priority);
  }
#else
void RGFW_setThreadPriority(RGFW_thread thread, u8 priority) {
  RGFW_UNUSED(thread);
  RGFW_UNUSED(priority);
}
#endif
#endif
